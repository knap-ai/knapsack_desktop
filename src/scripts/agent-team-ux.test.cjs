...
test('background agent replies produce unread indicators until selected', () => {
  ...
  assert.match(chat, /if \(!active\) return\s*let cancelled = false\s*const timers = new Set/)
  assert.match(chat, /schedule\(pollGateway, HEALTH_POLL_INTERVAL_MS\)/)
  assert.match(chat, /timers\.forEach\(timer => clearTimeout\(timer\)\)/)
  assert.match(chat, /const registerCleanup = \(unlisten: \(\) => void\)/)
  assert.match(chat, /if \(cancelled\) \{\s*unlisten\(\)/)
  ...
  assert.match(chat, /Listen for Claude Code started[\s\S]*?useEffect\(\(\) => \{\s*if \(!active\) return/)
  assert.match(chat, /const registerCleanup = \(unlisten: \(\) => void\)[\s\S]*?if \(cancelled\) \{\s*unlisten\(\)/)
  assert.match(chat, /if \(!active\) return\s*void refreshStudioConnections\(\)/)
  assert.match(chat, /\}, \[active, refreshStudioConnections\]\)/)
