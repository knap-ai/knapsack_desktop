...
test('Email Autopilot handles raw JSON and keeps accounts isolated', () => {
  ...
  assert.match(feed, /const senderAddresses = emailAddressesInHeader\(email\.sender\)/)
  assert.match(feed, /senderAddresses\.includes\(mailbox\)/)
  assert.doesNotMatch(feed, /sender\.includes\(mailbox\)/)
...
test('agent email and calendar capability requests use bounded native data', () => {
  ...
  assert.match(browser, /fn native_workspace_capability_reply\(request: &str\)/)
  assert.match(browser, /Email::get_recent_emails\(12\)/)
  assert.match(browser, /CalendarEvent::find_by_timestamp_range/)
  assert.match(browser, /native_workspace_capability_reply\(user_text\)/)
