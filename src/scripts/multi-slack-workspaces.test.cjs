...
test('Slack configuration uses named accounts and preserves sibling workspaces', () => {
  ...
  assert.match(channels, /default_configured && !has_named_default/)
  assert.match(channels, /merged_slack_account_config/)
  assert.match(channels, /environment_default && !named_account_exists/)
...
test('Slack UI can add and remove workspaces independently', () => {
  ...
  assert.match(chat, /hasChannelSavedConfig\(channelStatus\.genericChannels\.slack\) \|\| slackAccounts\.length > 0/)
