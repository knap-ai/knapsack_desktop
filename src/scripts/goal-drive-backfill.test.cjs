...
test('legacy Drive documents receive one account-scoped goal-index backfill', () => {
  ...
  assert.match(model, /pub fn find_by_drive_id_for_account\(/)
  assert.match(model, /pub fn claim_unscoped_drive_id_for_account\(/)
  assert.match(model, /TRIM\(account_email\) = ''/)
  assert.match(model, /\[Goal-related section\]/)
  assert.match(model, /WHERE drive_id = \?1 AND account_email = \?2/)
  assert.match(model, /WHERE drive_id = \?9 AND account_email = \?8/)
  ...
  assert.match(drive, /find_by_drive_id_for_account\(&drive_id, account_email\)/)
  assert.match(drive, /claim_unscoped_drive_id_for_account\(&drive_id, account_email\)/)
  assert.match(drive, /and trashed = false/)
  ...
  assert.match(email, /candidate_conditions/)
  assert.match(email, /LOWER\(subject\) LIKE \? OR LOWER\(body\) LIKE \?/)
  assert.match(email, /params_from_iter\(candidate_terms\.iter\(\)\)/)
  assert.match(email, /FROM emails WHERE COALESCE\(is_deleted, 0\) = 0 AND \(\{candidate_conditions\}\) ORDER BY date DESC LIMIT 500/)
