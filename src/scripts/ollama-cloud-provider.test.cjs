...
test('Ollama Cloud uses the hosted endpoint and an authenticated provider configuration', () => {
  ...
  assert.match(service, /if !cloud \{[\s\S]*?if let Some\(url\) = &payload\.base_url/)
  assert.doesNotMatch(service, /if cloud \{\s*tokens\.ollama_base_url = Some\(OLLAMA_CLOUD_BASE_URL/)
...
test('Cloud chats retain hosted capabilities and use Ollama native tool calling', () => {
  ...
  assert.match(agent, /Uuid::new_v4\(\)/)
  assert.match(agent, /\['\.', '\/', '_', '-'\]/)
...
test('self-hosted LAN endpoints retain local behavior when Cloud is toggled', () => {
  const service = read('src-tauri/src/clawd/service.rs')
  const agent = read('src-tauri/src/clawd/chat_agent.rs')
  assert.match(service, /Cloud's endpoint is selected by `ollama_runtime_base_url`; do not replace/)
  assert.match(agent, /fn is_local_ollama_endpoint\(base_url: &str\) -> bool/)
  assert.match(agent, /host\.ends_with\("\.local"\)/)
