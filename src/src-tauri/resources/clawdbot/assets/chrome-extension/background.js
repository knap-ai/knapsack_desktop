...
/** @type {{ tabId: number, startUrl: string, actions: Array, startTime: number } | null} */
let recorderSession = null

/**
 * Start recording on a specific tab. Injects the content script and begins
 * capturing navigation events via webNavigation.
 */
async function startRecording(tabId) {
  ...
  // Inject the recorder content script
  await chrome.scripting.executeScript({
    target: { tabId },
    files: ['recorder-content.js'],
  })

  // Track navigations on this tab
  chrome.webNavigation.onCommitted.addListener(onRecorderNavigation, {
    url: [{ schemes: ['http', 'https'] }],
  })

  setBadge(tabId, { text: 'REC', color: '#FF0000' })
  console.log(`[Recorder] Started recording on tab ${tabId}: ${recorderSession.startUrl}`)
}

function onRecorderNavigation(details) {
  if (!recorderSession) return
  if (details.tabId !== recorderSession.tabId) return
  if (details.frameId !== 0) return // Main frame only

  // Record navigation events (user clicking links, submitting forms)
  recorderSession.actions.push({
    actionType: 'navigate',
    timestampMs: Date.now() - recorderSession.startTime,
    url: details.url,
    selector: null,
    value: null,
  })
...
async function stopRecording() {
  if (!recorderSession) return null

  const result = {
    startUrl: recorderSession.startUrl,
    actions: recorderSession.actions,
    durationMs: Date.now() - recorderSession.startTime,
  }

  // Clean up content script indicator
  try {
    await chrome.tabs.sendMessage(recorderSession.tabId, {
      type: 'OPENCLAW_RECORDER_CLEANUP',
    })
  } catch {
    // Tab may have been closed
  }

  chrome.webNavigation.onCommitted.removeListener(onRecorderNavigation)
  setBadge(recorderSession.tabId, tabs.has(recorderSession.tabId) ? 'on' : 'off')

  const session = recorderSession
  recorderSession = null

  console.log(`[Recorder] Stopped. Captured ${result.actions.length} actions.`)
  return result
}

// Handle messages from recorder content script and from Knapsack frontend
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  // Recorded actions from content script
  if (msg?.type === 'OPENCLAW_RECORDER_ACTION' && recorderSession) {
    if (sender.tab?.id === recorderSession.tabId) {
      recorderSession.actions.push(msg.action)
    }
    return false
  }

  // Stop recording request from content script indicator click
  if (msg?.type === 'OPENCLAW_RECORDER_STOP') {
    stopRecording().then((result) => sendResponse(result))
    return true
  }

  // External control: start recording
  if (msg?.type === 'OPENCLAW_RECORDER_START') {
  ...
  // External control: stop recording and get results
  if (msg?.type === 'OPENCLAW_RECORDER_STOP_AND_GET') {
    stopRecording().then((result) => sendResponse(result))
    return true
  }

  // External control: get recording status
  if (msg?.type === 'OPENCLAW_RECORDER_STATUS') {
    sendResponse({
      recording: !!recorderSession,
      tabId: recorderSession?.tabId || null,
      actionCount: recorderSession?.actions.length || 0,
      startUrl: recorderSession?.startUrl || null,
    })
    return false
  }
...
// Also handle messages from the Knapsack backend via the relay's external messaging
