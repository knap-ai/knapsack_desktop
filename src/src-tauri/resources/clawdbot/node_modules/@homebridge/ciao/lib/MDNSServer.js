        ...
        if (networkUpdate.changes) {
            for (const change of networkUpdate.changes) {
                ...
                if (!change.outdatedIpv4 && change.updatedIpv4) {
                    // Can happen during DHCP assignment on a persistent interface;
                    // treat like an ADD: create and bind a new socket.
                    debug("IPv4 address appeared on interface %s (%s), binding socket.", change.name, change.updatedIpv4);
                    this.advertiseFamilies.forEach((family) => {
                        if (family !== "IPv6" /* IPFamily.IPv6 */) {
                            const newSocket = this.createDgramSocket(change.name, true, "udp4");
                            this.bindSocket(newSocket, change, "IPv4" /* IPFamily.IPv4 */).catch(reason => {
                                console.log("Could not bind re-appeared network interface: " + reason.stack);
                            });
                        }
                    });
                }
                else if (change.outdatedIpv4 && !change.updatedIpv4) {
                    // Can happen during DHCP release / network failover on a persistent interface;
                    // treat like a REMOVE: close and drop the existing socket.
                    debug("IPv4 address lost on interface %s (%s), closing socket.", change.name, change.outdatedIpv4);
                    if (socket) {
                        this.sockets.delete(change.name);
                        socket.close();
                    }
                }
