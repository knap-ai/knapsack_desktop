...
import { m as statRegularFile, p as isFileMissingError } from "./internal-CcZnGEyS.js";
import "./memory-core-host-engine-foundation-BMMnV9vi.js";
import { a as deriveQmdScopeChatType, c as buildSessionEntry, i as deriveQmdScopeChannel, l as listSessionFilesForAgent, n as resolveCliSpawnInvocation, o as isQmdScopeAllowed, r as runCliCommand, s as parseQmdQueryJson } from "./memory-core-host-engine-qmd-CSAcp0sq.js";
import { t as requireNodeSqlite } from "./memory-core-host-engine-storage-DdyEWu0Y.js";
...
var QmdMemoryManager = class QmdMemoryManager {
	...
	async addCollection(pathArg, name, pattern) {
		const candidateFlags = this.collectionPatternFlag === "--mask" ? ["--mask", "--glob"] : ["--glob", "--mask"];
		let lastError;
	...
	ensureWatcher() {
		if (!this.syncSettings?.sync.watch || this.watcher || this.closed) return;
	...
	scheduleWatchSync() {
		if (!this.syncSettings?.sync.watch) return;
	...
	async maybeWarmSession(sessionKey) {
		if (!this.syncSettings?.sync.onSessionStart) return;
	...
	async maybeSyncDirtySearchState() {
		if (!this.syncSettings?.sync.onSearch || !this.dirty) return;
	...
	*/
	...
	isQueryToolNotFoundError(err) {
		const detail = (err instanceof Error ? err.message : String(err)).match(/ failed \(code \d+\): ([\s\S]*)$/)?.[1];
	...
	normalizeQmdLookupSegment(segment) {
		...
		const normalizePart = (value) => value.normalize("NFKD").toLowerCase().replace(/[^\p{Letter}\p{Number}]+/gu, "-").replace(/-{2,}/g, "-").replace(/^-+|-+$/g, "");
		const normalizedName = normalizePart(parsed.name);
		const normalizedExt = parsed.ext.normalize("NFKD").toLowerCase().replace(/[^\p{Letter}\p{Number}.]+/gu, "");
