...
import { s as planManifestModelCatalogRows } from "./manifest-DkU_xlZi.js";
import { r as normalizeProviderId } from "./provider-id-DRW5WMbW.js";
import { d as isInstalledPluginEnabled } from "./installed-plugin-index-store-Bg0YPG9v.js";
...
import { _ as normalizeProviderModelIdWithPlugin } from "./provider-runtime-CwkNkui5.js";
import { t as resolveOpenClawAgentDir } from "./agent-paths-Cvhc4iGK.js";
import { b as parseModelRef, h as resolveModelRefFromString, i as buildModelAliasIndex, v as modelKey, y as normalizeModelRef } from "./model-selection-shared-VQV3de71.js";
import "./model-selection-GlsOqTDm.js";
import { t as resolvePluginWebSearchConfig } from "./plugin-web-search-config-D1EjmS_W.js";
...
function canonicalizeOpenRouterLookupId(id) {
	...
	return modelKey(provider, normalizeProviderModelIdWithPlugin({
		provider,
		context: {
			provider,
			modelId: model
...
function findConfiguredProviderModel(config, ref) {
...
function getConfiguredModelPricing(config, ref) {
	return toCachedModelPricing(findConfiguredProviderModel(config, ref)?.cost);
}
function hasPrivateOrLoopbackConfiguredEndpoint(config, ref) {
	const providerConfig = config.models?.providers?.[ref.provider];
	return isPrivateOrLoopbackBaseUrl(findConfiguredProviderModel(config, ref)?.baseUrl) || isPrivateOrLoopbackBaseUrl(providerConfig?.baseUrl);
}
function shouldFetchExternalPricingForRef(params) {
	...
	if (hasPrivateOrLoopbackConfiguredEndpoint(params.config, params.ref)) return false;
