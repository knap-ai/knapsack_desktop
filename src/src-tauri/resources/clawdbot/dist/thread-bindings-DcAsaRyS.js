...
import { g as formatThreadBindingDurationLabel, o as resolveThreadBindingEffectiveExpiresAt } from "./thread-bindings-policy-ypJ47uk9.js";
...
import { c as createForumTopicTelegram } from "./sticker-cache-Cf0p9p0n.js";
	...
	const sessionBindingAdapter = {
		...
		bind: async (input) => {
			...
			if (placement === "child") {
				...
				const threadName = (typeof metadata.threadName === "string" ? metadata.threadName.trim() : "") || (typeof metadata.label === "string" ? metadata.label.trim() : "") || `Agent: ${targetSessionKey.split(":").pop()}`;
				try {
					...
					logVerbose(`telegram: child thread-binding failed for ${chatId}: ${err instanceof Error ? err.message : String(err)}`);
					return null;
				}
			} else conversationId = normalizeConversationId(input.conversation.conversationId);
			if (!conversationId) return null;
