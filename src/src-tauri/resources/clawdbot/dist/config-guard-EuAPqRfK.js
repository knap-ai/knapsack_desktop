import { F as shouldMigrateStateFromPath } from "./logger-BCzP_yik.js";
...
async function ensureConfigReady(params) {
	...
	if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-CoXcmIK1.js")).runDoctorConfigPreflight({
			migrateState: false,
	...
	const allowInvalid = commandName ? params.allowInvalid === true || ALLOWED_INVALID_COMMANDS.has(commandName) || commandName === "gateway" && subcommandName && ALLOWED_INVALID_GATEWAY_SUBCOMMANDS.has(subcommandName) : false;
	const { formatConfigIssueLines } = await import("./issue-format-DR4EdcEp.js");
	...
	const [{ colorize, isRich, theme }, { shortenHomePath }, { formatCliCommand }] = await Promise.all([
		import("./theme-X3GQ_MnA.js"),
		import("./utils-BdVZXX6-.js"),
		import("./command-format-2DnQ1RCe.js")
	]);
	const rich = isRich();
