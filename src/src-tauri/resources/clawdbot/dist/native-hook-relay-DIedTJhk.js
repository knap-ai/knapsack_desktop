...
import { i as runBeforeToolCallHook, n as consumeAdjustedParamsForToolCall } from "./pi-tools.before-tool-call-Tbtq2-gG.js";
...
async function runNativeHookRelayPermissionRequest(params) {
	...
	try {
		const decision = await (pendingApproval ?? requestNativeHookRelayPermissionApprovalWithBudget({
			registration: params.registration,
			approvalKey,
			request
		}));
...
async function requestNativeHookRelayPermissionApprovalWithBudget(params) {
	if (!consumeNativeHookRelayPermissionBudget(params.registration.relayId)) {
		log.warn(`native hook permission approval rate limit exceeded; deferring: relay=${params.registration.relayId} run=${params.registration.runId}`);
		return "defer";
	}
...
function readNativeHookRelayEvent(value) {
	if (value === "pre_tool_use" || value === "post_tool_use" || value === "permission_request") return value;
...
function isJsonValue(value) {
	...
	while (stack.length) {
		...
		if (current.value === null || typeof current.value === "string") continue;
		...
		try {
			for (const item of Object.values(current.value)) stack.push({
				value: item,
				depth: current.depth + 1
			});
		} catch {
...
//#endregion
export { runAgentHarnessAfterToolCallHook as a, renderNativeHookRelayUnavailableResponse as i, invokeNativeHookRelay as n, runAgentHarnessBeforeMessageWriteHook as o, registerNativeHookRelay as r, buildNativeHookRelayCommand as t };
