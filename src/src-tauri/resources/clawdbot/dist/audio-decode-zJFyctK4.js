...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-DZMGBpvs.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-CdR-HkzY.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-CDX0bu_C.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-BSfa0bdP.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-SEX6adqF.js")).decode;
		return buf && createBuffer(await decode(buf));
	}
};
