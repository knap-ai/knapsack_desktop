import { t as getActivePluginRegistry } from "./runtime-Iz8uZ7EU.js";
...
function buildChatCommands() {
	const commands = [
		...
		...listChannelPlugins().filter((plugin) => plugin.capabilities.nativeCommands).map((plugin) => defineDockCommand(plugin))
	];
...
function getNativeCommandSurfaces() {
	...
	cachedNativeCommandSurfaces = new Set(listChannelPlugins().filter((plugin) => plugin.capabilities.nativeCommands).map((plugin) => plugin.id));
