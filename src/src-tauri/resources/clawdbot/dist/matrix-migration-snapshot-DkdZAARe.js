...
import { g as shouldPreferNativeJiti } from "./chat-meta-Cdrnv7R-.js";
...
import { a as resolveMatrixAccountStorageRoot, c as resolveMatrixDefaultOrOnlyAccountId, i as resolveConfiguredMatrixAccountIds, l as resolveMatrixLegacyFlatStoragePaths, n as getMatrixScopedEnvVarNames, o as resolveMatrixChannelConfig, r as requiresExplicitMatrixDefaultAccount, s as resolveMatrixCredentialsPath, t as findMatrixAccountEntry } from "./matrix-config-helpers-B5EXOHUL.js";
import { t as createBackupArchive } from "./backup-create-B_57aHT_.js";
...
function loadStoredMatrixCredentials(env, accountId) {
	...
	try {
		if (!fsSync.existsSync(credentialsPath)) return null;
		const parsed = JSON.parse(fsSync.readFileSync(credentialsPath, "utf8"));
...
function detectLegacyBotSdkCryptoStore(cryptoRootDir) {
	try {
		if (!fsSync.statSync(cryptoRootDir).isDirectory()) return {
			detected: false,
	...
	try {
		return { detected: fsSync.existsSync(path.join(cryptoRootDir, "bot-sdk.json")) || fsSync.existsSync(path.join(cryptoRootDir, "matrix-sdk-crypto.sqlite3")) || fsSync.readdirSync(cryptoRootDir, { withFileTypes: true }).some((entry) => entry.isDirectory() && fsSync.existsSync(path.join(cryptoRootDir, entry.name, "matrix-sdk-crypto.sqlite3"))) };
	} catch (err) {
		return {
			detected: false,
...
function resolveLegacyMatrixFlatStorePlan(params) {
	...
	if (!fsSync.existsSync(legacy.cryptoPath)) return null;
...
function loadLegacyBotSdkMetadata(cryptoRootDir) {
	...
	try {
		if (!fsSync.existsSync(metadataPath)) return fallback;
		const parsed = JSON.parse(fsSync.readFileSync(metadataPath, "utf8"));
...
function resolveMatrixLegacyCryptoPlans(params) {
	...
	for (const accountId of resolveMatrixAccountIds(params.cfg)) {
		...
		if (!fsSync.existsSync(legacyCryptoPath)) continue;
...
function resolveMatrixMigrationPlan(params) {
	...
	if (!fsSync.existsSync(legacy.storagePath) && !fsSync.existsSync(legacy.cryptoPath)) return null;
	const target = resolveLegacyMatrixFlatStoreTarget({
		cfg: params.cfg,
		env: params.env,
...
function moveLegacyPath(params) {
	...
	if (fsSync.existsSync(params.targetPath)) {
...
async function maybeCreateMatrixMigrationSnapshot(params) {
	...
	if (existingMarker?.archivePath && fsSync.existsSync(existingMarker.archivePath)) {
	...
	if (existingMarker?.archivePath && !fsSync.existsSync(existingMarker.archivePath)) params.log?.warn?.(`matrix: previous migration snapshot is missing (${existingMarker.archivePath}); creating a replacement backup before continuing`);
