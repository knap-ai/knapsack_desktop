...
import { s as quoteCmdScriptArg, t as renderCmdRestartLogSetup } from "./restart-logs-BdpufG4l.js";
import { a as resolveTaskScriptPath } from "./schtasks-Dd2ePtU6.js";
...
async function emitPreparedGatewayRestart(hooks) {
	...
	if (!emitGatewayRestart()) await preparedHooks?.afterEmitRejected?.().catch(() => void 0);
}
/**
...
function deferGatewayRestartUntilIdle(opts) {
	...
	try {
		...
		emitPreparedGatewayRestart(opts.emitHooks);
		return;
	}
	if (pending <= 0) {
		opts.hooks?.onReady?.();
