...
import { a as loadConfig, d as readConfigFileSnapshotForWrite } from "./io-Dv_xNAZB.js";
import { c as normalizeAgentId, u as resolveAgentIdFromSessionKey } from "./session-key-EpIbK3Oz.js";
...
import { b as resolveAgentWorkspaceDir, g as listAgentIds, l as resolveAgentSkillsFilter, p as resolveSessionAgentId, u as resolveEffectiveModelFallbacks, y as resolveAgentDir } from "./agent-scope-_6dFncNS.js";
import { f as resolveMessageChannel } from "./message-channel-C2Lnao8s.js";
...
import { b as normalizeModelRef, f as resolveConfiguredModelRef, x as parseModelRef, y as modelKey } from "./model-selection-shared-grYiFZof.js";
...
import { o as normalizeAccountId } from "./delivery-context.shared-BRwSoIeK.js";
import { i as emitAgentEvent, t as clearAgentRunContext, u as registerAgentRunContext } from "./agent-events-DMk-SEBA.js";
import { i as LiveSessionModelSwitchError, r as runWithModelFallback } from "./model-fallback-CHc4qFzF.js";
import { t as getAgentRuntimeCommandSecretTargetIds } from "./command-secret-targets-5rfgNL2Q.js";
...
import { n as normalizeSpawnedRunMetadata } from "./spawned-context-7b-pqA3M.js";
import { r as resolveSession } from "./live-model-switch-C-99YSoH.js";
import { n as resolveSendPolicy } from "./send-policy-CeDRAbMh.js";
import { t as clearSessionAuthProfileOverride } from "./session-override-Cu-0dtRK.js";
import { n as applyVerboseOverride } from "./level-overrides-xpqKLTb9.js";
import { n as prependInternalEventContext, t as persistSessionEntry$1 } from "./attempt-execution.shared-CT4MWoqD.js";
...
async function resolveAgentRuntimeConfig(runtime, params) {
	...
	}) ? (await (await import("./command-config-resolution.runtime-0V4M4IAB.js")).resolveCommandConfigWithSecrets({
		config: loadedRaw,
		commandName: "agent",
...
function loadAttemptExecutionRuntime() {
	attemptExecutionRuntimePromise ??= import("./attempt-execution.runtime-kxgP7fhl.js");
	return attemptExecutionRuntimePromise;
}
function loadAcpManagerRuntime() {
	acpManagerRuntimePromise ??= import("./manager-DldW_kFR.js");
	return acpManagerRuntimePromise;
}
function loadAcpPolicyRuntime() {
	acpPolicyRuntimePromise ??= import("./policy-C6huxPRa.js");
	return acpPolicyRuntimePromise;
}
function loadAcpRuntimeErrorsRuntime() {
	acpRuntimeErrorsRuntimePromise ??= import("./errors-BdpzXpms.js");
	return acpRuntimeErrorsRuntimePromise;
}
function loadAcpSessionIdentifiersRuntime() {
	acpSessionIdentifiersRuntimePromise ??= import("./session-identifiers-CrRWq42M.js");
	return acpSessionIdentifiersRuntimePromise;
}
function loadDeliveryRuntime() {
	deliveryRuntimePromise ??= import("./delivery.runtime-DRKTtaY-.js");
	return deliveryRuntimePromise;
}
function loadSessionStoreRuntime() {
	sessionStoreRuntimePromise ??= import("./session-store.runtime-3K4EtqJT.js");
	return sessionStoreRuntimePromise;
}
function loadTranscriptResolveRuntime() {
	transcriptResolveRuntimePromise ??= import("./transcript-resolve.runtime-DdmXyKAZ.js");
	return transcriptResolveRuntimePromise;
}
function loadCliDepsRuntime() {
	cliDepsRuntimePromise ??= import("./deps-bVdlIvND.js");
	return cliDepsRuntimePromise;
}
function loadExecDefaultsRuntime() {
	execDefaultsRuntimePromise ??= import("./exec-defaults-DOtFSnHO.js");
	return execDefaultsRuntimePromise;
}
function loadSkillsRuntime() {
	skillsRuntimePromise ??= import("./skills-CYOU4kJz.js");
	return skillsRuntimePromise;
}
function loadSkillsFilterRuntime() {
	skillsFilterRuntimePromise ??= import("./filter-CywHQa3H.js");
	return skillsFilterRuntimePromise;
}
function loadSkillsRefreshStateRuntime() {
	skillsRefreshStateRuntimePromise ??= import("./refresh-state-DeBLMBnd.js");
	return skillsRefreshStateRuntimePromise;
}
function loadSkillsRemoteRuntime() {
	skillsRemoteRuntimePromise ??= import("./skills-remote-DEEqQYQf.js");
...
async function agentCommandInternal(opts, runtime = defaultRuntime, deps) {
	...
	const { body, transcriptBody, cfg, normalizedSpawned, agentCfg, thinkOverride, thinkOnce, verboseOverride, timeoutMs, sessionId, sessionKey, sessionStore, storePath, isNewSession, persistedThinking, persistedVerbose, sessionAgentId, outboundSession, workspaceDir, agentDir, runId, acpManager, acpResolution } = prepared;
	...
	try {
		...
		if (!isThinkingLevelSupported({
			...
			if (Boolean(thinkOnce || thinkOverride)) throw new Error(`Thinking level "${resolvedThinkLevel}" is not supported for ${provider}/${model}. Use one of: ${formatThinkingLevels(provider, model)}.`);
