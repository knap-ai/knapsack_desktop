...
import { t as CONFIG_DIR, v as resolveUserPath, x as shortenHomePath } from "./utils-CE3P21nG.js";
import { r as theme } from "./theme-D-TumEpz.js";
import { c as loadConfig, f as readConfigFileSnapshot } from "./io-DhtVmzAJ.js";
import { n as MANIFEST_KEY } from "./legacy-names-JQT-hzFf.js";
...
import { r as parseRegistryNpmSpec } from "./npm-registry-spec-ChTidTqx.js";
import { a as resolveArchiveKind } from "./archive-CQLVFZEE.js";
import { t as findBundledPluginSource } from "./bundled-sources-B9i5z3_o.js";
import { t as parseFrontmatter } from "./frontmatter-DA7tCmRr.js";
import { a as resolvePinnedNpmInstallRecordForCli, n as persistPluginInstall, r as recordHookInstall, t as persistHookPackInstall } from "./plugins-install-persist-BkuhKz4Q.js";
import { s as parseClawHubPluginSpec } from "./clawhub-y4k4A2d9.js";
import { o as unscopedPackageName, r as resolveSafeInstallDir } from "./install-safe-path-YYLCtZgM.js";
import { i as installPluginFromPath, r as installPluginFromNpmSpec } from "./install-V93VXjQN.js";
import { n as formatClawHubSpecifier, r as installPluginFromClawHub } from "./clawhub-D4amINPA.js";
import { a as decidePreferredClawHubFallback, c as extractInstalledNpmPackageName, i as createPluginInstallLogger, l as formatPluginInstallWithHookFallbackError, n as buildPreferredClawHubSpec, r as createHookPackInstallLogger, s as extractInstalledNpmHookPackageName } from "./plugins-command-helpers-XbI77OBE.js";
import { r as resolveBundledInstallPlanForNpmFailure, t as resolveBundledInstallPlanBeforeNpm } from "./plugin-install-plan-CMVAuqHP.js";
import { t as cleanStaleMatrixPluginConfig } from "./matrix-BI7ZZ_5T.js";
import { a as installPluginFromMarketplace, i as readInstalledPackageVersion, n as updateNpmInstalledPlugins, r as expectedIntegrityForUpdate, s as resolveMarketplaceInstallShortcut } from "./update-COK0Zm-6.js";
import { r as resolvePluginInstallRequestContext, t as resolvePluginInstallInvalidConfigPolicy } from "./plugin-install-config-policy-C_04haW6.js";
...
async function loadHookInstallRuntime() {
	hookInstallRuntimePromise ??= import("./install.runtime-Bwb4ha4R.js");
...
async function tryInstallHookPackFromLocalPath(params) {
	if (params.link) {
		if (!fsSync.statSync(params.resolvedPath).isDirectory()) return {
			ok: false,
...
function isAllowedMatrixRecoveryIssue(issue) {
	return issue.path === "channels.matrix" && issue.message === "unknown channel id: matrix" || issue.path === "plugins.load.paths" && typeof issue.message === "string" && issue.message.includes("plugin path not found");
}
...
async function loadConfigFromSnapshotForInstall(request) {
	if (resolvePluginInstallInvalidConfigPolicy(request) !== "recover-matrix-only") throw buildInvalidPluginInstallConfigError("Config invalid; run `openclaw doctor --fix` before installing plugins.");
	...
	if (snapshot.legacyIssues.length > 0 || snapshot.issues.length === 0 || snapshot.issues.some((issue) => !isAllowedMatrixRecoveryIssue(issue))) throw buildInvalidPluginInstallConfigError("Config invalid outside the Matrix upgrade recovery path; run `openclaw doctor --fix` before reinstalling Matrix.");
...
async function loadConfigForInstall(request) {
	try {
		...
		if (extractErrorCode(err) !== "INVALID_CONFIG") throw err;
	}
...
async function runPluginInstallCommand(params) {
	...
	const cfg = await loadConfigForInstall(request).catch((error) => {
		defaultRuntime.error(formatErrorMessage(error));
		return null;
	});
	if (!cfg) return defaultRuntime.exit(1);
	if (opts.marketplace) {
...
function resolvePluginUpdateSelection(params) {
	...
	if (!parsedSpec || parsedSpec.selectorKind === "none") return { pluginIds: [params.rawId] };
...
//#endregion
export { runPluginInstallCommand as n, runPluginUpdateCommand as t };
