...
import { b as DEFAULT_USER_FILENAME, d as DEFAULT_AGENTS_FILENAME, f as DEFAULT_AGENT_WORKSPACE_DIR, h as DEFAULT_IDENTITY_FILENAME, l as resolveSessionAgentId, m as DEFAULT_HEARTBEAT_FILENAME, n as resolveAgentConfig, p as DEFAULT_BOOTSTRAP_FILENAME, v as DEFAULT_SOUL_FILENAME, x as ensureAgentWorkspace, y as DEFAULT_TOOLS_FILENAME } from "./agent-scope-DMGrQp5u.js";
import { t as formatCliCommand } from "./command-format-ChfKqObn.js";
import { i as loadConfig } from "./config-CWifpzL_.js";
import { A as resolveAgentMainSessionKey, k as canonicalizeMainSessionAlias } from "./sessions-ezP1qtWM.js";
import { C as DEFAULT_BROWSER_EVALUATE_ENABLED, E as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, w as DEFAULT_OPENCLAW_BROWSER_COLOR } from "./chrome-CGPBw-bD.js";
import { d as resolveSandboxPath, o as syncSkillsToWorkspace } from "./skills-Dj7GqTPz.js";
import { t as registerBrowserRoutes } from "./routes-CyIJmYmu.js";
import { o as resolveProfile, t as createBrowserRouteContext } from "./server-context-DGqqHDqz.js";
...
async function readRegistry() {
	try {
		const raw = await fsPromises.readFile(SANDBOX_REGISTRY_PATH, "utf-8");
...
async function writeRegistry(registry) {
	...
	await fsPromises.writeFile(SANDBOX_REGISTRY_PATH, `${JSON.stringify(registry, null, 2)}\n`, "utf-8");
}
...
async function readBrowserRegistry() {
	try {
		const raw = await fsPromises.readFile(SANDBOX_BROWSER_REGISTRY_PATH, "utf-8");
		const parsed = JSON.parse(raw);
		if (parsed && Array.isArray(parsed.entries)) return parsed;
	} catch {}
	return { entries: [] };
...
async function writeBrowserRegistry(registry) {
	await fsPromises.mkdir(SANDBOX_STATE_DIR, { recursive: true });
	await fsPromises.writeFile(SANDBOX_BROWSER_REGISTRY_PATH, `${JSON.stringify(registry, null, 2)}\n`, "utf-8");
}
