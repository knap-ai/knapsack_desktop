...
import { A as resolveCitationRedirectUrl, C as resolveSearchCacheTtlMs, O as withTrustedWebSearchEndpoint, S as readProviderEnvValue, T as resolveSearchTimeoutSeconds, a as mergeScopedSearchConfig, b as readCachedSearchPayload, c as setScopedCredentialValue, k as writeCachedSearchPayload, m as buildUnsupportedSearchFilterResponse, o as resolveProviderWebSearchPluginConfig, p as buildSearchCacheKey, r as getScopedCredentialValue, s as setProviderWebSearchPluginConfigValue, w as resolveSearchCount, x as readConfiguredSecretString } from "./provider-web-search-BvBfxXst.js";
import { c as wrapWebContent } from "./external-content-CoIROwJI.js";
import { t as DEFAULT_GOOGLE_API_BASE_URL } from "./api-tF_YSNoD.js";
...
function resolveGeminiConfig(searchConfig) {
	...
	return gemini && typeof gemini === "object" && !Array.isArray(gemini) ? gemini : {};
}
...
function resolveGeminiModel(gemini) {
	return (typeof gemini?.model === "string" ? gemini.model.trim() : "") || DEFAULT_GEMINI_MODEL;
}
async function runGeminiSearch(params) {
	return withTrustedWebSearchEndpoint({
		...
		if (!res.ok) {
			const safeDetail = (await res.text() || res.statusText).replace(/key=[^&\s]+/gi, "key=***");
		...
		if (data.error) {
			...
			throw new Error(`Gemini API error (${data.error.code}): ${rawMessage.replace(/key=[^&\s]+/gi, "key=***")}`);
		}
...
function createGeminiWebSearchProvider() {
	return {
		...
		getConfiguredCredentialValue: (config) => resolveProviderWebSearchPluginConfig(config, "google")?.apiKey,
