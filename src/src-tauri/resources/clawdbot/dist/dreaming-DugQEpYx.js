...
import { B as resolveMemoryLightDreamingConfig, F as resolveMemoryDeepDreamingConfig, M as formatMemoryDreamingDay, P as resolveMemoryCorePluginConfig, V as resolveMemoryRemDreamingConfig, z as resolveMemoryDreamingWorkspaces } from "./dreaming-K8xreO0H.js";
import { o as parseUsageCountedSessionIdFromFileName } from "./artifacts-BKUYGR-f.js";
import { s as peekSystemEventEntries } from "./system-events-B6vO-QxY.js";
import "./text-runtime-B1c54bxG.js";
import "./infra-runtime-Bqxrc1fI.js";
import { n as appendMemoryHostEvent } from "./events-D_R_jQEz.js";
import "./memory-core-host-status-BfwKpmnZ.js";
import { c as buildSessionEntry, d as loadSessionTranscriptClassificationForAgent, f as normalizeSessionTranscriptPathForComparison, l as listSessionFilesForAgent, p as sessionPathForFile } from "./engine-qmd-BblS7k9c.js";
import "./memory-core-host-engine-qmd-CvttOYTZ.js";
import "./memory-host-events-CYayY0QO.js";
import { n as withTrailingNewline, t as replaceManagedMarkdownBlock } from "./memory-host-markdown-DLqyX2bD.js";
import { a as readShortTermRecallEntries, c as recordShortTermRecalls, i as rankShortTermPromotionCandidates, n as applyShortTermPromotions, o as recordDreamingPhaseSignals, u as repairShortTermPromotionArtifacts } from "./short-term-promotion-CI1S22E7.js";
import { n as normalizeTrimmedString, t as includesSystemEventToken } from "./dreaming-shared-CkfGe3lS.js";
import { r as generateAndAppendDreamNarrative, t as buildNarrativeSessionKey } from "./dreaming-narrative-BKXbhYcL.js";
...
async function runLightDreaming(params) {
	...
	const entries = dedupeEntries((await readShortTermRecallEntries({
		workspaceDir: params.workspaceDir,
		nowMs
	})).filter((entry) => entryWithinLookback(entry, cutoffMs)).toSorted((a, b) => {
...
async function deleteNarrativeSessionBestEffort(subagent, sessionKey) {
	try {
...
function isLegacyPhaseDreamingJob(job) {
	...
	if (description?.includes(LEGACY_LIGHT_SLEEP_CRON_TAG) || description?.includes(LEGACY_REM_SLEEP_CRON_TAG)) return true;
	const name = normalizeTrimmedString(job.name);
	const payloadText = normalizeTrimmedString(job.payload?.text);
	if (name === LEGACY_LIGHT_SLEEP_CRON_NAME && payloadText === LEGACY_LIGHT_SLEEP_EVENT_TEXT) return true;
...
function hasPendingManagedDreamingCronEvent(sessionKey) {
	return resolveDreamingTriggerSessionKeys(sessionKey).some((candidateSessionKey) => peekSystemEventEntries(candidateSessionKey).some((event) => event.contextKey?.startsWith("cron:") === true && normalizeTrimmedString(event.text) === DREAMING_SYSTEM_EVENT_TEXT));
}
...
async function runShortTermDreamingPromotionIfTriggered(params) {
	...
	if (!includesSystemEventToken(params.cleanedBody, DREAMING_SYSTEM_EVENT_TEXT)) return;
	...
	for (const workspaceDir of workspaces) try {
		...
		if (params.subagent && (candidates.length > 0 || applied.applied > 0)) {
			...
			if (detachNarratives) queueMicrotask(() => {
				generateAndAppendDreamNarrative({
					subagent: params.subagent,
					workspaceDir,
					data,
...
function registerShortTermPromotionDreaming(api) {
	...
	const resolveCurrentConfig = () => api.runtime.config?.loadConfig?.() ?? api.config;
	...
	api.on("before_agent_reply", async (event, ctx) => {
		try {
			...
			const hasManagedDreamingToken = includesSystemEventToken(event.cleanedBody, DREAMING_SYSTEM_EVENT_TEXT);
...
//#endregion
export { seedHistoricalDailyMemorySignals as i, resolveShortTermPromotionDreamingConfig as n, previewRemDreaming as r, registerShortTermPromotionDreaming as t };
