...
import { Js as extractPdfContent } from "./pi-embedded-CbCYZxIb.js";
import { T as estimateBase64DecodedBytes, w as canonicalizeBase64 } from "./common-zReYuoMM.js";
import { r as convertHeicToJpeg } from "./image-ops-By4YfyWa.js";
import { t as detectMime } from "./mime-CsUtzcn2.js";
import { n as fetchWithSsrFGuard } from "./fetch-guard-CYl1q2XH.js";
import { u as readResponseWithLimit } from "./web-media-PKy3RbRG.js";
...
function renderFileContextBlock(params) {
	const fallbackName = typeof params.fallbackName === "string" && params.fallbackName.trim().length > 0 ? params.fallbackName.trim() : "attachment";
	const safeName = sanitizeFileName(params.filename, fallbackName);
	const safeContent = escapeFileBlockContent(params.content);
	const attrs = [`name="${xmlEscapeAttr(safeName)}"`, typeof params.mimeType === "string" && params.mimeType.trim() ? `mime="${xmlEscapeAttr(params.mimeType.trim())}"` : void 0].filter(Boolean).join(" ");
