...
import { l as escapeRegExp, t as CONFIG_DIR, v as resolveUserPath } from "./utils-CE3P21nG.js";
...
import { o as openFileWithinRoot, t as SafeOpenError } from "./fs-safe-Dv_jH6UN.js";
...
import { a as ensureGatewayStartupAuth } from "./nodes.helpers-B1rYLucl.js";
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-_2ysvYvZ.js");
	} catch (err) {
...
function resolveCdpReachabilityTimeouts(params) {
	const normalized = normalizeTimeoutMs$1(params.timeoutMs);
...
function createProfileAvailability({ opts, profile, state, getProfileState, setProfileRunning }) {
	...
	const closePlaywrightBrowserConnectionForProfile = async (cdpUrl) => {
		try {
			await (await import("./pw-ai-_2ysvYvZ.js")).closePlaywrightBrowserConnection(cdpUrl ? { cdpUrl } : void 0);
		} catch {}
	};
	...
	const ensureBrowserAvailable = async () => {
		...
		if (capabilities.usesChromeMcp) {
			if (profile.userDataDir && !fsSync.existsSync(profile.userDataDir)) throw new BrowserProfileUnavailableError(`Browser user data directory not found for profile "${profile.name}": ${profile.userDataDir}`);
...
async function stopBrowserRuntime(params) {
	...
	try {
		await (await import("./pw-ai-_2ysvYvZ.js")).closePlaywrightBrowserConnection();
	} catch {}
}
//#endregion
		...
		try {
			...
			if (savedVp) {
				...
				try {
					...
					if (Number(p?.w) !== savedVp.w || Number(p?.h) !== savedVp.h || Number(p?.dpr) !== savedVp.dpr) await send("Emulation.setDeviceMetricsOverride", {
						width: savedVp.w,
						height: savedVp.h,
						deviceScaleFactor: savedVp.dpr,
						mobile: false,
...
function createBrowserProfilesService(ctx) {
	...
	const createProfile = async (params) => {
		...
		if (normalizedUserDataDir && !fsSync.existsSync(normalizedUserDataDir)) throw new BrowserValidationError(`browser user data directory not found: ${normalizedUserDataDir}`);
		if (rawCdpUrl) {
			let parsed;
			try {
...
//#endregion
export { withBrowserNavigationPolicy as $, resolveBrowserConfig as A, DEFAULT_UPLOAD_DIR as B, createBrowserRouteContext as C, readBrowserVersion as D, parseBrowserMajorVersion as E, DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME as F, DEFAULT_FILL_FIELD_TYPE as G, resolveStrictExistingPathsWithinRoot as H, ensureBrowserControlAuth as I, BrowserTabNotFoundError as J, normalizeBrowserFormField as K, resolveBrowserControlAuth as L, DEFAULT_AI_SNAPSHOT_MAX_CHARS as M, DEFAULT_BROWSER_EVALUATE_ENABLED as N, resolveGoogleChromeExecutableForPlatform as O, DEFAULT_OPENCLAW_BROWSER_COLOR as P, assertBrowserNavigationResultAllowed as Q, withNoProxyForCdpUrl as R, stopBrowserRuntime as S, getChromeWebSocketUrl as T, matchBrowserUrlPattern as U, resolveExistingPathsWithinRoot as V, getBrowserProfileCapabilities as W, assertBrowserNavigationAllowed as X, InvalidBrowserNavigationUrlError as Y, assertBrowserNavigationRedirectChainAllowed as Z, getBrowserControlState as _, parseRoleRef as a, isDefaultBrowserPluginEnabled as b, appendCdpPath as c, normalizeCdpHttpBaseForJsonEndpoints as d, deleteBridgeAuthForPort as et, redactCdpUrl as f, createBrowserControlContext as g, createBrowserRouteDispatcher as h, getRoleSnapshotStats as i, resolveProfile as j, markPwAiLoaded as k, fetchJson as l, fetchBrowserJson as m, buildRoleSnapshotFromAiSnapshot as n, formatAriaSnapshot as o, withCdpSocket as p, normalizeBrowserFormFieldValue as q, buildRoleSnapshotFromAriaSnapshot as r, normalizeCdpWsUrl as s, registerBrowserRoutes as t, setBridgeAuthForPort as tt, getHeadersWithAuth as u, startBrowserControlServiceFromConfig as v, movePathToTrash as w, createBrowserRuntimeState as x, stopBrowserControlService as y, DEFAULT_TRACE_DIR as z };
