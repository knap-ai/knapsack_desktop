import { a as hasConfiguredSecretInput, l as normalizeSecretInputString, t as DEFAULT_SECRET_PROVIDER_ALIAS } from "./types.secrets-DTXJhyWe.js";
import { t as enablePluginInConfig } from "./enable-CpDMTfhy.js";
import { a as resolveBundledWebSearchPluginId, i as listBundledWebSearchProviders, n as sortWebSearchProviders } from "./web-search-providers.shared-CiXeJJJg.js";
import { t as resolvePluginWebSearchProviders } from "./web-search-providers.runtime-BGcv6M1I.js";
import { n as sortFlowContributionsByLabel } from "./types-Bg_tuPYV.js";
...
const SEARCH_PROVIDER_OPTIONS = resolveSearchProviderSetupContributions().map((contribution) => contribution.provider);
function showsSearchProviderInSetup(entry) {
	return entry.onboardingScopes?.includes("text-inference") ?? false;
}
function canRepairBundledProviderSelection(config, provider) {
...
function resolveSearchProviderSetupContributions(config) {
	if (!config) return sortFlowContributionsByLabel(sortWebSearchProviders(listBundledWebSearchProviders()).filter(showsSearchProviderInSetup).map((provider) => buildSearchProviderSetupContribution({
		provider,
		source: "bundled"
	})));
	const merged = new Map(resolvePluginWebSearchProviders({
		config,
		bundledAllowlistCompat: true,
		env: process.env
	}).map((provider) => [provider.id, buildSearchProviderSetupContribution({
		provider,
		source: "runtime"
	})]));
	for (const provider of listBundledWebSearchProviders()) {
		if (merged.has(provider.id) || !canRepairBundledProviderSelection(config, provider)) continue;
		merged.set(provider.id, buildSearchProviderSetupContribution({
			provider,
			source: "bundled"
		}));
	}
...
function buildSearchEnvRef(config, provider) {
	const entry = resolveSearchProviderEntry(config, provider) ?? SEARCH_PROVIDER_OPTIONS.find((candidate) => candidate.id === provider) ?? listBundledWebSearchProviders().find((candidate) => candidate.id === provider);
	const envVar = entry?.envVars.find((k) => Boolean(process.env[k]?.trim())) ?? entry?.envVars[0];
	if (!envVar) throw new Error(`No env var mapping for search provider "${provider}" at ${entry?.credentialPath ?? "unknown path"} in secret-input-mode=ref.`);
	return {
		source: "env",
