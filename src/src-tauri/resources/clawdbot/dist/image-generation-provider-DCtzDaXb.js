...
import { l as sanitizeConfiguredModelProviderRequest } from "./provider-request-config-Bxj9Jz-Z.js";
import { r as assertOkOrThrowHttpError } from "./provider-http-errors-CqyrJgS5.js";
import { a as postJsonRequest, o as postMultipartRequest, u as resolveProviderHttpRequestConfig } from "./shared-C_SB36H2.js";
import { t as isProviderApiKeyConfigured } from "./provider-auth-B7ecZcum.js";
...
import { i as OPENAI_DEFAULT_IMAGE_MODEL } from "./default-models-DSXAAO7b.js";
import { a as resolveConfiguredOpenAIBaseUrl } from "./shared-o8Ieu6vJ.js";
...
function resolveOpenAIImageTimeoutMs(timeoutMs) {
...
function appendOpenAIImageOptions(target, req) {
	...
	const entries = {
		...
		...openai?.background !== void 0 ? { background: openai.background } : {},
...
function toOpenAIDataUrl(image) {
	return `data:${image.mimeType?.trim() || DEFAULT_OUTPUT_MIME};base64,${Buffer.from(image.buffer).toString("base64")}`;
}
...
function logCodexImageAuthSelected(params) {
	const model = params.req.model || "gpt-image-2";
	log.info(`image auth selected: provider=openai-codex mode=${sanitizeLogValue(params.authMode)} transport=codex-responses requestedModel=${sanitizeLogValue(model)} responsesModel=gpt-5.4 timeoutMs=${params.timeoutMs}`);
}
async function generateOpenAICodexImage(params) {
	...
	return {
		...
		model,
		metadata: { responses: results.map((result) => result.metadata).filter(Boolean) }
	};
}
	...
	return createOpenAIImageGenerationProviderBase({
		...
		async generateImage(req) {
			...
			try {
				...
				const output = resolveOutputMime(req.outputFormat);
				return {
					images: (data.data ?? []).map((entry, index) => {
