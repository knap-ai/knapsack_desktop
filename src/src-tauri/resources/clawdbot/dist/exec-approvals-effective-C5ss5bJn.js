...
import { D as resolveExecApprovalsFromFile, f as minSecurity, t as DEFAULT_EXEC_APPROVAL_ASK_FALLBACK, u as maxAsk, w as resolveExecApprovalAllowedDecisions } from "./exec-approvals-D6YTmZxM.js";
...
function resolveAskNote(params) {
	if (params.hostAsk === "off" && params.requestedAsk !== "off") return "host ask=off suppresses prompts";
...
function resolveExecPolicyScopeSnapshot(params) {
	...
	const effectiveAsk = resolved.agent.ask === "off" ? "off" : maxAsk(requestedAsk.value, resolved.agent.ask);
	return {
