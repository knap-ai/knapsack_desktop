...
import { c as optimizeImageToPng, i as convertHeicToJpeg, l as resizeToJpeg, o as hasAlphaChannel } from "./image-ops-DjlnMkqM.js";
import { n as extensionForMime, p as maxBytesForKind, s as kindFromMime, t as detectMime } from "./mime-C9472kP8.js";
import { l as readLocalFileSafely, t as SafeOpenError } from "./fs-safe-Dv_jH6UN.js";
...
function isPixelLimitError(error) {
	return error instanceof Error && error.message.includes(`${25e6.toLocaleString("en-US")} pixel input limit`);
}
...
function assertHostReadMediaAllowed(params) {
	...
	if (normalizedMime && HOST_READ_ALLOWED_DOCUMENT_MIMES.has(normalizedMime)) return;
	const ext = path.extname(params.fileName ?? params.mediaUrl).trim().toLowerCase();
	if (ext && HOST_READ_ALLOWED_DOCUMENT_EXTS.has(ext)) return;
	throw new LocalMediaAccessError("path-not-allowed", `Host-local media sends only allow images, audio, video, PDF, and Office documents (got ${normalizedMime ?? "unknown"}).`);
}
...
async function loadWebMediaInternal(mediaUrl, options = {}) {
	const { maxBytes, optimizeImages = true, ssrfPolicy, localRoots, sandboxValidated = false, readFile: readFileOverride, hostReadCapability = false, workspaceDir } = options;
	...
	if (workspaceDir && !path.isAbsolute(mediaUrl)) mediaUrl = path.resolve(workspaceDir, mediaUrl);
	try {
...
async function optimizeImageToJpeg(buffer, maxBytes, opts = {}) {
	...
	for (const side of sides) for (const quality of qualities) try {
		...
		if (isPixelLimitError(error)) throw error;
	}
	if (smallest) return {
...
//#endregion
export { assertLocalMediaAllowed as a, LocalMediaAccessError as i, loadWebMediaRaw as n, getDefaultLocalRoots as o, optimizeImageToJpeg as r, loadWebMedia as t };
