...
import { t as buildManifestChannelMeta } from "./channel-meta-BEaGWV3f.js";
...
function loadOfficialCatalogEntries(options) {
	const builtInEntries = parseCatalogEntries(official_external_channel_catalog_default);
	const fileEntries = loadCatalogEntriesFromPaths(resolveOfficialCatalogPaths(options));
	return [...builtInEntries, ...fileEntries].map((entry) => buildExternalCatalogEntry(entry)).filter((entry) => Boolean(entry));
}
