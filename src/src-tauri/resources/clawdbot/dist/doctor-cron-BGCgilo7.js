...
import { i as TrimmedNonEmptyStringFieldSchema, n as LowercaseNonEmptyStringFieldSchema, o as parseOptionalField, t as DeliveryThreadIdFieldSchema } from "./delivery-field-schemas-WIpFQ1d8.js";
...
import { t as coerceFiniteScheduleNumber } from "./schedule-Cs_MqEXP.js";
...
function isManagedDreamingJob(raw) {
	if (normalizeOptionalString(raw.description)?.includes(MANAGED_DREAMING_CRON_TAG)) return true;
	if (normalizeOptionalString(raw.name) !== MANAGED_DREAMING_CRON_NAME) return false;
	const payload = raw.payload ?? void 0;
	const payloadKind = normalizeOptionalLowercaseString(payload?.kind);
	if (payloadKind === "systemevent") return normalizeOptionalString(payload?.text) === DREAMING_SYSTEM_EVENT_TEXT;
	if (payloadKind === "agentturn") return normalizeOptionalString(payload?.message) === DREAMING_SYSTEM_EVENT_TEXT;
	return false;
}
