...
function createPersistCronSessionEntry(params) {
	return async () => {
		...
		if (params.runSessionKey !== params.agentSessionKey) params.cronSession.store[params.runSessionKey] = runSessionEntry;
		await params.updateSessionStore(params.cronSession.storePath, (store) => {
			...
			if (params.runSessionKey !== params.agentSessionKey) store[params.runSessionKey] = runSessionEntry;
		});
	};
}
