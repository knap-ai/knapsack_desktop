import { a as normalizeLowercaseStringOrEmpty$1, s as normalizeOptionalLowercaseString } from "./string-coerce-C1IzJjqi.js";
import { v as sleep } from "./utils-BMRcljdi.js";
import { s as normalizeStringEntries } from "./string-normalization-Bvcn03I9.js";
import { a as withFileLock$1 } from "./file-lock-DUSWWPN-.js";
...
import { C as buildUserAgent, D as formatUnknownError, E as formatMSTeamsSendErrorHint, O as isRevokedProxyError, S as loadMSTeamsSdkWithAuth, T as classifyMSTeamsSendError, _ as resolveMSTeamsStorePath, b as createMSTeamsAdapter, h as resolveMSTeamsCredentials, m as loadDelegatedTokens, v as readAccessToken, x as createMSTeamsTokenProvider } from "./graph-users-CHwyVrDv.js";
...
function findPreferredDmConversationByUserId(entries, id) {
	...
	for (const entry of entries) {
		...
		const convType = normalizeLowercaseStringOrEmpty$1(entry.reference.conversation?.conversationType ?? "");
...
function buildTeamsFileInfoCard(file) {
	...
	const fileType = lastDot >= 0 ? normalizeLowercaseStringOrEmpty$1(file.name.slice(lastDot + 1)) : "";
	return {
		contentType: "application/vnd.microsoft.teams.card.file.info",
...
async function resolveMSTeamsSendContext(params) {
	...
	if (recipient.type === "user") {
		const resolvedType = normalizeLowercaseStringOrEmpty$1(ref.conversation?.conversationType ?? "");
	...
	const storedConversationType = normalizeLowercaseStringOrEmpty$1(ref.conversation?.conversationType ?? "");
