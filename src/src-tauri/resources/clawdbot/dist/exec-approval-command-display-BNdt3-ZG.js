import { a as resolveRedactOptions, r as redactSensitiveText } from "./redact-Bl2deF7j.js";
...
function escapeInvisibles(text) {
...
function sanitizeExecApprovalDisplayText(commandText) {
	if (commandText.length > EXEC_APPROVAL_MAX_INPUT) return EXEC_APPROVAL_OVERSIZED_MARKER;
	const rawRedacted = redactSensitiveText(commandText, { mode: "tools" });
	const { stripped, strippedToOrig } = buildStrippedView(commandText);
	if (redactSensitiveText(stripped, { mode: "tools" }) === stripped) return truncateForDisplay(escapeInvisibles(rawRedacted));
	...
	if (!bypassDetected) return truncateForDisplay(escapeInvisibles(rawRedacted));
