...
import { s as resolveMergedAccountConfig } from "./account-helpers-DqNwQN08.js";
...
function normalizeLocalE164(number) {
	const digits = number.replace(/^whatsapp:/i, "").trim().replace(/[^\d+]/g, "");
...
function canBypassConfigWritePolicy(params) {
	return params.channel?.trim().toLowerCase() === INTERNAL_MESSAGE_CHANNEL && params.gatewayClientScopes?.includes("operator.admin") === true;
}
...
function formatWhatsAppConfigAllowFromEntries(allowFrom) {
	return allowFrom.map((entry) => String(entry).trim()).filter((entry) => Boolean(entry)).map((entry) => entry === "*" ? entry : normalizeLocalWhatsAppTarget(entry)).filter((entry) => Boolean(entry));
}
...
//#endregion
export { resolveWhatsAppConfigDefaultTo as S, resolveChannelConfigWrites as _, createHybridChannelConfigAdapter as a, resolveOptionalConfigString as b, createScopedChannelConfigAdapter as c, createTopLevelChannelConfigAdapter as d, createTopLevelChannelConfigBase as f, mapAllowFromEntries as g, formatWhatsAppConfigAllowFromEntries as h, canBypassConfigWritePolicy as i, createScopedChannelConfigBase as l, formatTrimmedAllowFromEntries as m, authorizeConfigWrite as n, createHybridChannelConfigBase as o, formatConfigWriteDeniedMessage as p, buildAccountScopedDmSecurityPolicy as r, createScopedAccountConfigAccessors as s, adaptScopedAccountAccessor as t, createScopedDmSecurityResolver as u, resolveIMessageConfigAllowFrom as v, resolveWhatsAppConfigAllowFrom as x, resolveIMessageConfigDefaultTo as y };
