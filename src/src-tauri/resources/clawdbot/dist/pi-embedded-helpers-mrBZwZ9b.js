...
import { C as normalizeAccountId$1, E as resolveAgentIdFromSessionKey, T as normalizeMainKey, _ as DEFAULT_AGENT_ID, a as DEFAULT_IDENTITY_FILENAME, c as DEFAULT_USER_FILENAME, h as resolveOpenClawPackageRootSync, i as DEFAULT_HEARTBEAT_FILENAME, l as ensureAgentWorkspace, n as DEFAULT_AGENT_WORKSPACE_DIR, o as DEFAULT_SOUL_FILENAME, r as DEFAULT_BOOTSTRAP_FILENAME, s as DEFAULT_TOOLS_FILENAME, t as DEFAULT_AGENTS_FILENAME, w as normalizeAgentId, y as buildAgentMainSessionKey } from "./workspace-CUznpDHg.js";
import { A as escapeRegExp, B as resolveUserPath, C as requireActivePluginRegistry, I as normalizeE164, T as CONFIG_DIR, d as defaultRuntime, g as CHAT_CHANNEL_ORDER, h as CHANNEL_IDS, l as createSubsystemLogger, n as runExec, st as resolvePreferredOpenClawTmpDir, v as getChatChannelMeta } from "./exec-DBtWJ4Ld.js";
import { l as resolveSessionAgentId, n as resolveAgentConfig } from "./agent-scope-xZu8sXcF.js";
import { G as resolveSlackAccount, H as resolveTelegramAccount, K as resolveSlackReplyToMode, L as normalizeChannelId, Y as normalizeChatType, Z as resolveDiscordAccount, _ as listDeliverableMessageChannels, a as resizeToJpeg, n as getImageMetadata, v as normalizeMessageChannel, z as normalizeWhatsAppTarget } from "./image-ops-CQkyIkuI.js";
import { t as parseBooleanValue } from "./boolean-Bb19hm9Y.js";
import { t as formatCliCommand } from "./command-format-BmTFj7S4.js";
import { A as resolveWhatsAppAccount, _ as resolveEnableState, a as writeConfigFile, d as parseDurationMs, g as normalizePluginsConfig, n as loadConfig, p as loadPluginManifestRegistry, s as parseByteSize, t as createConfigIO, v as resolveMemorySlotDecision } from "./config-DAGaFnCt.js";
import { n as resolveSignalAccount } from "./accounts-DGzI_52F.js";
import { n as MANIFEST_KEY, t as LEGACY_MANIFEST_KEYS } from "./legacy-names-BWhO1_ll.js";
import { A as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, C as DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH, D as DEFAULT_BROWSER_EVALUATE_ENABLED, E as DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, O as DEFAULT_OPENCLAW_BROWSER_COLOR, T as DEFAULT_AI_SNAPSHOT_MAX_CHARS, _ as ensureChromeExtensionRelayServer, a as resolveOpenClawUserDataDir, b as isLoopbackHost, c as captureScreenshot, d as normalizeCdpWsUrl, f as snapshotAria, h as getHeadersWithAuth, i as launchOpenClawChrome, k as DEFAULT_OPENCLAW_BROWSER_ENABLED, l as createTargetViaCdp, n as isChromeCdpReady, o as stopOpenClawChrome, p as appendCdpPath, r as isChromeReachable, s as resolveBrowserExecutableForPlatform, v as stopChromeExtensionRelayServer, w as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS } from "./chrome-DdEflVKx.js";
import { t as parseFrontmatterBlock } from "./frontmatter-Uu27Y56g.js";
import { n as formatErrorMessage, t as extractErrorCode } from "./errors-Daa2hz8x.js";
import { r as saveMediaBuffer, t as ensureMediaDir } from "./store-l7p5BQMc.js";
import { t as resolveIMessageAccount } from "./accounts-B2d4d0iP.js";
import { n as resolveSessionFilePath, s as resolveStorePath, t as resolveDefaultSessionStorePath } from "./paths-CpGplyYJ.js";
import { t as emitSessionTranscriptUpdate } from "./transcript-events-JGGQViao.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-BVaD9DCP.js";
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-D1WsVpMm.js");
	} catch (err) {
...
//#endregion
//#region src/browser/routes/path-output.ts
...
function createProfileContext(opts, profile) {
	...
	const resetProfile = async () => {
		...
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-D1WsVpMm.js")).closePlaywrightBrowserConnection();
		} catch {}
...
function createBrowserRouteContext(opts) {
	...
	const applyResolvedConfig = (current, freshResolved) => {
	...
	const refreshResolvedConfig = (current) => {
		if (!refreshConfigFromDisk) return;
		const cfg = loadConfig();
		applyResolvedConfig(current, resolveBrowserConfig(cfg.browser, cfg));
	};
	const refreshResolvedConfigFresh = (current) => {
		if (!refreshConfigFromDisk) return;
		const freshCfg = createConfigIO().loadConfig();
		applyResolvedConfig(current, resolveBrowserConfig(freshCfg.browser, freshCfg));
	};
...
//#endregion
export { updateSessionStoreEntry as $, resolveBootstrapMaxChars as $t, isCompactionFailureError as A, resolveBrowserConfig as At, parseImageSizeError as B, applySkillEnvOverrides as Bt, formatAssistantErrorText as C, resolveChannelGroupPolicy as Ct, isAuthAssistantError as D, resolveGroupSessionKey as Dt, getApiErrorPayloadFingerprint as E, resolveConversationLabel as Et, isRateLimitAssistantError as F, resolveSkillsPromptForRun as Ft, extractDeliveryInfo as G, collectExplicitAllowlist as Gt, ensureSandboxWorkspaceForSession as H, resolveSandboxConfigForAgent as Ht, isRawApiErrorPayload as I, resolvePluginSkillDirs as It, loadSessionStore as J, normalizeToolName as Jt, appendAssistantMessageToSessionTranscript as K, expandPolicyWithPluginGroups as Kt, isTimeoutErrorMessage as L, assertMediaNotDataUrl as Lt, isFailoverAssistantError as M, buildWorkspaceSkillCommandSpecs as Mt, isFailoverErrorMessage as N, buildWorkspaceSkillSnapshot as Nt, isBillingAssistantError as O, createBrowserRouteContext as Ot, isLikelyContextOverflowError as P, loadWorkspaceSkillEntries as Pt, updateSessionStore as Q, ensureSessionHeader$1 as Qt, isTransientHttpError as R, assertSandboxPath as Rt, classifyFailoverReason as S, listChannelDocks as St, formatRawAssistantErrorForUi as T, resolveChannelGroupToolsPolicy as Tt, resolveSandboxContext as U, applyOwnerOnlyToolPolicy as Ut, sanitizeUserFacingText as V, applySkillEnvOverridesFromSnapshot as Vt, resolveSandboxRuntimeStatus as W, buildPluginToolGroups as Wt, recordSessionMetaFromInbound as X, stripPluginOnlyAllowlist as Xt, readSessionUpdatedAt as Y, resolveToolProfilePolicy as Yt, updateLastRoute as Z, buildBootstrapContextFiles as Zt, sanitizeSessionMessagesImages as _, canonicalizeMainSessionAlias as _t, formatXHighModelHint as a, normalizeDeliveryContext as at, isGoogleModelApi as b, deriveSessionMetaPatch as bt, normalizeReasoningLevel as c, acquireSessionWriteLock as ct, normalizeVerboseLevel as d, resolveChannelResetConfig as dt, sanitizeGoogleTurnOrdering as en, isCacheEnabled as et, resolveResponseUsageMode as f, resolveSessionResetPolicy as ft, normalizeTextForComparison as g, resolveFreshSessionTotalTokens as gt, isMessagingToolDuplicateNormalized as h, DEFAULT_RESET_TRIGGERS as ht, formatThinkingLevels as i, mergeDeliveryContext as it, isContextOverflowError as j, resolveProfile as jt, isCloudCodeAssistFormatError as k, registerBrowserRoutes as kt, normalizeThinkLevel as l, resolveSessionKey as lt, isMessagingToolDuplicate as m, resolveThreadFlag as mt, validateGeminiTurns as n, deliveryContextFromSession as nt, listThinkingLevels as o, normalizeSessionDeliveryFields as ot, supportsXHighThinking as p, resolveSessionResetType as pt, resolveMirroredTranscriptText as q, expandToolGroups as qt, pickFallbackThinkingLevel as r, deliveryContextKey as rt, normalizeElevatedLevel as s, normalizeAccountId as st, validateAnthropicTurns as t, resolveCacheTtlMs as tt, normalizeUsageDisplay as u, evaluateSessionFreshness as ut, downgradeOpenAIReasoningBlocks as v, resolveExplicitAgentSessionKey as vt, formatBillingErrorMessage as w, resolveChannelGroupRequireMention as wt, BILLING_ERROR_USER_MESSAGE as x, getChannelDock as xt, isAntigravityClaude as y, resolveMainSessionKey as yt, parseImageDimensionError as z, resolveSandboxedMediaSource as zt };