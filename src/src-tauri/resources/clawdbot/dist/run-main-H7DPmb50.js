...
import { F as VERSION, I as loadDotEnv } from "./config-BJWIHf6x.js";
...
import { d as installUnhandledRejectionHandler } from "./runner-BY_Cu0GM.js";
...
import { n as emitCliBanner, t as ensureConfigReady } from "./config-guard-CACfoFyI.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-ifSF2YWo.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-CJQ1y5Q3.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-BkBHOhDB.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-BRDyE0fe.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-C--Zm-5F.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-nBoIPlMw.js").then((n) => n.t);
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-nBoIPlMw.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-BIcSB9ge.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-BIcSB9ge.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-xBGqNo3t.js");
	...
	if (primary && shouldRegisterPrimarySubcommand(parseArgv)) {
		const { registerSubCliByName } = await import("./register.subclis-_zfREDBy.js").then((n) => n.i);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { loadConfig } = await import("./config-BJWIHf6x.js").then((n) => n.t);
