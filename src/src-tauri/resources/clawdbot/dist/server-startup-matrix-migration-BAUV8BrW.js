...
import { s as listBundledPluginMetadata } from "./chat-meta-Cdrnv7R-.js";
import { s as resolveStableNodePath } from "./runtime-paths-CEXChkqS.js";
import { y as applyPathPrepend } from "./model-runtime-D4KJqIwp.js";
import { i as parseAbsoluteTimeMs } from "./stagger-B4-85uDC.js";
import { o as trimLogTail } from "./restart-sentinel-CVZLtNbX.js";
import { i as autoMigrateLegacyMatrixState, n as hasPendingMatrixMigration, o as autoPrepareLegacyMatrixCrypto, r as maybeCreateMatrixMigrationSnapshot, t as hasActionableMatrixMigration } from "./matrix-migration-snapshot-DkdZAARe.js";
import { i as resolveControlUiDistIndexPathForRoot, r as resolveControlUiDistIndexHealth } from "./control-ui-assets-DYuJ0rj8.js";
import { a as isBetaTag, n as DEV_BRANCH, o as isStableTag, r as channelToNpmTag } from "./update-channels-SLpbr_hy.js";
import { n as compareSemverStrings, s as detectPackageManager$1 } from "./update-check-DTdDwACj.js";
...
async function resolveAvailableManager(runCommand, root, timeoutMs) {
	...
	if (preferred === "pnpm" && await ensurePnpmAvailable(runCommand, timeoutMs)) return {
		manager: "pnpm",
		fallback: false
	};
	for (const manager of managerPreferenceOrder(preferred)) if (await isManagerAvailable(runCommand, manager, timeoutMs)) return {
		manager,
...
async function runGatewayUpdate(opts = {}) {
	...
	if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
		...
		if (channel === "dev") {
			...
			try {
				for (const sha of candidates) {
					...
					const depsStep = await runStep(step(`preflight deps install (${shortSha})`, managerInstallArgs(manager.manager, { compatFallback: manager.fallback && manager.manager === "npm" }), worktreeDir));
					steps.push(depsStep);
					if (depsStep.exitCode !== 0) continue;
					const buildStep = await runStep(step(`preflight build (${shortSha})`, managerScriptArgs(manager.manager, "build"), worktreeDir));
					steps.push(buildStep);
					if (buildStep.exitCode !== 0) continue;
					const lintStep = await runStep(step(`preflight lint (${shortSha})`, managerScriptArgs(manager.manager, "lint"), worktreeDir));
					steps.push(lintStep);
		...
		const manager = await resolveAvailableManager(runCommand, gitRoot, timeoutMs);
		const depsStep = await runStep(step("deps install", managerInstallArgs(manager.manager, { compatFallback: manager.fallback && manager.manager === "npm" }), gitRoot));
		steps.push(depsStep);
...
//#endregion
export { computeNextRunAtMs as _, collectInstalledGlobalPackageErrors as a, detectGlobalInstallManagerForRoot as c, resolveGlobalInstallSpec as d, resolveGlobalPackageRoot as f, coerceFiniteScheduleNumber as g, readPackageVersion as h, cleanupGlobalRenameDirs as i, globalInstallArgs as l, readPackageName as m, runGatewayUpdate as n, createGlobalInstallEnv as o, normalizePackageTagInput as p, canResolveRegistryVersionForPackageTarget as r, detectGlobalInstallManagerByPresence as s, runStartupMatrixMigration as t, resolveExpectedInstalledVersionFromSpec as u, computePreviousRunAtMs as v };
