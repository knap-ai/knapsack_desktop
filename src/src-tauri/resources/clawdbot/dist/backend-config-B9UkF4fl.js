...
import { l as normalizeExtraMemoryPaths, m as statRegularFile, p as isFileMissingError, s as isMemoryPath } from "./internal-CcZnGEyS.js";
...
function resolveMemoryBackendConfig(params) {
	...
	const mergedExtraCollections = [...params.cfg.agents?.defaults?.memorySearch?.qmd?.extraCollections ?? [], ...agentEntry?.memorySearch?.qmd?.extraCollections ?? []].filter((value) => Boolean(value && typeof value === "object" && typeof value.path === "string"));
	const allQmdPaths = [
		...qmdCfg?.paths ?? [],
		...searchExtraPaths,
