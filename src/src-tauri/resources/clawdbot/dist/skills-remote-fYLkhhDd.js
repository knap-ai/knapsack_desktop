...
import { n as readJsonFile, r as writeJsonAtomic, t as createAsyncLock } from "./json-files-R-JecRHb.js";
import { i as NODE_SYSTEM_RUN_COMMANDS } from "./node-commands-BnNUZOb5.js";
import { l as resolvePluginSkillDirs, o as loadWorkspaceSkillEntries } from "./skills-C4NiYd5P.js";
import { t as listAgentWorkspaceDirs } from "./workspace-dirs-CgiZHK2d.js";
import { a as resolvePairingPaths, i as reconcilePendingPairingRequests, n as verifyPairingToken, o as resolveMissingRequestedScope, r as pruneExpiredPending, t as generatePairingToken } from "./pairing-token-VGfB192W.js";
...
function resolveNodeApprovalRequiredScope(pending) {
	const commands = Array.isArray(pending.commands) ? pending.commands : [];
	if (commands.some((command) => NODE_SYSTEM_RUN_COMMANDS.some((allowed) => allowed === command))) return OPERATOR_ADMIN_SCOPE;
	if (commands.length > 0) return OPERATOR_WRITE_SCOPE;
	return null;
}
...
async function approveNodePairing(requestId, optionsOrBaseDir, maybeBaseDir) {
	...
	return await withLock(async () => {
		...
		if (requiredScope && options !== void 0) {
...
function getRemoteSkillEligibility() {
	...
	return {
		...
		note: labels.length > 0 ? `Remote macOS node available (${labels.join(", ")}). Run macOS-only skills via exec host=node on that node.` : "Remote macOS node available. Run macOS-only skills via exec host=node on that node."
	};
}
...
//#endregion
export { getSkillsSnapshotVersion as _, refreshRemoteNodeBins as a, approveNodePairing as c, rejectNodePairing as d, renamePairedNode as f, ensureSkillsWatcher as g, verifyNodeToken as h, refreshRemoteBinsForConnectedNodes as i, getPairedNode as l, updatePairedNodeMetadata as m, primeRemoteSkillsCache as n, removeRemoteNodeInfo as o, requestNodePairing as p, recordRemoteNodeInfo as r, setSkillsRemoteRegistry as s, getRemoteSkillEligibility as t, listNodePairing as u, registerSkillsChangeListener as v };
