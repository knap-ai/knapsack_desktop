...
import { c as resolveComparableIdentity } from "./identity-BiH3hhFs.js";
...
async function stringifyCreds(creds) {
	const { BufferJSON } = await import("./session.runtime-sWPafQwp.js").then((n) => n.a);
...
async function shouldClearOnLogout(authDir, isLegacyAuthDir) {
	try {
		if (!(await fs$1.stat(authDir)).isDirectory()) return true;
		if (isLegacyAuthDir) return (await fs$1.readdir(authDir, { withFileTypes: true })).some((entry) => {
			if (!entry.isFile()) return false;
			if (entry.name === "oauth.json") return false;
			if (entry.name === "creds.json" || entry.name === "creds.json.bak") return true;
			return entry.name.endsWith(".json") ? /^(app-state-sync|session|sender-key|pre-key)-/.test(entry.name) : false;
		});
		if ((await fs$1.stat(resolveWebCredsPath(authDir)).catch(() => null))?.isFile()) return true;
		return (await fs$1.stat(resolveWebCredsBackupPath(authDir)).catch(() => null))?.isFile() === true;
	} catch (error) {
