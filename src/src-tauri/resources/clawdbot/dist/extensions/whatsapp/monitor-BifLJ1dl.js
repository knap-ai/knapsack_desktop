...
import { a as markdownToWhatsApp, i as jidToE164, o as resolveJidToE164, r as isSelfChatMode, s as toWhatsappJid, t as text_runtime_exports } from "./text-runtime-BvOFWrFo.js";
import { i as normalizeWhatsAppPayloadTextPreservingIndentation, n as normalizeWhatsAppOutboundPayload, o as sendWhatsAppOutboundWithRetry, t as normalizeWhatsAppLoadedMedia } from "./outbound-media-contract--dg4rsHm.js";
import { r as sendReactionWhatsApp } from "./send-CsXEtdbz.js";
import { n as cacheInboundMessageMeta, r as lookupInboundMessageMeta, t as buildQuotedMessageOptions } from "./quoted-message-CGSqtm8Y.js";
import { _ as readWebSelfIdentityForDecision, a as getWebAuthAgeMs, h as readWebSelfId, r as WhatsAppAuthUnstableError } from "./auth-store-Dt6dXsn-.js";
import { a as getSelfIdentity, c as resolveComparableIdentity, i as getReplyContext, n as getMentionIdentities, o as getSenderIdentity, r as getPrimaryIdentityId, s as identitiesOverlap, t as getComparableIdentityValues } from "./identity-BiH3hhFs.js";
import { i as resolveWhatsAppInboundPolicy, n as checkInboundAccessControl, r as resolveWhatsAppCommandAuthorized } from "./access-control-D3uf0CqN.js";
import { c as waitForWaConnection, d as newConnectionId, f as resolveHeartbeatSeconds, l as DEFAULT_RECONNECT_POLICY, m as sleepWithAbort, n as WhatsAppConnectionController, o as createWaSocket, p as resolveReconnectPolicy, u as computeBackoff } from "./connection-controller-Cxgtxfr6.js";
...
import { evaluateSessionFreshness, getRuntimeConfigSourceSnapshot, loadConfig as loadConfig$1, loadSessionStore, recordSessionMetaFromInbound, resolveChannelContextVisibilityMode, resolveChannelResetConfig, resolveSessionKey, resolveSessionResetPolicy, resolveSessionResetType, resolveStorePath, resolveThreadFlag, updateLastRoute, updateSessionStore } from "openclaw/plugin-sdk/config-runtime";
...
import { drainPendingDeliveries, enqueueSystemEvent, recordChannelActivity } from "openclaw/plugin-sdk/infra-runtime";
...
function resolveOutboundMessageId(result) {
	return typeof result === "object" && result && "key" in result ? result.key?.id ?? "unknown" : "unknown";
}
...
function isBotMentionedFromTargets(msg, mentionCfg, targets) {
	...
	const isSelfChat = typeof mentionCfg.isSelfChat === "boolean" ? mentionCfg.isSelfChat : isSelfChatMode(targets.self.e164, mentionCfg.allowFrom);
...
async function applyGroupGating(params) {
	...
	if (!shouldBypassMention && requireMention && mentionDecision.shouldSkip) return skipGroupMessageAndStoreHistory(params, `Group message stored for context (no mention detected) in ${params.conversationId}: ${params.msg.body}`);
...
async function deliverWebReply(params) {
	...
	const normalizedReply = normalizeWhatsAppOutboundPayload(replyResult, { normalizeText: normalizeWhatsAppPayloadTextPreservingIndentation });
	...
	await sendMediaWithLeadingCaption({
		...
		send: async ({ mediaUrl, caption }) => {
			const media = normalizeWhatsAppLoadedMedia(await loadWebMedia$1(mediaUrl, {
	...
	for (const chunk of remainingText) await msg.reply(chunk, getQuote());
}
//#endregion
...
async function dispatchWhatsAppBufferedReply(params) {
	...
	const { queuedFinal, counts } = await dispatchReplyWithBufferedBlockDispatcher({
		...
		dispatcherOptions: {
			...
			deliver: async (payload, info) => {
				...
				didSendReply = true;
				const shouldLog = deliveryPayload.text ? true : void 0;
				...
				if (shouldLogVerbose$1()) logVerbose$1(`Reply body: ${deliveryPayload.text != null ? reply.text : "<media>"}${reply.hasMedia ? " (media)" : ""} -> ${fromDisplay}`);
			},
	...
	if (!(queuedFinal || counts.tool > 0 || counts.block > 0 || counts.final > 0)) {
...
async function processMessage(params) {
	...
	if (params.preflightAudioTranscript === void 0 && hasAudioBody && params.msg.mediaPath) try {
		const { transcribeFirstAudio } = await import("./audio-preflight.runtime-DYBhC358.js");
	...
	if (params.ackAlreadySent !== true) await maybeSendAckReaction({
		cfg: params.cfg,
		msg: params.msg,
		agentId: params.route.agentId,
		sessionKey: params.route.sessionKey,
...
function loadReplyResolverRuntime() {
	replyResolverRuntimePromise ??= import("./reply-resolver.runtime-BVGUQhpd.js");
	...
	try {
		while (true) {
			...
			try {
				connection = await controller.openConnection({
					...
					onWatchdogTimeout: (snapshot) => {
						...
						const minutesSinceLastMessage = Math.floor((Date.now() - watchdogBaselineAt) / 6e4);
			...
			if (decision.action === "stop") {
				...
				if (decision.healthState === "logged-out") runtime.error(`WhatsApp session logged out. Run \`${formatCliCommand("openclaw channels login --channel web")}\` to relink.`);
					...
					runtime.error(`WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("openclaw channels login --channel web")}\`. Stopping web monitoring.`);
				} else {
					reconnectLogger.warn({
						connectionId: connection.connectionId,
						status: decision.normalized.statusLabel,
...
//#endregion
export { whatsappHeartbeatLog as S, resolveSessionKey as _, loadWebMediaRaw as a, resolveStorePath as b, monitorWebInbox as c, extractMediaPlaceholder as d, extractText as f, resolveChannelResetConfig as g, loadSessionStore as h, loadWebMedia$1 as i, extractContactContext as l, evaluateSessionFreshness as m, LocalMediaAccessError as n, optimizeImageToJpeg as o, resetWebInboundDedupe as p, getDefaultLocalRoots as r, optimizeImageToPng as s, monitorWebChannel as t, extractLocationData as u, resolveSessionResetPolicy as v, resolveThreadFlag as x, resolveSessionResetType as y };
