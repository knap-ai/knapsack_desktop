...
function resolveSlackRoutingContext(params) {
	...
	const boundThreadRoute = canonicalThreadId ? resolveRuntimeConversationBindingRoute({
		route,
		conversation: {
			channel: "slack",
			accountId: account.accountId,
...
async function prepareSlackMessage(params) {
	...
	const { route, runtimeBinding, replyToMode, threadContext, threadTs, isThreadReply, threadKeys, sessionKey, historyKey } = resolveSlackRoutingContext({
		ctx,
		account,
		message,
		isDirectMessage,
