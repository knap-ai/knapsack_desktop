...
import { i as normalizeSlackAllowOwnerEntry } from "./allow-list-GR9Gx-es.js";
import { t as SLACK_TEXT_LIMIT } from "./limits-DeULmw1T.js";
import { n as resolveSlackNativeStreaming, r as resolveSlackStreamingMode, t as mapStreamingModeToSlackLegacyDraftStreamMode } from "./streaming-compat-Dw57oLZf.js";
import { i as buildSlackBlocksFallbackText, r as normalizeSlackOutboundText, t as sendMessageSlack } from "./send-39pZP1aH.js";
import { f as removeSlackReaction, l as reactSlackMessage, r as editSlackMessage, t as deleteSlackMessage } from "./actions-CwbvM4sE.js";
import { r as recordSlackThreadParticipation } from "./sent-thread-cache-CfB-Fl5Q.js";
import { C as updateLastRoute, S as resolveStorePath } from "./room-context-BEHuA2F1.js";
import { t as escapeSlackMrkdwn } from "./mrkdwn-18IzcEAY.js";
import { n as resolveSlackThreadTargets, t as prepareSlackMessage } from "./prepare-BQ2zoi19.js";
import { a as resolveDeliveredSlackReplyThreadTs, i as readSlackReplyBlocks, n as deliverReplies, o as resolveSlackThreadTs, t as createSlackReplyDeliveryPlan } from "./replies-B3vw0pqw.js";
import { normalizeOptionalLowercaseString } from "openclaw/plugin-sdk/text-runtime";
import { resolveAgentOutboundIdentity } from "openclaw/plugin-sdk/outbound-runtime";
import { resolveChannelStreamingBlockEnabled, resolveChannelStreamingNativeTransport, resolveChannelStreamingPreviewToolProgress } from "openclaw/plugin-sdk/channel-streaming";
...
import { createDraftStreamLoop, deliverFinalizableDraftPreview } from "openclaw/plugin-sdk/channel-lifecycle";
import { createReplyDispatcherWithTyping, dispatchInboundMessage } from "openclaw/plugin-sdk/reply-runtime";
...
async function dispatchPreparedSlackMessage(prepared) {
	...
	const disableBlockStreaming = resolveSlackDisableBlockStreaming({
		...
		blockStreamingEnabled: resolveChannelStreamingBlockEnabled(account.config)
	});
	...
	const { dispatcher, replyOptions, markDispatchIdle } = createReplyDispatcherWithTyping({
		...
		deliver: async (payload, info) => {
			...
			if (await deliverFinalizableDraftPreview({
				...
				buildFinalEdit: () => {
					if (!previewStreamingEnabled || streamMode === "status_final" || reply.hasMedia || payload.isError || trimmedFinalText.length === 0 && !slackBlocks?.length) return;
					return {
	...
	const pushPreviewToolProgress = (line) => {
		...
		if (previewToolProgressLines.at(-1) === escaped) return;
		previewToolProgressLines = [...previewToolProgressLines, escaped].slice(-8);
		draftStream.update(["Working…", ...previewToolProgressLines.map((entry) => `• ${entry}`)].join("\n"));
	...
	const anyReplyDelivered = observedReplyDelivery || queuedFinal || streamFallbackDelivered || (counts.block ?? 0) > 0 || (counts.final ?? 0) > 0;
