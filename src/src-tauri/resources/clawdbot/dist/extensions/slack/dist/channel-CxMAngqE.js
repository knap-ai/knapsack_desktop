...
import { d as PAIRING_APPROVED_MESSAGE, m as resolveConfiguredFromRequiredCredentialStatuses, p as projectCredentialSnapshotFields, r as createSlackWebClient, u as DEFAULT_ACCOUNT_ID } from "./client-BY0ZhGHl.js";
...
import { i as slackSecurityAdapter, n as isSlackPluginAccountConfigured, r as slackConfigAdapter, t as createSlackPluginBase } from "./shared-C9qcXNm_.js";
...
function resolveForwardingFallbackSlackTarget(target) {
...
function isSlackPluginForwardingFallbackHandledNatively(input) {
	...
	if (originTarget && slackTargetsMatch(forwardingTarget, originTarget)) return true;
...
function resolveSlackApproverDmTargets(params) {
	...
	return (params.approvalKind === "plugin" ? getSlackApprovalApprovers(params) : getSlackExecApprovalApprovers(params)).map((approver) => ({ to: `user:${approver}` }));
}
const baseSlackApprovalCapability = createApproverRestrictedNativeApprovalCapability({
	...
	nativeRuntime: createLazyChannelApprovalNativeRuntimeAdapter({
		...
		load: async () => (await import("./approval-handler.runtime-FX6ylx3g.js")).slackApprovalNativeRuntime
	})
});
...
const slackApprovalCapability = {
	...
	native: baseSlackNativeAdapter ? {
		...
		describeDeliveryCapabilities: (params) => {
			return {
				...baseSlackNativeAdapter.describeDeliveryCapabilities(params),
...
async function loadSlackActionRuntime$1() {
	slackActionRuntimePromise$1 ??= import("./action-runtime.runtime-zrLbExpY.js");
...
const loadSlackSetupSurfaceModule = createLazyRuntimeModule(() => import("./setup-surface-BO6IU00P.js"));
const loadSlackScopesModule = createLazyRuntimeModule(() => import("./scopes-CWwLQOAw.js"));
const loadSlackOutboundAdapterModule = createLazyRuntimeModule(() => import("./outbound-adapter-Ba8hu4Vf.js").then((n) => n.t));
...
const loadSlackDirectoryConfigModule = createLazyRuntimeModule(() => import("./directory-config-BYH5Y8si.js").then((n) => n.t));
const loadSlackResolveChannelsModule = createLazyRuntimeModule(() => import("./resolve-channels-CXV6oSKF.js").then((n) => n.n));
const loadSlackResolveUsersModule = createLazyRuntimeModule(() => import("./resolve-users-BCBZwBB5.js").then((n) => n.n));
async function loadSlackActionRuntime() {
	slackActionRuntimePromise ??= import("./action-runtime.runtime-zrLbExpY.js");
...
async function loadSlackProbeModule() {
	slackProbeModulePromise ??= import("./probe-DTCmOwpe.js").then((n) => n.n);
	return await slackProbeModulePromise;
}
async function loadSlackMonitorModule() {
	slackMonitorModulePromise ??= import("./monitor-CquryIVN.js").then((n) => n.t);
	return await slackMonitorModulePromise;
}
async function loadSlackDirectoryLiveModule() {
	slackDirectoryLiveModulePromise ??= import("./directory-live-COvr0w-z.js").then((n) => n.t);
