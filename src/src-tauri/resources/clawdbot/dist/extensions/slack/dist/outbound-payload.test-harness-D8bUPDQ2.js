import { n as slackOutbound } from "./outbound-adapter-Ba8hu4Vf.js";
...
function comparePrimitive(a, b, options, memorize = DEFAULT_MEMORIZE) {
	const aFormat = memorize("expected", format$1(a, FORMAT_OPTIONS));
	const bFormat = memorize("actual", format$1(b, FORMAT_OPTIONS));
...
function getObjectsDifference(a, b, formatOptions, options, memorize = DEFAULT_MEMORIZE) {
	...
	if (aCompare === bCompare) return getCommonMessage(NO_DIFF_MESSAGE, options);
		...
		const aDisplay = memorize("expected", format$1(a, formatOptions));
		const bDisplay = memorize("actual", format$1(b, formatOptions));
...
function prettifyValue(value, options) {
	if (typeof value !== "string") return format$1(value, getDefaultFormatOptions(options));
	return value;
}
//#endregion
...
async function saveInlineSnapshots(environment, snapshots) {
	const MagicString = (await import("./magic-string.es-BPXBBMwL.js")).default;
