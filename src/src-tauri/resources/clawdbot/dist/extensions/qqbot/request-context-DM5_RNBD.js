import { T as debugLog } from "./sender-BZ_TJkxQ.js";
import { c as getQQBotDataDir, o as getHomeDir, p as isWindows } from "./target-parser-C00R-uf5.js";
...
var SlashCommandRegistry = class {
	...
	register(cmd) {
		if (cmd.requireAuth) this.frameworkCommands.set(lc(cmd.name), cmd);
		else this.commands.set(lc(cmd.name), cmd);
	}
...
let _resolveVersion = null;
/** Register the version resolver — called by the outer layer. */
function registerVersionResolver(fn) {
	_resolveVersion = fn;
}
...
function registerPluginVersion(version) {
...
registerCommand({
	...
	handler: async () => {
		return [
			`🦞 OpenClaw 框架版本：${resolveRuntimeServiceVersion()}`,
...
registerCommand({
	...
	handler: async (ctx) => {
		...
		let runtime;
		try {
			if (!_runtimeGetter) throw new Error("runtime not available");
		...
		const loadExecConfig = () => {
			const exec = (configApi.loadConfig().tools ?? {}).exec ?? {};
			return {
...
function normalizeCommandAllowlistEntry(entry) {
	if (typeof entry === "string" || typeof entry === "number" || typeof entry === "boolean" || typeof entry === "bigint") return `${entry}`.trim().replace(/^qqbot:\s*/i, "").trim();
	return "";
}
...
//#endregion
export { registerApproveRuntimeGetter as a, matchSlashCommand as i, runWithRequestContext as n, registerPluginVersion as o, getFrameworkCommands as r, registerVersionResolver as s, getRequestContext as t };
