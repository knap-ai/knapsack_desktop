...
import { n as LogService, r as noop } from "./logger-DsQxmf2q.js";
import { i as MATRIX_IDB_SNAPSHOT_LOCK_OPTIONS, n as isRepairableSecretStorageAccessError } from "./recovery-key-store-If4OMiPE.js";
...
async function loadMatrixCryptoNodeRuntime() {
	matrixCryptoNodeRuntimePromise ??= import("./crypto-node.runtime-B_K0KCl7.js");
...
var MatrixVerificationManager = class {
	...
	maybeAutoConfirmSas(session) {
		...
		session.sasAutoConfirmTimer = setTimeout(() => {
			...
			this.confirmSasForSession(session, callbacks, { trustOwnDevice: false }).then(() => {
				this.touchVerificationSession(session);
			}).catch((err) => {
				session.error = formatMatrixErrorMessage(err);
				this.touchVerificationSession(session);
