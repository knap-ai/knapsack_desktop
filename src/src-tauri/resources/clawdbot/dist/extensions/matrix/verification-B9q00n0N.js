...
import { n as formatMatrixEncryptionUnavailableError } from "./encryption-guidance-zTcuVe8M.js";
...
var verification_exports = /* @__PURE__ */ __exportAll({
	acceptMatrixVerification: () => acceptMatrixVerification,
	bootstrapMatrixVerification: () => bootstrapMatrixVerification,
	cancelMatrixVerification: () => cancelMatrixVerification,
	confirmMatrixVerificationReciprocateQr: () => confirmMatrixVerificationReciprocateQr,
	confirmMatrixVerificationSas: () => confirmMatrixVerificationSas,
	generateMatrixVerificationQr: () => generateMatrixVerificationQr,
	getMatrixEncryptionStatus: () => getMatrixEncryptionStatus,
	getMatrixRoomKeyBackupStatus: () => getMatrixRoomKeyBackupStatus,
	getMatrixVerificationSas: () => getMatrixVerificationSas,
	getMatrixVerificationStatus: () => getMatrixVerificationStatus,
	listMatrixVerifications: () => listMatrixVerifications,
	mismatchMatrixVerificationSas: () => mismatchMatrixVerificationSas,
	requestMatrixVerification: () => requestMatrixVerification,
	resetMatrixRoomKeyBackup: () => resetMatrixRoomKeyBackup,
	restoreMatrixRoomKeyBackup: () => restoreMatrixRoomKeyBackup,
	runMatrixSelfVerification: () => runMatrixSelfVerification,
	scanMatrixVerificationQr: () => scanMatrixVerificationQr,
	startMatrixVerification: () => startMatrixVerification,
...
async function waitForMatrixSelfVerificationTrustStatus(params) {
	...
	while (Date.now() - startedAt < params.timeoutMs) {
		const [status, crossSigning] = await Promise.all([params.client.getOwnDeviceIdentityVerificationStatus(), params.client.getOwnCrossSigningPublicationStatus()]);
		last = status;
		crossSigningPublished = crossSigning.published;
		if (last.verified && crossSigningPublished) return await params.client.getOwnDeviceVerificationStatus();
		await setTimeout(Math.min(250, Math.max(25, params.timeoutMs - (Date.now() - startedAt))));
	}
...
async function completeMatrixSelfVerification(params) {
	...
	if (!bootstrap.verification.verified) await params.client.trustOwnIdentityAfterSelfVerification?.();
	const ownerVerification = bootstrap.verification.verified && bootstrap.crossSigning.published ? bootstrap.verification : await waitForMatrixSelfVerificationTrustStatus({
...
async function restoreMatrixRoomKeyBackup(opts = {}) {
	return await withStartedActionClient(opts, async (client) => await client.restoreRoomKeyBackup({ recoveryKey: normalizeOptionalString(opts.recoveryKey) }));
}
...
//#endregion
export { scanMatrixVerificationQr as _, confirmMatrixVerificationSas as a, verifyMatrixRecoveryKey as b, getMatrixRoomKeyBackupStatus as c, listMatrixVerifications as d, mismatchMatrixVerificationSas as f, runMatrixSelfVerification as g, restoreMatrixRoomKeyBackup as h, confirmMatrixVerificationReciprocateQr as i, getMatrixVerificationSas as l, resetMatrixRoomKeyBackup as m, bootstrapMatrixVerification as n, generateMatrixVerificationQr as o, requestMatrixVerification as p, cancelMatrixVerification as r, getMatrixEncryptionStatus as s, acceptMatrixVerification as t, getMatrixVerificationStatus as u, startMatrixVerification as v, verification_exports as y };
