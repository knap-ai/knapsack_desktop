import { t as buildFeishuConversationId } from "./conversation-id-DWS3Ep2A.js";
...
import { c as encodeQuery, d as isRecord$3, f as normalizeString, g as normalizeCommentFileType, h as buildFeishuCommentTarget, l as extractReplyText, m as readString$2, p as parseCommentContentElements, s as resolveFeishuRuntimeAccount } from "./accounts-DJRdWYvk.js";
import { a as resolveFeishuReplyPolicy, c as createFeishuCardInteractionEnvelope, l as decodeFeishuCardAction, n as resolveFeishuAllowlistMatch, r as resolveFeishuGroupConfig, s as buildFeishuCardActionTextFallback, t as isFeishuGroupAllowed } from "./policy-FYTUbd61.js";
import { t as getFeishuRuntime } from "./runtime-6ATtgXoM.js";
import { a as getFeishuUserAgent, i as createFeishuWSClient, n as createEventDispatcher, r as createFeishuClient } from "./client-ChfW5mSA.js";
import { i as createCommentTypingReactionLifecycle, t as deliverCommentThreadText } from "./drive-BLrndCtZ.js";
...
import { a as sendCardFeishu, c as sendStructuredCardFeishu, d as extractMentionTargets, f as isMentionForwardRequest, g as normalizeFeishuExternalKey, h as sendMediaFeishu, i as resolveFeishuCardTemplate, l as parsePostContent, m as downloadMessageResourceFeishu, n as getMessageFeishu, p as isFeishuGroupChatType, r as listFeishuThreadMessages, s as sendMessageFeishu, u as buildMentionedCardContent } from "./send-CJm1UGJA.js";
import { i as waitForAbortableDelay, r as raceWithTimeoutAndAbort } from "./probe-Dd4yPYHJ.js";
import { a as feishuWebhookRateLimiter, c as wsClients, i as botOpenIds, l as fetchBotIdentityForMonitor, n as FEISHU_WEBHOOK_MAX_BODY_BYTES, o as httpServers, r as botNames, s as recordWebhookStatus, t as FEISHU_WEBHOOK_BODY_TIMEOUT_MS } from "./monitor.state-DT63phKO.js";
...
function createFeishuReplyDispatcher(params) {
	...
	const closeStreaming = async () => {
		...
		if (streaming?.isActive()) {
			...
			if (streamText) deliveredFinalTexts.add(streamText);
		}
		streaming = null;
		streamingStartPromise = null;
		streamText = "";
...
async function handleFeishuMessage(params) {
	...
	try {
		...
		if (broadcastAgents) {
			...
			const allowReasoningPreview = resolveFeishuReasoningPreviewEnabled({
				storePath: core.channel.session.resolveStorePath(cfg.session?.store, { agentId: route.agentId }),
...
async function resolveDriveCommentEventCore(params) {
	...
	const context = await fetchDriveCommentContext({
		client: createClient ? createClient(account ?? { accountId }) : createFeishuClient((await import("./accounts-DJRdWYvk.js").then((n) => n.t)).resolveFeishuAccount({
			cfg,
			accountId
		})),
		fileToken,
...
function createFeishuMessageReceiveHandler({ cfg, core, accountId, runtime, chatHistories, fireAndForget, handleMessage, resolveDebounceText: resolveText, hasProcessedMessage, recordProcessedMessage, getBotOpenId = () => void 0, getBotName = () => void 0, resolveSequentialKey = ({ accountId, event }) => `feishu:${accountId}:${event.message.chat_id?.trim() || "unknown"}` }) {
	...
	const inboundDebouncer = core.channel.debounce.createInboundDebouncer({
		...
		onFlush: async (entries) => {
			...
			const freshEntries = [];
			for (const entry of dedupedEntries) if (!await hasProcessedMessage(entry.message.message_id, accountId, log)) freshEntries.push(entry);
...
async function resolveReactionSyntheticEvent(params) {
	...
	const { resolveFeishuAccount } = await import("./accounts-DJRdWYvk.js").then((n) => n.t);
...
function parseFeishuCardActionEventPayload(value) {
	...
	if (!isRecord$3(operator) || !isRecord$3(action) || !isRecord$3(context)) return null;
	const token = readString$2(value.token);
	const openId = readString$2(operator.open_id);
	const userId = readString$2(operator.user_id);
	const unionId = readString$2(operator.union_id);
	const tag = readString$2(action.tag);
	const actionValue = action.value;
	const contextOpenId = readString$2(context.open_id);
	const contextUserId = readString$2(context.user_id);
	const chatId = readString$2(context.chat_id);
	if (!token || !openId || !userId || !unionId || !tag || !isRecord$3(actionValue) || !contextOpenId || !contextUserId || !chatId) return null;
	return {
		operator: {
			open_id: openId,
			user_id: userId,
