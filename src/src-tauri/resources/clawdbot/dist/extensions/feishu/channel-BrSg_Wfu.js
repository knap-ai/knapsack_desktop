...
import { i as resolveFeishuGroupToolPolicy } from "./policy-FYTUbd61.js";
	...
	status: {
		...
		resolveStatusLines: async ({ cfg, configured }) => {
			...
			if (configured && resolvedCredentials) try {
				const { probeFeishu } = await import("./probe-Dd4yPYHJ.js").then((n) => n.n);
...
const loadFeishuChannelRuntime = createLazyRuntimeNamedExport(() => import("./channel.runtime-En-ed7EY.js"), "feishuChannelRuntime");
...
async function createFeishuActionClient(account) {
	const { createFeishuClient } = await import("./client-ChfW5mSA.js").then((n) => n.t);
...
const feishuPlugin = createChatChannelPlugin({
	base: {
		...
		gateway: { startAccount: async (ctx) => {
			const { monitorFeishuProvider } = await import("./monitor-C_w8R_0D.js");
