import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString, r as lowercasePreservingWhitespace } from "./string-coerce-C1IzJjqi.js";
import { o as normalizeSingleOrTrimmedStringList } from "./string-normalization-Bvcn03I9.js";
import { p as resolveSessionAgentId, x as resolveDefaultAgentId } from "./agent-scope-_6dFncNS.js";
import { c as listActiveMemoryPublicArtifacts } from "./memory-state-I5DhT9-V.js";
...
import { n as getActiveMemorySearchManager } from "./memory-host-search-CgQXJKch.js";
import { d as resolveMemoryWikiConfig, o as WIKI_SEARCH_BACKENDS, s as WIKI_SEARCH_CORPORA } from "./config-CJXHjDBM.js";
...
function normalizeWikiClaimEvidence(value) {
	...
	if (!sourceId && !evidencePath && !lines && !note && weight === void 0 && !updatedAt) return null;
	return {
		...sourceId ? { sourceId } : {},
...
async function pathExists$3(inputPath) {
	try {
...
async function initializeMemoryWikiVault(config, options) {
	...
	const createdFiles = [];
	if (!await pathExists$3(rootDir)) createdDirectories.push(rootDir);
	...
	for (const relativeDir of WIKI_VAULT_DIRECTORIES) {
		...
		if (!await pathExists$3(fullPath)) createdDirectories.push(fullPath);
...
function buildRelatedBlockBody(params) {
	...
	if (backlinks.length > 0) sections.push("### Referenced By", renderWikiPageLinks({
		config: params.config,
		pages: backlinks
	}));
...
function buildAgentDigest(params) {
	const pages = [...params.pages].toSorted((left, right) => left.relativePath.localeCompare(right.relativePath)).map((page) => {
		...
		}, typeof page.confidence === "number" ? { confidence: page.confidence } : {}, { freshnessLevel: pageFreshness.level }, pageFreshness.lastTouchedAt ? { lastTouchedAt: pageFreshness.lastTouchedAt } : {}, {
...
function buildSnippet(raw, query) {
	...
	return raw.split(/\r?\n/).find((line) => normalizeLowercaseStringOrEmpty(line).includes(queryLower) && line.trim().length > 0)?.trim() || raw.split(/\r?\n/).find((line) => line.trim().length > 0)?.trim() || "";
}
...
function isClaimTextOrIdMatch(claim, queryLower) {
	if (normalizeLowercaseStringOrEmpty(claim.text).includes(queryLower)) return true;
...
function buildDigestCandidatePaths(params) {
	...
	return params.digest.pages.map((page) => {
		...
		if (!normalizeLowercaseStringOrEmpty(buildDigestPageSearchText(page, claims)).includes(queryLower)) return {
			path: page.path,
			score: 0
		};
		...
		const matchingClaims = claims.filter((claim) => isClaimTextOrIdMatch(claim, queryLower)).toSorted((left, right) => scoreDigestClaimMatch(right, queryLower) - scoreDigestClaimMatch(left, queryLower));
...
function isClaimMatch(claim, queryLower) {
	return isClaimTextOrIdMatch(claim, queryLower);
}
function rankClaimMatch(page, claim, queryLower) {
	const freshness = assessClaimFreshness({
		page,
		claim
	});
...
function getMatchingClaims(page, queryLower) {
	return page.claims.filter((claim) => isClaimMatch(claim, queryLower)).toSorted((left, right) => rankClaimMatch(page, right, queryLower) - rankClaimMatch(page, left, queryLower));
}
...
function scorePage(page, query) {
	...
	if (!(titleLower.includes(queryLower) || pathLower.includes(queryLower) || idLower.includes(queryLower) || metadataLower.includes(queryLower) || rawLower.includes(queryLower))) return 0;
	let score = 1 + scoreWikiMetadataMatch({
		title: page.title,
		path: page.relativePath,
		id: page.id,
...
function toWikiSearchResult(page, query) {
	return {
		corpus: "wiki",
		path: page.relativePath,
		title: page.title,
...
function toMemoryWikiSearchResult(result) {
	return {
		corpus: "memory",
		path: result.path,
...
async function searchWikiCorpus(params) {
	...
	const results = candidatePages.map((page) => toWikiSearchResult(page, params.query)).filter((page) => page.score > 0);
	...
	return [...results, ...remainingPages.map((page) => toWikiSearchResult(page, params.query)).filter((page) => page.score > 0)];
}
...
async function searchMemoryWiki(params) {
	...
	const memoryResults = sharedMemoryManager ? (await sharedMemoryManager.search(params.query, { maxResults })).map((result) => toMemoryWikiSearchResult(result)) : [];
...
function pathExists$2(filePath) {
...
async function ingestMemoryWikiSource(params) {
	...
	const created = !await pathExists$2(pagePath);
...
async function writeImportedSourcePage(params) {
	...
	const created = !await pathExists$1(pageAbsPath);
	...
	const existing = await fs$1.readFile(pageAbsPath, "utf8").catch(() => "");
	if (existing !== rendered) await fs$1.writeFile(pageAbsPath, rendered, "utf8");
...
async function runWikiStatus(params) {
	...
	const status = await resolveMemoryWikiStatus(params.config, { appConfig: params.appConfig });
	writeOutput(params.json ? JSON.stringify(status, null, 2) : renderMemoryWikiStatus(status), params.stdout);
	return status;
}
async function runWikiDoctor(params) {
	...
	const report = buildMemoryWikiDoctorReport(await resolveMemoryWikiStatus(params.config, { appConfig: params.appConfig }));
	if (!report.healthy) process.exitCode = 1;
	writeOutput(params.json ? JSON.stringify(report, null, 2) : renderMemoryWikiDoctor(report), params.stdout);
	return report;
}
...
async function runWikiBridgeImport(params) {
	return runWikiCommandWithSummary({
		...
		render: (value) => `Bridge import synced ${value.artifactCount} artifacts across ${value.workspaces} workspaces (${value.importedCount} new, ${value.updatedCount} updated, ${value.skippedCount} unchanged, ${value.removedCount} removed). Indexes ${value.indexesRefreshed ? `refreshed (${value.indexUpdatedFiles.length} files)` : `not refreshed (${value.indexRefreshReason})`}.`
	});
}
...
function registerWikiCli(program, pluginConfig, appConfig) {
	...
	addWikiSearchConfigOptions(wiki.command("search").description("Search wiki pages and, when configured, the active memory corpus").argument("<query>", "Search query").option("--max-results <n>", "Maximum results", (value) => Number(value))).option("--json", "Print JSON").action(async (query, opts) => {
		await runWikiSearch({
			config,
			appConfig,
			query,
