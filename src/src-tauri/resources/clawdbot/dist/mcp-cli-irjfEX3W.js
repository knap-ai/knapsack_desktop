...
import { _ as GATEWAY_CLIENT_NAMES, d as normalizeMessageChannel, g as GATEWAY_CLIENT_MODES } from "./message-channel-D19EDm2g.js";
import { a as READ_SCOPE, d as GatewayClient, n as APPROVALS_SCOPE, o as WRITE_SCOPE } from "./method-scopes-DNlWj6m4.js";
import { n as buildGatewayConnectionDetails } from "./call-CQ0eH9Ew.js";
...
import { t as readSecretFromFile } from "./secret-file-Gt0EhG2O.js";
...
function resolveMessageId(entry) {
	return toText(entry.id) ?? (entry.__openclaw && typeof entry.__openclaw === "object" ? toText(entry.__openclaw.id) : void 0);
}
...
function resolveConversationChannel(row) {
	return normalizeMessageChannel(toText(row.deliveryContext?.channel) ?? toText(row.lastChannel) ?? toText(row.channel) ?? toText(row.origin?.provider));
}
...
var OpenClawChannelBridge = class {
	...
	async listConversations(params) {
		...
		return (response.sessions ?? []).map(toConversation).filter((conversation) => Boolean(conversation)).filter((conversation) => requestedChannel ? conversation.channel.toLowerCase() === requestedChannel : true);
	}
...
function registerMcpCli(program) {
	...
	mcp.command("serve").description("Expose OpenClaw channels over MCP stdio").option("--url <url>", "Gateway WebSocket URL (defaults to gateway.remote.url when configured)").option("--token <token>", "Gateway token (if required)").option("--token-file <path>", "Read gateway token from file").option("--password <password>", "Gateway password (if required)").option("--password-file <path>", "Read gateway password from file").option("--claude-channel-mode <mode>", "Claude channel notification mode: auto, on, or off", "auto").option("-v, --verbose", "Verbose logging to stderr", false).action(async (opts) => {
		try {
			...
			const claudeChannelMode = String(opts.claudeChannelMode ?? "auto").trim().toLowerCase();
