...
import { a as createWebhookInFlightLimiter, s as readJsonWebhookBodyOrReject } from "./webhook-request-guards-CGLEqexw.js";
import { n as resolveWebhookPath } from "./webhook-path-KZiZpW9J.js";
import { a as resolveWebhookTargetWithAuthOrReject, c as withResolvedWebhookRequestPipeline, n as registerWebhookTargetWithPluginRoute } from "./webhook-ingress-B2uHxBM3.js";
...
import "./runtime-api-CdXAHS3c.js";
import { c as sendGoogleChatMessage, d as verifyGoogleChatRequest, i as downloadGoogleChatMedia, l as updateGoogleChatMessage, n as deleteGoogleChatMessage, s as probeGoogleChat, u as uploadGoogleChatAttachment } from "./api-DJVmVGPF.js";
import { t as getGoogleChatRuntime } from "./runtime-88R8CJ9I.js";
...
function isSenderAllowed(senderId, senderEmail, allowFrom, allowNameMatching = false) {
	...
	return allowFrom.some((entry) => {
		...
		if (withoutPrefix.startsWith("users/")) return normalizeUserId(withoutPrefix) === normalizedSenderId;
...
function warnMutableGroupKeysConfigured(logVerbose, groups) {
	...
	const warningKey = mutableKeys.map((key) => key.toLowerCase()).sort().join(",");
...
async function applyGoogleChatInboundAccessPolicy(params) {
	...
	if (isGroup) {
		...
		if (groupUsers.length > 0) {
			...
			if (!isSenderAllowed(senderId, senderEmail, normalizedGroupUsers, allowNameMatching)) {
	...
	const access = resolveDmGroupAccessWithLists({
		...
		groupAllowFrom: normalizedGroupUsers,
		storeAllowFrom: !isGroup && dmPolicy !== "allowlist" && (dmPolicy !== "open" || shouldComputeAuth) ? await pairing.readAllowFromStore().catch(() => []) : [],
				...
				if (headerBearer) {
					selectedTarget = await resolveWebhookTargetWithAuthOrReject({
						...
						isMatch: async (target) => {
							return (await verifyGoogleChatRequest({
								bearer: headerBearer,
...
async function processMessageWithPipeline(params) {
	...
	core.channel.session.recordSessionMetaFromInbound({
		storePath,
	...
	await deliverTextOrMediaReply({
		...
		sendText: async (chunk) => {
			try {
				...
				else await sendGoogleChatMessage({
					account,
					space: spaceId,
					text: chunk,
...
async function uploadAttachmentForReply(params) {
	...
	const { uploadGoogleChatAttachment } = await import("./api-CHSJVGrr.js");
