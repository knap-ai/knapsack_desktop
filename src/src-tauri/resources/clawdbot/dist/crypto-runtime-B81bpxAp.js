import { n as LogService, r as noop } from "./logger-a6JNXTRe.js";
import { n as isRepairableSecretStorageAccessError } from "./recovery-key-store-26SPaa6M.js";
...
async function loadMatrixCryptoNodeRuntime() {
	matrixCryptoNodeRuntimePromise ??= import("./crypto-node.runtime-Ccf2p1OS.js");
...
async function restoreIdbFromDisk(snapshotPath) {
	...
	for (const resolvedPath of candidatePaths) try {
		const snapshot = parseSnapshotPayload(fsSync.readFileSync(resolvedPath, "utf8"));
...
async function persistIdbToDisk(params) {
	...
	try {
		...
		fsSync.writeFileSync(snapshotPath, JSON.stringify(snapshot));
		fsSync.chmodSync(snapshotPath, 384);
...
//#endregion
export { MatrixCryptoBootstrapper, MatrixDecryptBridge, MatrixVerificationManager, createMatrixCryptoFacade, isMatrixDeviceOwnerVerified, persistIdbToDisk, restoreIdbFromDisk };
