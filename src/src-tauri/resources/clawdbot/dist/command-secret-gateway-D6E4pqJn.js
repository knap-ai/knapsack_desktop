import { d as resolveSecretInputRef } from "./types.secrets-DTXJhyWe.js";
import { t as describeUnknownError } from "./shared-CkQ4yE36.js";
import { a as resolveSecretRefValue } from "./resolve-eBylNaNU.js";
import { _ as GATEWAY_CLIENT_NAMES, g as GATEWAY_CLIENT_MODES } from "./message-channel-D19EDm2g.js";
import { n as resolveBundledWebFetchPluginId } from "./plugin-auto-enable-CqpAn9Qh.js";
import { wt as validateSecretsResolveResult } from "./method-scopes-DNlWj6m4.js";
import { r as callGateway } from "./call-CQ0eH9Ew.js";
import { d as getPath, p as setPathExistingStrict, r as discoverConfigSecretTargetsByIds } from "./target-registry-CO65L87H.js";
import { c as createResolverContext, d as analyzeCommandSecretAssignmentsFromSnapshot, i as collectConfigAssignments, p as assertExpectedResolvedSecretValue, t as resolveRuntimeWebTools } from "./runtime-web-tools-Bbt6e7oI.js";
import { c as resolveBundledWebSearchPluginId } from "./web-search-providers.shared-CiXeJJJg.js";
...
function classifyRuntimeWebTargetPathState(params) {
	...
	if (pluginId) {
		if (params.path.endsWith(".config.webFetch.apiKey")) {
			...
			const configuredProvider = typeof fetch?.provider === "string" ? fetch.provider.trim().toLowerCase() : "";
			if (!configuredProvider) return "active";
			return resolveBundledWebFetchPluginId(configuredProvider) === pluginId ? "active" : "inactive";
		}
		const search = params.config.tools?.web?.search;
		if (search?.enabled === false) return "inactive";
		const configuredProvider = typeof search?.provider === "string" ? search.provider.trim().toLowerCase() : "";
		if (!configuredProvider) return "active";
...
function describeInactiveRuntimeWebTargetPath(params) {
	...
	if (pluginId) {
		...
		const configuredProvider = typeof search?.provider === "string" ? search.provider.trim().toLowerCase() : "";
		const configuredPluginId = configuredProvider ? resolveBundledWebSearchPluginId(configuredProvider) : void 0;
...
async function resolveCommandSecretRefsLocally(params) {
	...
	const discoveredTargets = discoverConfigSecretTargetsByIds(sourceConfig, params.targetIds).filter((target) => !params.allowedPaths || params.allowedPaths.has(target.path));
	...
	if (targetsRuntimeWebResolution({
		...
		localResolutionDiagnostics.push(`${params.commandName}: failed to resolve web tool secrets locally (${describeUnknownError(error)}).`);
	}
...
async function resolveTargetSecretLocally(params) {
	...
	try {
		...
		if (!enforcesResolvedSecrets(params.mode)) params.localResolutionDiagnostics.push(`${params.commandName}: failed to resolve ${params.target.path} locally (${describeUnknownError(error)}).`);
	}
}
