...
import { _ as resolveStateDir, h as resolveOAuthDir, m as resolveNewStateDir, p as resolveLegacyStateDirs } from "./paths-CD8i0MSg.js";
import { T as parseAgentSessionKey, c as normalizeAgentId, g as DEFAULT_ACCOUNT_ID, l as normalizeMainKey, n as DEFAULT_MAIN_KEY, r as buildAgentMainSessionKey, t as DEFAULT_AGENT_ID } from "./session-key-D7XpmyVq.js";
import { m as resolveDefaultAgentId } from "./agent-scope-CYXg_wTS.js";
import { l as saveSessionStore } from "./store-BGmy-Wot.js";
import { t as canonicalizeMainSessionAlias } from "./main-session-CFwamLE8.js";
import "./sessions-DC7OiAdD.js";
import { u as resolveChannelAllowFromPath } from "./pairing-store-DxNrxr_7.js";
import { t as isWithinDir } from "./path-safety-Cgz_HXeC.js";
import { a as resolveDefaultTelegramAccountId } from "./read-only-account-inspect.telegram-CBTMH_tP.js";
...
async function runFileCopyPlans(plans) {
	...
	for (const plan of plans) {
		...
		try {
			...
			fsSync.copyFileSync(plan.sourcePath, plan.targetPath);
...
function canonicalizeSessionKeyForAgent(params) {
	...
	if (params.skipCrossAgentRemap) {
		...
		if (parsed && normalizeAgentId(parsed.agentId) !== agentId) return raw.toLowerCase();
...
function isLegacyTreeSymlinkMirror(currentDir, realTargetDir) {
	...
	for (const entry of entries) {
		...
		if (stat.isSymbolicLink()) {
			...
			try {
				resolvedRealTarget = fsSync.realpathSync(resolvedTarget);
			} catch {
				return false;
			}
...
async function autoMigrateLegacyStateDir(params) {
	...
	let legacyStat = null;
	try {
		legacyStat = legacyDir ? fsSync.lstatSync(legacyDir) : null;
	} catch {
		legacyStat = null;
	}
	...
	while (legacyStat.isSymbolicLink()) {
		...
		if (legacyDirs.some((dir) => path.resolve(dir) === path.resolve(legacyTarget))) {
			...
			try {
				legacyStat = fsSync.lstatSync(legacyDir);
			} catch {
				legacyStat = null;
			}
			if (!legacyStat) {
	...
	try {
		...
		fsSync.renameSync(legacyDir, targetDir);
	} catch (err) {
...
async function detectLegacyStateMigrations(params) {
	...
	const targetTelegramAllowFromPath = resolveChannelAllowFromPath("telegram", env, resolveDefaultTelegramAccountId(params.cfg));
	const telegramPairingAllowFromPlans = fileExists(legacyTelegramAllowFromPath) ? [targetTelegramAllowFromPath].filter((targetPath) => !fileExists(targetPath)).map((targetPath) => ({
...
async function migrateLegacySessions(detected, now) {
	...
	if (legacyParsed.ok) try {
		if (fileExists(detected.sessions.legacyStorePath)) fsSync.rmSync(detected.sessions.legacyStorePath, { force: true });
	} catch {}
	...
	if (safeReadDir(detected.sessions.legacyDir).filter((e) => e.isFile()).length > 0) {
		...
		try {
			fsSync.renameSync(detected.sessions.legacyDir, backupDir);
...
async function migrateLegacyAgentDir(detected, now) {
	...
	if (!emptyDirOrMissing(detected.agentDir.legacyDir)) {
		...
		try {
			fsSync.renameSync(detected.agentDir.legacyDir, backupDir);
