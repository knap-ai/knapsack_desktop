import { c as isPathInside, s as isNotFoundPathError, t as sameFileIdentity } from "./file-identity-B8OnIgGC.js";
import { o as openFileWithinRoot, r as copyFileWithinRoot, s as openWritableFileWithinRoot, t as SafeOpenError } from "./fs-safe-Dv_jH6UN.js";
import { n as resolveSafeBaseDir } from "./path-safety-Cgz_HXeC.js";
...
async function applyStagedEntryMode(params) {
	...
	if (params.mode !== 0) await fs.chmod(destinationPath, params.mode).catch(() => void 0);
}
...
async function writeZipFileEntry(params) {
	...
	try {
		...
		tempHandle = await fs.open(tempPath, OPEN_WRITE_CREATE_FLAGS, targetMode || 438);
		...
		if (typeof params.entry.unixPermissions === "number") {
			...
			if (mode !== 0) await fs.chmod(verifiedPath, mode).catch(() => void 0);
		}
	} catch (err) {
		if (tempPath) await fs.rm(tempPath, { force: true }).catch(() => void 0);
...
//#endregion
export { resolveArchiveKind as a, prepareArchiveDestinationDir as c, readJsonFile as i, withStagedArchiveDestination as l, extractArchive as n, resolvePackedRootDir as o, fileExists as r, mergeExtractedTreeIntoDestination as s, createTarEntryPreflightChecker as t, isWindowsDrivePath as u };
