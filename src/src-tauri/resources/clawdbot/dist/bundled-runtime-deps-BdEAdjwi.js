...
import { r as resolveHomeRelativePath } from "./home-dir-CfZ08GnU.js";
...
function shouldRemoveRuntimeDepsLock(owner, nowMs, isAlive = isProcessAlive) {
	...
	return typeof owner.createdAtMs === "number" && nowMs - owner.createdAtMs > BUNDLED_RUNTIME_DEPS_LOCK_STALE_MS;
}
...
function withBundledRuntimeDepsInstallRootLock(installRoot, run) {
	...
	while (!locked) try {
		...
		if (Date.now() - startedAt > BUNDLED_RUNTIME_DEPS_LOCK_TIMEOUT_MS) throw new Error(`Timed out waiting for bundled runtime deps lock at ${lockDir}`, { cause: error });
...
function ensureBundledPluginRuntimeDeps(params) {
	...
	return withBundledRuntimeDepsInstallRootLock(installRoot, () => {
		...
		const installExecutionRoot = (cacheDir && isPluginRootInstall && resolveSourceCheckoutBundledPluginPackageRoot(params.pluginRoot) ? cacheDir : void 0) ?? (isPluginRootInstall ? path.join(installRoot, PLUGIN_ROOT_INSTALL_STAGE_DIR) : void 0);
...
//#endregion
export { resolveBundledRuntimeDependencyInstallRoot as a, scanBundledPluginRuntimeDeps as c, repairBundledRuntimeDepsInstallRoot as i, installBundledRuntimeDeps as n, resolveBundledRuntimeDependencyPackageInstallRoot as o, registerBundledRuntimeDependencyNodePath as r, resolveBundledRuntimeDependencyPackageRoot as s, ensureBundledPluginRuntimeDeps as t };
