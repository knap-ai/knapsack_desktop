...
import { t as DEFAULT_AGENT_ID } from "./session-key-DAhnzjyr.js";
import { H as normalizeTrustedSafeBinDirs, W as listRiskyConfiguredSafeBins } from "./io-y3Az_Onx.js";
import { a as hasConfiguredSecretInput } from "./types.secrets-DgX397vI.js";
import { n as formatPermissionRemediation, r as inspectPathPermissions, t as formatPermissionDetail } from "./audit-fs-DTQIjp2r.js";
...
import { l as isPrivateNetworkAllowedByPolicy, s as isBlockedHostnameOrIp } from "./ssrf-0bPJMoZR.js";
import { i as loadExecApprovals } from "./exec-approvals-BF_Qfdq8.js";
import { n as resolveBrowserConfig, r as resolveProfile } from "./config-CLRQPUSt.js";
import { et as resolveBrowserControlAuth, f as redactCdpUrl } from "./routes-XT2JfvT-.js";
import { a as listInterpreterLikeSafeBins, r as isInterpreterLikeAllowlistPattern, s as resolveMergedSafeBinProfileFixtures } from "./exec-inline-eval-CpmxWJsW.js";
import { n as hasPotentialConfiguredChannels } from "./config-presence-DA3j8Nmi.js";
import { n as DEFAULT_GATEWAY_HTTP_TOOL_DENY } from "./dangerous-tools-DPr4YFtb.js";
import { t as collectEnabledInsecureOrDangerousFlags } from "./dangerous-config-flags-DRxyeRNu.js";
...
async function loadChannelPlugins() {
	channelPluginsModulePromise ??= import("./plugins-BStRM4Qp.js");
	return await channelPluginsModulePromise;
}
async function loadAuditNonDeepModule() {
	auditNonDeepModulePromise ??= import("./audit.nondeep.runtime-DkuMHjJs.js");
	return await auditNonDeepModulePromise;
}
async function loadAuditDeepModule() {
	auditDeepModulePromise ??= import("./audit.deep.runtime-RHmYS7tw.js");
	return await auditDeepModulePromise;
}
async function loadAuditChannelModule() {
	auditChannelModulePromise ??= import("./audit-channel.collect.runtime-BXNlK80Q.js");
	return await auditChannelModulePromise;
}
async function loadGatewayProbeDeps() {
	gatewayProbeDepsPromise ??= Promise.all([
		import("./call-CPmFyzOB.js"),
		import("./probe-auth-zI28oAru.js"),
		import("./probe-DphlbdkL.js")
...
function collectBrowserControlFindings(cfg, env) {
	...
	const tokenConfigured = Boolean(browserAuth.token) || hasNonEmptyString(env.OPENCLAW_GATEWAY_TOKEN) || hasConfiguredSecretInput(cfg.gateway?.auth?.token, cfg.secrets?.defaults);
	...
	const passwordConfigured = Boolean(browserAuth.password) || passwordCanWin && (hasNonEmptyString(env.OPENCLAW_GATEWAY_PASSWORD) || hasConfiguredSecretInput(cfg.gateway?.auth?.password, cfg.secrets?.defaults));
...
function collectOpenExecSurfacePaths(cfg) {
	...
	const visit = (value, scope) => {
		...
		for (const [key, nested] of Object.entries(record)) {
			...
			if (asRecord(nested)) visit(nested, `${scope}.${key}`);
		}
	};
