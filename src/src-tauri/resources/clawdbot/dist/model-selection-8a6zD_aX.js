...
import { m as resolveThinkingDefaultForModel } from "./thinking.shared-B8IAgSi8.js";
...
import { i as resolveAgentConfig, o as resolveAgentEffectiveModelPrimary, u as resolveAgentModelFallbacksOverride } from "./agent-scope-CYXg_wTS.js";
import { t as resolveConfiguredProviderFallback } from "./configured-provider-fallback-Dcx3exfu.js";
import { createRequire } from "node:module";
//#region src/plugin-sdk/google-model-id.ts
...
//#endregion
//#region src/plugin-sdk/xai-model-id.ts
...
function inferUniqueProviderFromConfiguredModels(params) {
	...
	if (configuredProviders) for (const [providerId, providerConfig] of Object.entries(configuredProviders)) {
		...
		for (const entry of models) {
			...
			if (!modelId) continue;
			if (modelId === model || modelId.toLowerCase() === normalized) addProvider(providerId);
		}
		if (providers.size > 1) return;
	}
	if (providers.size !== 1) return;
