...
function resolveThinkingPolicyContext(params) {
	...
	return {
		normalizedProvider,
		modelId,
		modelKey: normalizeOptionalLowercaseString(params.model) ?? "",
		reasoning: (params.catalog?.find((entry) => normalizeProviderId(entry.provider) === normalizedProvider && entry.id === modelId))?.reasoning
	};
}
...
function resolveThinkingProfile(params) {
	...
	}) === true ? buildBinaryThinkingProfile(defaultLevel) : buildBaseThinkingProfile(defaultLevel);
	const policyContext = {
		provider: context.normalizedProvider,
...
function listThinkingLevelOptions(provider, model) {
	return resolveThinkingProfile({
		provider,
		model
	}).levels.map(({ id, label }) => ({
...
//#endregion
export { listThinkingLevels as a, supportsXHighThinking as c, normalizeReasoningLevel as d, normalizeThinkLevel as f, resolveResponseUsageMode as g, normalizeVerboseLevel as h, listThinkingLevelOptions as i, formatXHighModelHint as l, normalizeUsageDisplay as m, isThinkingLevelSupported as n, resolveSupportedThinkingLevel as o, normalizeTraceLevel as p, listThinkingLevelLabels as r, resolveThinkingDefaultForModel as s, formatThinkingLevels as t, normalizeElevatedLevel as u };
