...
import { n as pickPrimaryTailnetIPv4, r as pickPrimaryTailnetIPv6 } from "./tailnet-BFTC0Dnp.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-BjlagKDo.js";
import { n as resolveWideAreaDiscoveryDomain, t as getWideAreaZonePath } from "./widearea-dns-B06zzK_-.js";
...
function zoneFileNeedsBootstrap(zonePath) {
	if (!fsSync.existsSync(zonePath)) return true;
	try {
		const content = fsSync.readFileSync(zonePath, "utf-8");
...
function ensureImportLine(corefilePath, importGlob) {
	const existing = fsSync.readFileSync(corefilePath, "utf-8");
...
function registerDnsCli(program) {
	program.command("dns").description("DNS helpers for wide-area discovery (Tailscale + CoreDNS)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.openclaw.ai/cli/dns")}\n`).command("setup").description("Set up CoreDNS to serve your discovery domain for unicast DNS-SD (Wide-Area Bonjour)").option("--domain <domain>", "Wide-area discovery domain (e.g. openclaw.internal)").option("--apply", "Install/update CoreDNS config and (re)start the service (requires sudo)", false).action(async (opts) => {
		...
		if (!fsSync.existsSync(corefilePath)) writeFileSudoIfNeeded(corefilePath, `import ${importGlob}\n`);
		...
		await fsSync.promises.mkdir(path.dirname(zonePath), { recursive: true });
		if (zoneFileNeedsBootstrap(zonePath)) {
			...
			fsSync.writeFileSync(zonePath, zoneLines.join("\n"), "utf-8");
		}
		defaultRuntime.log("");
