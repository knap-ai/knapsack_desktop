...
import { i as parseDotPath } from "./shared-BkZJwMus.js";
import { t as loadPluginManifestRegistry } from "./manifest-registry-pYrbrdHJ.js";
import { n as getPath } from "./path-utils-BMwQl5-A.js";
...
function listBundledWebProviderSecretTargetRegistryEntries() {
	...
	for (const record of loadPluginManifestRegistry({}).plugins) {
		...
		for (const config of WEB_PROVIDER_SECRET_CONFIGS) if (hasWebProviderContract(record, config.contract) && hasSensitiveConfigHint(record, config.configPath)) entries.push(createPluginOpenClawConfigSecretTargetEntry(record.id, config.configPath));
	}
...
const compiledBundledChannelOpenClawTargets = /* @__PURE__ */ new Map();
...
function getCompiledBundledChannelOpenClawTargets(channelId) {
	...
	if (compiledBundledChannelOpenClawTargets.has(normalizedChannelId)) return compiledBundledChannelOpenClawTargets.get(normalizedChannelId) ?? null;
	const compiledEntries = loadBundledChannelSecretContractApi(normalizedChannelId)?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "openclaw.json").map(compileTargetRegistryEntry) ?? null;
...
function resolveConfigSecretTargetByPath(pathSegments) {
	...
	const explicitBundledChannelEntries = explicitBundledChannelId ? getCompiledBundledChannelOpenClawTargets(explicitBundledChannelId) : null;
