...
import { i as withFileLock$1 } from "./file-lock-B6EjeH4S.js";
...
import { C as formatUnknownError, S as formatMSTeamsSendErrorHint, _ as loadMSTeamsSdkWithAuth, f as resolveMSTeamsCredentials, g as createMSTeamsTokenProvider, h as createMSTeamsAdapter, p as readAccessToken, v as buildUserAgent, w as isRevokedProxyError, x as classifyMSTeamsSendError, y as getMSTeamsRuntime } from "./graph-users-F-Pl04ex.js";
...
const STORE_FILENAME$1 = "msteams-conversations.json";
const MAX_CONVERSATIONS = 1e3;
const CONVERSATION_TTL_MS = 365 * 24 * 60 * 60 * 1e3;
function pruneToLimit$1(conversations) {
...
function pruneExpired$1(conversations, nowMs, ttlMs) {
	let removed = false;
	const kept = {};
...
function createMSTeamsConversationStoreFs(params) {
	...
	const readStore = async () => {
		...
		const pruned = pruneExpired$1(value.conversations, nowMs, ttlMs).conversations;
		return {
			version: 1,
			conversations: pruneToLimit$1(pruned)
		};
	};
	const list = async () => {
	...
	const upsert = async (conversationId, reference) => {
		...
		await withFileLock(filePath, empty, async () => {
			...
			const nowMs = Date.now();
			store.conversations = pruneExpired$1(store.conversations, nowMs, ttlMs).conversations;
			store.conversations = pruneToLimit$1(store.conversations);
...
function createMSTeamsPollStoreFs(params) {
	...
	const readStore = async () => {
		...
		return {
			version: 1,
			polls: pruneToLimit(pruneExpired(value.polls ?? {}))
		};
	};
...
async function resolveGraphChatId(params) {
	...
	const res = await fetchFn(`${GRAPH_ROOT}${path}`, { headers: { Authorization: `Bearer ${token}` } });
...
async function sendMSTeamsMessages(params) {
	...
	const sendMessageInContext = async (ctx, message, messageIndex) => {
		return extractMessageId(await sendWithRetry(async () => await ctx.sendActivity(await buildActivity(message, params.conversationRef, params.tokenProvider, params.sharePointSiteId, params.mediaMaxBytes, { feedbackLoopEnabled: params.feedbackLoopEnabled })), {
			messageIndex,
...
//#endregion
export { createMSTeamsPollStoreFs as _, sendMessageMSTeams as a, renderReplyPayloadsToMessages as c, withRevokedProxyFallback as d, getPendingUpload as f, uploadToConsentUrl as g, parseFileConsentInvoke as h, sendAdaptiveCardMSTeams as i, sendMSTeamsMessages as l, buildFileInfoCard as m, deleteMessageMSTeams as n, sendPollMSTeams as o, removePendingUpload as p, editMessageMSTeams as r, buildConversationReference as s, probeMSTeams as t, AI_GENERATED_ENTITY as u, extractMSTeamsPollVote as v, createMSTeamsConversationStoreFs as y };
