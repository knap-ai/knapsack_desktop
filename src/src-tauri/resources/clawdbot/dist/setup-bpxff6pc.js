...
import { u as readConfigFileSnapshot, y as writeConfigFile } from "./io-Dv_xNAZB.js";
import "./config-yDDhhyz6.js";
import { t as WizardCancelledError } from "./prompts-ocoL7LgP.js";
import { c as formatPluginCompatibilityNotice, n as buildPluginCompatibilityNotices } from "./status-Clo7fOEI.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-Du2vAKZ4.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CiwTRtkB.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-kvRDRGKe.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-q4BLejEP.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-T6o6jvuQ.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DyprGcZh.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-ukbTWhuc.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-ukbTWhuc.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-DLTfs3jx.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-DG_7AncM.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-nWyjtfJ-.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-D8Ppm3zt.js");
		const { setupChannels } = await import("./onboard-channels-DBwmUkyk.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-CDl82lgL.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CkWqHe-R.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-CTylBMEW.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-CKH7RU1-.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-CL-BPA9d.js");
