...
import { D as resolveSiteName, F as normalizeCacheKey, I as readCache, L as readResponseText, R as resolveCacheTtlMs, T as resolveSearchCount, V as writeCache, a as getScopedCredentialValue, k as withTrustedWebSearchEndpoint, n as enablePluginInConfig, u as setScopedCredentialValue, z as resolveTimeoutSeconds } from "./provider-web-search-COZNygbd.js";
...
function decodeHtmlEntities(text) {
	return text.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, "\"").replace(/&apos;/g, "'").replace(/&#39;/g, "'").replace(/&#x27;/g, "'").replace(/&#x2F;/g, "/").replace(/&nbsp;/g, " ").replace(/&ndash;/g, "-").replace(/&mdash;/g, "--").replace(/&hellip;/g, "...").replace(/&#(\d+);/g, (_, code) => String.fromCodePoint(Number(code))).replace(/&#x([0-9a-f]+);/gi, (_, code) => String.fromCodePoint(parseInt(code, 16)));
}
...
const DuckDuckGoSearchSchema = Type.Object({
	...
	region: Type.Optional(Type.String({ description: "Optional DuckDuckGo region code such as us-en, uk-en, or de-de." })),
	safeSearch: Type.Optional(Type.String({ description: "SafeSearch level: strict, moderate, or off." }))
...
function createDuckDuckGoWebSearchProvider() {
	return {
		...
		setCredentialValue: (searchConfigTarget, value) => setScopedCredentialValue(searchConfigTarget, "duckduckgo", value),
		...
		createTool: (ctx) => ({
			...
			execute: async (args) => await runDuckDuckGoSearch({
				config: ctx.config,
...
//#endregion
export { duckduckgo_default as t };
