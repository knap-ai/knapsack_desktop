...
import { c as randomIdempotencyKey } from "./call-CQ0eH9Ew.js";
import { i as parsePairingList, r as parseNodeList } from "./node-resolve-CbG10E1V.js";
import { a as parseCanvasSnapshotPayload, c as parseCameraSnapPayload, d as writeCameraPayloadToFile, i as canvasSnapshotTempPath, l as writeBase64ToFile, n as screenRecordTempPath, o as cameraTempPath, r as writeScreenRecordToFile, s as parseCameraClipPayload, t as parseScreenRecordPayload, u as writeCameraClipPayloadToFile } from "./nodes-screen-CGrdr0Pv.js";
import { t as parseTimeoutMs } from "./parse-timeout-DBU4bJ5s.js";
...
import { a as resolveNodeId, i as resolveNode, n as callGatewayCli, o as unauthorizedHintForMessage, r as nodesCallOpts, s as formatPermissions, t as buildNodeInvokeParams } from "./rpc-DGeAaoOQ.js";
...
function registerNodesInvokeCommands(nodes) {
	nodesCallOpts(nodes.command("invoke").description("Invoke a command on a paired node").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").requiredOption("--command <command>", "Command (e.g. canvas.eval)").option("--params <json>", "JSON object string for params", "{}").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 15000)", "15000").option("--idempotency-key <key>", "Idempotency key (optional)").action(async (opts) => {
		await runNodesCommand("invoke", async () => {
			...
			if (BLOCKED_NODE_INVOKE_COMMANDS.has(command.toLowerCase())) throw new Error(`command "${command}" is reserved for shell execution; use the exec tool with host=node instead`);
