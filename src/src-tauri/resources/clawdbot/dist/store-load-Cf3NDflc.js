import { d as normalizeStringifiedOptionalString } from "./string-coerce-C1IzJjqi.js";
...
import { a as readSessionStoreCache, i as isSessionStoreCacheEnabled, l as getFileStatSnapshot, o as setSerializedSessionStore, s as writeSessionStoreCache } from "./store-cache-C-s7DrKa.js";
import { a as normalizeSessionRuntimeModelFields } from "./types--vJusqfs.js";
...
async function rotateSessionFile(storePath, overrideBytes) {
	...
	try {
		...
		log$1.info("backed up session store file before rotation", {
			backupPath: path.basename(backupPath),
			sizeBytes: fileSize
		});
	} catch (err) {
		log$1.warn("session store rotation backup failed", { err });
		return false;
	}
	try {
...
function loadSessionStore(storePath, opts = {}) {
	...
	if (maintenance.mode === "enforce" && Object.keys(store).length > maintenance.maxEntries) {
		...
		if (pruned > 0 || capped > 0) {
			...
			setSerializedSessionStore(storePath, void 0);
...
//#endregion
export { getActiveSessionMaintenanceWarning as a, rotateSessionFile as c, capEntryCount as i, normalizeSessionStore as n, pruneStaleEntries as o, resolveMaintenanceConfig as r, resolveMaintenanceConfigFromInput as s, loadSessionStore as t };
