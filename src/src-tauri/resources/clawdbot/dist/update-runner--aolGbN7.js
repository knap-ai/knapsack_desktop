...
import { a as parseSemver } from "./runtime-guard-k7yNlhML.js";
import { t as resolveStableNodePath } from "./stable-node-path-rJUNRAuA.js";
import { r as runCommandWithTimeout } from "./exec-BaXt0vsL.js";
import { u as trimLogTail } from "./restart-sentinel-C0D17vX5.js";
import { t as applyPathPrepend } from "./path-prepend-Dhwdea3U.js";
import { i as resolveControlUiDistIndexPathForRoot, r as resolveControlUiDistIndexHealth } from "./control-ui-assets-2BY8uGhf.js";
import { f as isBetaTag, l as DEV_BRANCH, n as compareSemverStrings, p as isStableTag, s as detectPackageManager, u as channelToNpmTag } from "./update-check-DdV49UgC.js";
...
//#endregion
//#region src/infra/npm-update-compat-sidecars.ts
const LEGACY_QA_CHANNEL_DIR$1 = ["qa", "channel"].join("-");
const LEGACY_QA_LAB_DIR$1 = ["qa", "lab"].join("-");
const EMPTY_RUNTIME_SIDECAR = "export {};\n";
const NPM_UPDATE_COMPAT_SIDECARS = [{
	path: `dist/extensions/${LEGACY_QA_CHANNEL_DIR$1}/runtime-api.js`,
	content: EMPTY_RUNTIME_SIDECAR
}, {
	path: `dist/extensions/${LEGACY_QA_LAB_DIR$1}/runtime-api.js`,
	content: EMPTY_RUNTIME_SIDECAR
}];
const NPM_UPDATE_COMPAT_SIDECAR_PATHS = new Set(NPM_UPDATE_COMPAT_SIDECARS.map((entry) => entry.path));
const NPM_UPDATE_OMITTED_BUNDLED_PLUGIN_ROOTS = new Set([
	`dist/extensions/${LEGACY_QA_CHANNEL_DIR$1}`,
	`dist/extensions/${LEGACY_QA_LAB_DIR$1}`,
...
const LEGACY_VERIFIER_COMPAT_INVENTORY_PATHS = [`dist/extensions/${LEGACY_QA_CHANNEL_DIR}/runtime-api.js`];
...
function isPackagedDistPath(relativePath) {
	...
	if (LEGACY_VERIFIER_COMPAT_INVENTORY_PATHS.includes(relativePath)) return true;
...
async function collectInstalledPathErrors(params) {
	...
	for (const relativePath of params.expectedFiles) if (!(actualSet !== null ? actualSet.has(relativePath) : await pathExists(path.join(params.packageRoot, relativePath)))) {
		if (NPM_UPDATE_COMPAT_SIDECAR_PATHS.has(relativePath)) continue;
	...
	if (actualSet !== null && params.unexpectedMessage) {
		...
		for (const relativePath of params.actualFiles ?? []) {
			if (NPM_UPDATE_COMPAT_SIDECAR_PATHS.has(relativePath)) continue;
			if (!expectedSet.has(relativePath)) errors.push(params.unexpectedMessage(relativePath));
		}
	}
	return errors;
}
...
//#endregion
export { createGlobalInstallEnv as a, globalInstallArgs as c, resolveGlobalInstallTarget as d, normalizePackageTagInput as f, collectInstalledGlobalPackageErrors as i, resolveExpectedInstalledVersionFromSpec as l, readPackageVersion as m, canResolveRegistryVersionForPackageTarget as n, detectGlobalInstallManagerByPresence as o, readPackageName as p, cleanupGlobalRenameDirs as r, detectGlobalInstallManagerForRoot as s, runGatewayUpdate as t, resolveGlobalInstallSpec as u };
