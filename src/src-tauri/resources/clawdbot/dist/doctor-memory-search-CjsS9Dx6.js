...
import { l as resolveUsableCustomProviderApiKey, s as resolveApiKeyForProvider } from "./model-auth-Di2YDk33.js";
...
import { c as repairShortTermPromotionArtifacts, n as auditDreamingArtifacts, r as auditShortTermPromotionArtifacts, s as repairDreamingArtifacts } from "./memory-core-engine-runtime-DTvwujOU.js";
...
import { c as noteWorkspaceMemoryHealth, o as maybeRepairWorkspaceMemoryHealth } from "./doctor-workspace-BR3jispW.js";
...
async function noteMemorySearchHealth(cfg, opts) {
	...
	if (resolved.provider !== "auto") {
		...
		if (resolved.provider === "lmstudio") {
			...
			note([
				gatewayProbeWarning ? "Memory search provider \"lmstudio\" is configured, but the gateway reports embeddings are not ready." : "Memory search provider \"lmstudio\" is configured, but the gateway could not confirm embeddings are ready.",
				gatewayProbeWarning,
				`Verify: ${formatCliCommand("openclaw memory status --deep")}`
			].filter(Boolean).join("\n"), "Memory search");
			return;
