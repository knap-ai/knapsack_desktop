import { C as textToSpeech } from "./speech-runtime-DncpdrLW.js";
import { $ as getQQBotMediaDir, A as getAccessToken, B as sendChannelMessage, C as formatDuration, D as textToSilk, E as resolveTTSConfig, F as sendC2CImageMessage, G as sendGroupVideoMessage, H as sendGroupFileMessage, I as sendC2CInputNotify, K as sendGroupVoiceMessage, L as sendC2CMessage, M as initApiConfig, N as onMessageSent, O as PLUGIN_USER_AGENT, P as sendC2CFileMessage, Q as getQQBotDataDir, R as sendC2CVideoMessage, S as convertSilkToWav, T as isVoiceAttachment, U as sendGroupImageMessage, V as sendDmMessage, W as sendGroupMessage, X as stopBackgroundTokenRefresh, Y as startBackgroundTokenRefresh, _ as normalizeMediaTags, at as sanitizeFileName, b as formatFileSize, c as sendPhoto, d as sendVideoMsg, et as isLocalPath, f as sendVoice, g as parseQQBotPayload, h as isMediaPayload, it as runDiagnostics, j as getGatewayUrl, k as clearTokenCache, m as isCronReminderPayload, nt as normalizePath, o as sendDocument, ot as debugError, p as encodePayloadForCron, rt as resolveQQBotPayloadLocalFilePath, s as sendMedia, st as debugLog, v as MAX_UPLOAD_SIZE, w as isGlobalTTSAvailable, x as audioFileToSilkBase64, y as downloadFile, z as sendC2CVoiceMessage } from "./outbound-DE5EbLPn.js";
import { t as getQQBotRuntime } from "./runtime-DFBwpBiF.js";
import { a as chunkText, i as TEXT_CHUNK_LIMIT, n as matchSlashCommand, r as qqbotPlugin } from "./slash-commands-Bu_4ImoV.js";
import { a as recordKnownUser, n as flushKnownUsers } from "./known-users-t_aADrCa.js";
...
async function transcribeAudio(audioPath, cfg) {
	...
	return (await resp.json()).text?.trim() || null;
}
//#endregion
...
async function sendPlainReply(payload, replyText, event, actx, sendWithRetry, consumeQuoteRef, toolMediaUrls) {
	...
	if (localMediaToSend.length > 0) {
		...
		for (const mediaPath of localMediaToSend) try {
			const result = await sendMedia({
				to: qualifiedTarget,
				text: "",
				mediaUrl: mediaPath,
	...
	if (toolMediaUrls.length > 0) {
		...
		for (const mediaUrl of toolMediaUrls) try {
			const result = await sendMedia({
				to: qualifiedTarget,
				text: "",
				mediaUrl,
...
async function sendTextChunks(text, event, actx, sendWithRetry, consumeQuoteRef) {
	...
	for (const chunk of chunks) try {
		await sendWithRetry(async (token) => {
			...
			if (event.type === "c2c") return await sendC2CMessage(account.appId, token, event.senderId, chunk, event.messageId, ref);
			else if (event.type === "group" && event.groupOpenid) return await sendGroupMessage(account.appId, token, event.groupOpenid, chunk, event.messageId);
			else if (event.type === "dm" && event.guildId) return await sendDmMessage(token, event.guildId, chunk, event.messageId);
			else if (event.channelId) return await sendChannelMessage(token, event.channelId, chunk, event.messageId);
		});
...
async function sendMarkdownReply(textWithoutImages, imageUrls, mdMatches, bareUrlMatches, event, actx, sendWithRetry, consumeQuoteRef) {
	...
	if (result.trim()) {
		...
		for (const chunk of mdChunks) try {
			await sendWithRetry(async (token) => {
				const ref = consumeQuoteRef();
				if (event.type === "c2c") return await sendC2CMessage(account.appId, token, event.senderId, chunk, event.messageId, ref);
				else if (event.type === "group" && event.groupOpenid) return await sendGroupMessage(account.appId, token, event.groupOpenid, chunk, event.messageId);
...
function loadFromFile() {
	...
	totalLinesOnDisk = 0;
	try {
		if (!fsSync.existsSync(REF_INDEX_FILE)) return cache;
		const lines = fsSync.readFileSync(REF_INDEX_FILE, "utf-8").split("\n");
		const now = Date.now();
		let expired = 0;
...
function appendLine(line) {
	try {
		ensureDir$1();
		fsSync.appendFileSync(REF_INDEX_FILE, JSON.stringify(line) + "\n", "utf-8");
		totalLinesOnDisk++;
	} catch (err) {
...
function compactFile() {
	...
	try {
		...
		fsSync.writeFileSync(tmpPath, lines.join("\n") + "\n", "utf-8");
		fsSync.renameSync(tmpPath, REF_INDEX_FILE);
...
function evictIfNeeded() {
	...
	if (cache.size >= MAX_ENTRIES) {
		const toRemove = [...cache.entries()].sort((a, b) => a[1]._createdAt - b[1]._createdAt).slice(0, cache.size - MAX_ENTRIES + 1e3);
...
async function handleStructuredPayload(ctx, replyText, recordActivity) {
	...
	if (isMediaPayload(parsedPayload)) {
		...
		else log?.error(`[qqbot:${account.accountId}] Unknown media type: ${parsedPayload.mediaType}`);
		recordActivity();
		return true;
	}
...
async function readStructuredPayloadLocalFile(filePath) {
	const openFlags = fsSync.constants.O_RDONLY | ("O_NOFOLLOW" in fsSync.constants ? fsSync.constants.O_NOFOLLOW : 0);
	const handle = await fsSync.promises.open(filePath, openFlags);
	try {
...
async function handleAudioPayload(ctx, payload) {
	...
	try {
		...
		if (ttsCfg) {
			...
			const globalResult = await textToSpeech({
				text: ttsText,
				cfg,
				channel: "qqbot"
			});
...
function doSaveSession(state) {
	...
	try {
		...
		fsSync.writeFileSync(filePath, JSON.stringify(stateToSave, null, 2), "utf-8");
...
async function startGateway(ctx) {
	...
	const cleanup = () => {
		...
		if (currentWs && (currentWs.readyState === WebSocket$1.OPEN || currentWs.readyState === WebSocket$1.CONNECTING)) currentWs.close();
		currentWs = null;
	};
	...
	const connect = async () => {
		...
		try {
			...
			const ws = new WebSocket$1(gatewayUrl, { headers: { "User-Agent": PLUGIN_USER_AGENT } });
			currentWs = ws;
