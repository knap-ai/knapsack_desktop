...
import { f as normalizeVerboseLevel, p as resolveResponseUsageMode } from "./thinking.shared-BtwPxLYS.js";
import { u as resolveAgentIdFromSessionKey } from "./session-key-DAhnzjyr.js";
import { _ as resolveRunModelFallbacksOverride } from "./agent-scope-bjWqU22i.js";
...
import { s as isCliProvider } from "./model-selection-BnFtDmP7.js";
import { s as loadConfig, w as parseNonNegativeByteSize } from "./io-y3Az_Onx.js";
...
import { o as normalizeAnyChannelId, s as normalizeChannelId } from "./registry-C5UkPpaO.js";
...
import { a as isInternalMessageChannel, d as resolveMessageChannel, o as isMarkdownCapableMessageChannel } from "./message-channel-BliByQBl.js";
import { A as resolveGroupSessionKey, l as updateSessionStore, n as loadSessionStore, u as updateSessionStoreEntry } from "./store-C5UK26Ce.js";
import "./runtime-Iz8uZ7EU.js";
import { s as getChannelPlugin } from "./plugins-B09-vgme.js";
import "./sessions-D9ZgHCHb.js";
import { a as resolveSessionTranscriptPath, i as resolveSessionFilePathOptions, r as resolveSessionFilePath } from "./paths-rhN9LKM_.js";
...
import { a as stripHeartbeatToken, c as isSilentReplyPrefixText, l as isSilentReplyText, s as SILENT_REPLY_TOKEN } from "./heartbeat-Bdfr_onc.js";
import { $l as registerAgentRunContext, AS as sanitizeUserFacingText, Ad as isRenderablePayload, DS as isContextOverflowError, Dd as shouldSuppressMessagingToolReplies, ES as isCompactionFailureError, Ed as filterMessagingToolMediaDuplicates, GS as resolveCronStyleNow, Md as resolveReplyToMode, Nu as resolveEffectiveBlockStreamingConfig, OS as isLikelyContextOverflowError, Od as applyReplyTagsToPayload, Rd as parseReplyDirectives, TS as BILLING_ERROR_USER_MESSAGE, Td as filterMessagingToolDuplicates, U as resolveModelAuthMode, Wd as runWithModelFallback, Xl as emitAgentEvent, Yp as resolvePathFromInput, _m as resolveModelCostConfig, _u as resolveOriginAccountId, av as deriveSessionTotalTokens, bu as isReasoningTagProvider, ca as runCliAgent, dm as buildFallbackNotice, eu as enqueueFollowupRun, fm as resolveFallbackTransition, gm as formatUsd, gu as refreshQueuedFollowupSession, h as readPostCompactionContext, hm as formatTokenCount, im as resolveBootstrapWarningSignaturesSeen, iu as routeReply, iv as derivePromptTokens, jS as isBillingErrorMessage, jd as createReplyToModeFilterForChannel, kS as isTransientHttpError, kd as applyReplyThreading, m as estimateMessagesTokens, mm as estimateUsageCost, nu as scheduleFollowupDrain, oa as getCliSessionId, ov as hasNonzeroUsage, pu as createTypingSignaler, ru as isRoutableChannel, s as queueEmbeddedPiMessage, sa as setCliSessionId, sv as normalizeUsage, t as runEmbeddedPiAgent, um as buildFallbackClearedNotice, vu as resolveOriginMessageProvider, yu as resolveOriginMessageTo } from "./pi-embedded-CbCYZxIb.js";
...
import { E as resolveSandboxConfigForAgent } from "./docker-Bhjg8g2t.js";
import { P as resolveSandboxRuntimeStatus, a as ensureSandboxWorkspaceForSession } from "./sandbox-DTlKNieF.js";
import "./common-zReYuoMM.js";
import "./image-ops-By4YfyWa.js";
import "./thinking-CJoSRIGR.js";
import "./path-alias-guards-CwRM04O1.js";
import { a as resolveSandboxedMediaSource, t as assertMediaNotDataUrl } from "./sandbox-paths-Cn8i8Jxm.js";
import "./channel-actions-fV93bLjV.js";
import { a as isAudioFileName } from "./mime-CsUtzcn2.js";
...
import { n as isDiagnosticsEnabled, t as emitDiagnosticEvent } from "./diagnostic-events-DLiZiw1x.js";
import "./diagnostic-CDTtMQBT.js";
import "./logging-CCTKg_fh.js";
import { n as resolveCronStorePath, t as loadCronStore } from "./config-runtime-BYNizC50.js";
import { l as lookupContextTokens } from "./level-overrides-DzIzQ3ZY.js";
...
import { p as resolveSendableOutboundReplyParts, s as hasOutboundReplyContent } from "./reply-payload-CiQ7TPgp.js";
...
import { n as createBlockReplyContentKey, r as createBlockReplyPipeline, t as createAudioAsVoiceBuffer } from "./block-reply-pipeline-DJWiwoeI.js";
...
function buildThreadingToolContext(params) {
	...
	if (!threading?.buildToolContext) {
		const threadTs = sessionCtx.MessageThreadId != null ? String(sessionCtx.MessageThreadId) : void 0;
		return {
			currentChannelId: originTo?.trim() || void 0,
			currentChannelProvider: provider ?? rawProvider,
			currentMessageId,
			currentThreadTs: threadTs || void 0,
			hasRepliedRef
		};
	}
...
function createReplyMediaPathNormalizer(params) {
	...
	const normalizeMediaSource = async (raw) => {
		...
		if (sandboxRoot) return await resolveSandboxedMediaSource({
			media,
			sandboxRoot
		});
...
async function runAgentTurnWithFallback(params) {
	...
	while (true) try {
		...
		return {
			kind: "final",
			payload: { text: isBilling ? BILLING_ERROR_USER_MESSAGE : isContextOverflow ? "⚠️ Context overflow — prompt too large for this model. Try a shorter message or a larger-context model." : isRoleOrderingError ? "⚠️ Message ordering conflict - please try again. If this persists, use /new to start a fresh session." : `⚠️ Agent failed before reply: ${trimmedMessage}.\nLogs: openclaw logs --follow` }
		};
	}
	const finalEmbeddedError = runResult?.meta?.error;
	const hasPayloadText = runResult?.payloads?.some((p) => p.text?.trim());
	if (finalEmbeddedError && isContextOverflowError(finalEmbeddedError.message) && !hasPayloadText) return {
		kind: "final",
...
function resolveMemoryFlushRelativePathForRun(params) {
...
function resolveMemoryFlushSettings(cfg) {
	...
	const prompt = ensureMemoryFlushSafetyHints(defaults?.prompt?.trim() || DEFAULT_MEMORY_FLUSH_PROMPT);
	const systemPrompt = ensureMemoryFlushSafetyHints(defaults?.systemPrompt?.trim() || DEFAULT_MEMORY_FLUSH_SYSTEM_PROMPT);
...
function shouldRunMemoryFlush(params) {
	...
	const totalTokens = (typeof override === "number" && Number.isFinite(override) && override > 0 ? Math.floor(override) : void 0) ?? resolveFreshSessionTotalTokens(params.entry);
	...
	if (hasAlreadyFlushedForCurrentCompaction(params.entry)) return false;
	return true;
}
/**
...
async function runMemoryFlushIfNeeded(params) {
	...
	const flushThreshold = contextWindowTokens - memoryFlushSettings.reserveTokensFloor - memoryFlushSettings.softThresholdTokens;
	...
	if (!(memoryFlushSettings && memoryFlushWritable && !params.isHeartbeat && !isCli && shouldRunMemoryFlush({
		entry,
...
function loadReplyPayloadsDedupeRuntime() {
	replyPayloadsDedupeRuntimePromise ??= import("./reply-payloads-dedupe.runtime-BS_frJdf.js");
	...
	*/
	...
	return async (queued) => {
		try {
			...
			let bootstrapPromptWarningSignaturesSeen = resolveBootstrapWarningSignaturesSeen(((sessionKey ? sessionStore?.[sessionKey] : void 0) ?? sessionEntry)?.systemPromptReport);
			try {
				const fallbackResult = await runWithModelFallback({
					cfg: queued.run.config,
