...
import { c as normalizeCacheKey, d as resolveCacheTtlMs, f as resolveTimeoutSeconds, l as readCache, m as writeCache, u as readResponseText } from "./web-fetch-utils-CHDte8L1.js";
import { E as resolveSiteName, O as withTrustedWebSearchEndpoint, c as setScopedCredentialValue, o as resolveProviderWebSearchPluginConfig, r as getScopedCredentialValue, s as setProviderWebSearchPluginConfigValue, w as resolveSearchCount } from "./provider-web-search-BvBfxXst.js";
...
import { t as assertHttpUrlTargetsPrivateNetwork } from "./ssrf-policy-B7Ivb-NK.js";
...
const SearxngSearchSchema = Type.Object({
	...
	categories: Type.Optional(Type.String({ description: "Optional comma-separated search categories such as general, news, or science." })),
	language: Type.Optional(Type.String({ description: "Optional language code for results such as en, de, or fr." }))
...
function createSearxngWebSearchProvider() {
	return {
		...
		setCredentialValue: (searchConfigTarget, value) => setScopedCredentialValue(searchConfigTarget, "searxng", value),
		getConfiguredCredentialValue: (config) => resolveProviderWebSearchPluginConfig(config, "searxng")?.baseUrl,
		...
		createTool: (ctx) => ({
			...
			execute: async (args) => await runSearxngSearch({
				config: ctx.config,
