...
import { a as resolveMatrixDefaultOrOnlyAccountId, n as requiresExplicitMatrixDefaultAccount } from "./account-selection-BoMWBXkb.js";
import { t as getMatrixRuntime } from "./runtime-Jdij2_G2.js";
import { n as resolveMatrixAccountStorageRoot, s as resolveMatrixLegacyFlatStoragePaths } from "./storage-paths-DQTtejVa.js";
...
function scoreStorageRoot(rootDir) {
	let score = 0;
	if (fsSync.existsSync(path.join(rootDir, "bot-storage.json"))) score += 8;
	if (fsSync.existsSync(path.join(rootDir, "crypto"))) score += 8;
	if (fsSync.existsSync(path.join(rootDir, THREAD_BINDINGS_FILENAME))) score += 4;
	if (fsSync.existsSync(path.join(rootDir, LEGACY_CRYPTO_MIGRATION_FILENAME))) score += 3;
	if (fsSync.existsSync(path.join(rootDir, RECOVERY_KEY_FILENAME))) score += 2;
	if (fsSync.existsSync(path.join(rootDir, IDB_SNAPSHOT_FILENAME))) score += 2;
	if (fsSync.existsSync(path.join(rootDir, STORAGE_META_FILENAME))) score += 1;
	return score;
}
function resolveStorageRootMtimeMs(rootDir) {
	try {
		return fsSync.statSync(rootDir).mtimeMs;
	} catch {
		return 0;
	}
}
function readStoredRootMetadata(rootDir) {
	const metadata = {};
	try {
		const parsed = JSON.parse(fsSync.readFileSync(path.join(rootDir, STORAGE_META_FILENAME), "utf8"));
	...
	try {
		const parsed = JSON.parse(fsSync.readFileSync(path.join(rootDir, STARTUP_VERIFICATION_FILENAME), "utf8"));
...
function resolvePreferredMatrixStorageRoot(params) {
	...
	let siblingEntries = [];
	try {
		siblingEntries = fsSync.readdirSync(parentDir, { withFileTypes: true });
	} catch {
		return {
...
async function maybeMigrateLegacyStorage(params) {
	...
	const hasLegacyStorage = fsSync.existsSync(legacy.storagePath);
	const hasLegacyCrypto = fsSync.existsSync(legacy.cryptoPath);
	if (!hasLegacyStorage && !hasLegacyCrypto) return;
	const hasTargetStorage = fsSync.existsSync(params.storagePaths.storagePath);
	const hasTargetCrypto = fsSync.existsSync(params.storagePaths.cryptoPath);
	...
	const { maybeCreateMatrixMigrationSnapshot } = await import("./migration-snapshot.runtime-DWM5oXjy.js");
	await maybeCreateMatrixMigrationSnapshot({
		trigger: "matrix-client-fallback",
		env: params.env,
		log: logger
	});
	fsSync.mkdirSync(params.storagePaths.rootDir, { recursive: true });
	const moved = [];
...
function moveLegacyStoragePathOrThrow(params) {
	if (!fsSync.existsSync(params.sourcePath)) return;
	if (fsSync.existsSync(params.targetPath)) throw new Error(`legacy Matrix ${params.label} target already exists (${params.targetPath}); refusing to overwrite it automatically`);
	fsSync.renameSync(params.sourcePath, params.targetPath);
	params.moved.push({
...
function rollbackLegacyMoves(moved) {
	for (const entry of moved.toReversed()) try {
		if (!fsSync.existsSync(entry.targetPath) || fsSync.existsSync(entry.sourcePath)) continue;
		fsSync.renameSync(entry.targetPath, entry.sourcePath);
	} catch (err) {
...
function writeStorageMeta(params) {
	try {
		...
		fsSync.mkdirSync(params.storagePaths.rootDir, { recursive: true });
		fsSync.writeFileSync(params.storagePaths.metaPath, JSON.stringify(payload, null, 2), "utf-8");
	} catch {}
}
//#endregion
export { writeStorageMeta as i, resolveMatrixStateFilePath as n, resolveMatrixStoragePaths as r, maybeMigrateLegacyStorage as t };
