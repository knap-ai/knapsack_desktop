...
import { o as resolvePluginSourceRoots } from "./manifest-registry-BSmqODu2.js";
import { r as replaceConfigFile } from "./config-CJQx-9zo.js";
import { t as promptYesNo } from "./prompt-B53VU_Ut.js";
import { n as setPluginEnabledInConfig, t as enablePluginInConfig } from "./enable-CpDMTfhy.js";
import { s as parseClawHubPluginSpec } from "./clawhub-y4k4A2d9.js";
import { a as buildPluginInspectReport, i as buildPluginDiagnosticsReport, n as buildPluginCompatibilityNotices, o as buildPluginSnapshotReport, s as formatPluginCompatibilityNotice, t as buildAllPluginInspectReports } from "./status-DrwAkOFC.js";
import { d as logSlotWarnings, i as createPluginInstallLogger, t as applySlotSelectionForPlugin } from "./plugins-command-helpers-XbI77OBE.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-BjlagKDo.js";
import { a as uninstallPlugin, i as resolveUninstallDirectoryTarget, r as resolveUninstallChannelConfigKeys } from "./matrix-BI7ZZ_5T.js";
import { n as runPluginInstallCommand, t as runPluginUpdateCommand } from "./plugins-update-command-JdJLXyqP.js";
import { o as listMarketplacePlugins } from "./update-COK0Zm-6.js";
...
function registerPluginsCli(program) {
	...
	plugins.command("inspect").alias("info").description("Inspect plugin details").argument("[id]", "Plugin id").option("--all", "Inspect all plugins").option("--json", "Print JSON").action((id, opts) => {
	...
	plugins.command("disable").description("Disable a plugin in config").argument("<id>", "Plugin id").action(async (id) => {
		const snapshot = await readConfigFileSnapshot();
		await replaceConfigFile({
			nextConfig: setPluginEnabledInConfig(snapshot.sourceConfig ?? snapshot.config, id, false),
			...snapshot.hash !== void 0 ? { baseHash: snapshot.hash } : {}
		});
	...
	plugins.command("uninstall").description("Uninstall a plugin").argument("<id>", "Plugin id").option("--keep-files", "Keep installed files on disk", false).option("--keep-config", "Deprecated alias for --keep-files", false).option("--force", "Skip confirmation prompt", false).option("--dry-run", "Show what would be removed without making changes", false).action(async (id, opts) => {
		...
		if (hasInstall && channels) {
			for (const key of resolveUninstallChannelConfigKeys(pluginId, { channelIds })) if (Object.hasOwn(channels, key)) preview.push(`channel config (channels.${key})`);
		}
		...
		if (result.actions.channelConfig) removed.push("channel config");
	...
	plugins.command("install").description("Install a plugin or hook pack (path, archive, npm spec, clawhub:package, or marketplace entry)").argument("<path-or-spec-or-plugin>", "Path (.ts/.js/.zip/.tgz/.tar.gz), npm package spec, or marketplace plugin name").option("-l, --link", "Link a local path instead of copying", false).option("--pin", "Record npm installs as exact resolved <name>@<version>", false).option("--dangerously-force-unsafe-install", "Bypass built-in dangerous-code install blocking (plugin hooks may still block)", false).option("--marketplace <source>", "Install a Claude marketplace plugin from a local repo/path or git/GitHub source").action(async (raw, opts) => {
		await runPluginInstallCommand({
			raw,
			opts
		});
	});
	plugins.command("update").description("Update installed plugins and tracked hook packs").argument("[id]", "Plugin or hook-pack id (omit with --all)").option("--all", "Update all tracked plugins and hook packs", false).option("--dry-run", "Show what would change without writing", false).action(async (id, opts) => {
