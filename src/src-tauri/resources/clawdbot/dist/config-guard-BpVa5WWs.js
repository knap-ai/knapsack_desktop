import { R as shouldMigrateStateFromPath } from "./logger-x0IvPL2B.js";
...
async function ensureConfigReady(params) {
	...
	if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-0y1Op8Ld.js")).runDoctorConfigPreflight({
			migrateState: false,
	...
	const { formatConfigIssueLines } = await import("./issue-format-iWJ3Jngm.js");
	...
	const [{ colorize, isRich, theme }, { shortenHomePath }, { formatCliCommand }] = await Promise.all([
		import("./theme-D7dpa3oU.js"),
		import("./utils-DMQR8q6t.js"),
		import("./command-format-B-CmRDIm.js")
	]);
	const rich = isRich();
