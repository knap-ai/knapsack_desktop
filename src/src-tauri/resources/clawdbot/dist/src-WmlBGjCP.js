...
import { r as resolveDefaultGroupPolicy } from "./runtime-group-policy-Dj5ka44z.js";
import { n as fetchWithSsrFGuard } from "./fetch-guard-Lvq2pw52.js";
import { x as buildMediaPayload } from "./reply-payload-SzVz53hc.js";
...
import { A as extractInlineImageCandidates, B as safeFetchWithPolicy, C as formatUnknownError, D as IMG_SRC_RE, E as GRAPH_ROOT, F as isUrlAllowed, I as normalizeContentType, L as resolveAttachmentFetchPolicy, M as isDownloadableAttachment, N as isLikelyImageAttachment, O as applyAuthorizationHeaderForUrl, P as isRecord, R as resolveMediaSsrfPolicy, S as formatMSTeamsSendErrorHint, T as ATTACHMENT_TAG_RE, V as safeHostForUrl, _ as loadMSTeamsSdkWithAuth, f as resolveMSTeamsCredentials, g as createMSTeamsTokenProvider, h as createMSTeamsAdapter, i as fetchGraphJson, j as inferPlaceholder, k as extractHtmlFromAttachment, m as createBotFrameworkJwtValidator, x as classifyMSTeamsSendError, y as getMSTeamsRuntime, z as resolveRequestUrl } from "./graph-users-F-Pl04ex.js";
import { o as resolveMSTeamsChannelAllowlist, s as resolveMSTeamsUserAllowlist } from "./resolve-allowlist-DWb6WPbt.js";
import { a as resolveMSTeamsRouteConfig, i as resolveMSTeamsReplyPolicy, n as resolveMSTeamsAllowlistMatch, t as isMSTeamsGroupAllowed } from "./policy-DVlc-gQZ.js";
...
async function storeSessionLearning(params) {
	...
	const learningsFile = path.join(params.storePath, `${sanitizeSessionKey(params.sessionKey)}.learnings.json`);
	let learnings = [];
	try {
...
function extractMSTeamsQuoteInfo(attachments) {
	for (const att of attachments) {
		const content = typeof att.content === "string" ? att.content : typeof att.content === "object" && att.content !== null ? String(att.content.text ?? att.content.body ?? "") : "";
		if (!content) continue;
...
function createTeamsReplyStreamController(params) {
	const stream = params.conversationType?.toLowerCase() === "personal" ? new TeamsHttpStream({
...
function createMSTeamsReplyDispatcher(params) {
	...
	const msteamsCfg = params.cfg.channels?.msteams;
	const conversationType = params.conversationRef.conversation?.conversationType?.toLowerCase();
	const sendTypingIndicator = conversationType === "personal" || conversationType === "groupchat" ? async () => {
...
function createMSTeamsMessageHandler(deps) {
	...
	const handleTeamsMessageNow = async (params) => {
		...
		if (activity.replyToId && isChannel && teamId) try {
			...
			const [parentMsg, replies] = await Promise.all([fetchChannelMessage(graphToken, groupId, conversationId, activity.replyToId), fetchThreadReplies(graphToken, groupId, conversationId, activity.replyToId)]);
			const allMessages = parentMsg ? [parentMsg, ...replies] : replies;
			const formatted = formatThreadContext(groupPolicy === "allowlist" ? allMessages.filter((msg) => {
...
function registerMSTeamsHandlers(handler, deps) {
	...
	handler.onMembersAdded(async (context, next) => {
		...
		for (const member of membersAdded) if (member.id === botId) {
			const isPersonal = (ctx.activity?.conversation?.conversationType?.toLowerCase() ?? "personal") === "personal";
