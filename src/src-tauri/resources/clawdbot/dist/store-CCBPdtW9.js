import { h as resolveConfigDir } from "./utils-CE3P21nG.js";
import { n as extensionForMime, t as detectMime } from "./mime-C9472kP8.js";
import { t as retainSafeHeadersForCrossOriginRedirect } from "./redirect-headers-Dd5ch3iy.js";
import { u as resolvePinnedHostname } from "./ssrf-BWlfjI7J.js";
...
async function cleanOldMedia(ttlMs = DEFAULT_TTL_MS, options = {}) {
	...
	const removeExpiredFilesInDir = async (dir) => {
		...
		for (const entry of dirEntries) {
			...
			if (stat.isDirectory()) {
				if (recursive) {
					if (await removeExpiredFilesInDir(fullPath)) await fs.rmdir(fullPath).catch(() => {});
				}
				continue;
			}
			if (!stat.isFile()) continue;
			if (now - stat.mtimeMs > ttlMs) await fs.rm(fullPath, { force: true }).catch(() => {});
		}
	...
	for (const file of entries) {
		...
		if (!stat || stat.isSymbolicLink()) continue;
		if (stat.isDirectory()) {
			if (await removeExpiredFilesInDir(full)) await fs.rmdir(full).catch(() => {});
			continue;
		}
		if (stat.isFile() && now - stat.mtimeMs > ttlMs) await fs.rm(full, { force: true }).catch(() => {});
	}
}
