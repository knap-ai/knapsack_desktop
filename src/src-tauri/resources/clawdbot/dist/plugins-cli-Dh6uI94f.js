...
import { i as resolvePluginSourceRoots } from "./discovery-DmIzxmAL.js";
...
import { i as uninstallPlugin, n as resolveUninstallChannelConfigKeys, r as resolveUninstallDirectoryTarget } from "./uninstall-D4E4tLKL.js";
import { n as setPluginEnabledInConfig, t as enablePluginInConfig } from "./enable-y339R98P.js";
import { m as parseClawHubPluginSpec } from "./clawhub-DUtsrJs-.js";
import { a as buildPluginDiagnosticsReport, c as formatPluginCompatibilityNotice, n as buildPluginCompatibilityNotices, o as buildPluginInspectReport, s as buildPluginSnapshotReport, t as buildAllPluginInspectReports } from "./status-Clo7fOEI.js";
import { i as createPluginInstallLogger, l as logSlotWarnings, t as applySlotSelectionForPlugin } from "./plugins-command-helpers-DMQfl4Ue.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-eRgd_5k_.js";
import { n as runPluginInstallCommand, t as runPluginUpdateCommand } from "./plugins-update-command-4OnDPwFM.js";
import { i as listMarketplacePlugins } from "./update-Bv_4hYzq.js";
...
function registerPluginsCli(program) {
	...
	plugins.command("list").description("List discovered plugins").option("--json", "Print JSON").option("--enabled", "Only show enabled plugins", false).option("--verbose", "Show detailed entries", false).action((opts) => {
		const report = buildPluginSnapshotReport(opts.json ? { logger: quietPluginJsonLogger } : void 0);
	...
	plugins.command("uninstall").description("Uninstall a plugin").argument("<id>", "Plugin id").option("--keep-files", "Keep installed files on disk", false).option("--keep-config", "Deprecated alias for --keep-files", false).option("--force", "Skip confirmation prompt", false).option("--dry-run", "Show what would be removed without making changes", false).action(async (id, opts) => {
		...
		if (cfg.plugins?.slots?.memory === pluginId) preview.push(`memory slot (will reset to "${defaultSlotIdForKey("memory")}")`);
