...
import "./channel-actions-CTdIgPy-.js";
import { _ as browserStop, a as untrackSessionBrowserTab, d as browserOpenTab, f as browserProfiles, g as browserStatus, h as browserStart, i as trackSessionBrowserTab, l as browserDoctor, m as browserSnapshot, o as browserCloseTab, r as touchSessionBrowserTab, u as browserFocusTab, y as browserTabs } from "./session-tab-registry-Dq2n4_Pr.js";
import { c as DEFAULT_AI_SNAPSHOT_MAX_CHARS, i as resolveExistingPathsWithinRoot, r as DEFAULT_UPLOAD_DIR } from "./paths-CR_dD-bq.js";
import { a as applyBrowserProxyPaths, c as browserPdfSave, d as browserArmFileChooser, f as browserNavigate, i as resolveRequestedBrowserProfile, l as browserAct, n as isPersistentBrowserProfileMutation, o as persistBrowserProxyFiles, p as browserScreenshotAction, s as browserConsoleMessages, u as browserArmDialog } from "./browser-runtime-BW8O6o45.js";
import { n as resolveProfile, t as resolveBrowserConfig } from "./config-M802LKai.js";
import { f as redactCdpUrl } from "./cdp.helpers-2643q7gc.js";
import { r as getBrowserProfileCapabilities } from "./target-id-fnO4XRxy.js";
import { l as normalizeBrowserFormField, u as normalizeBrowserFormFieldValue } from "./pw-role-snapshot-B6qiUGJd.js";
import { a as createBrowserRouteDispatcher, r as startBrowserControlServiceFromConfig, t as createBrowserControlContext } from "./control-service-y0fn2fZG.js";
...
function withConfiguredActTimeout(request, profileName) {
	...
	const configuredTimeout = normalizePositiveTimeoutMs(browserToolActionDeps.loadConfig().browser?.actionTimeoutMs) ?? 6e4;
	return {
		...typedRequest,
...
function registerBrowserInspectCommands(browser, parentOpts) {
	...
	browser.command("snapshot").description("Capture a snapshot (default: ai; aria is the accessibility tree)").option("--format <aria|ai>", "Snapshot format (default: ai)", "ai").option("--target-id <id>", "CDP target id (or unique prefix)").option("--limit <n>", "Max nodes (default: 500/800)", (v) => Number(v)).option("--mode <efficient>", "Snapshot preset (efficient)").option("--efficient", "Use the efficient snapshot preset", false).option("--interactive", "Role snapshot: interactive elements only", false).option("--compact", "Role snapshot: compact output", false).option("--depth <n>", "Role snapshot: max depth", (v) => Number(v)).option("--selector <sel>", "Role snapshot: scope to CSS selector").option("--frame <sel>", "Role snapshot: scope to an iframe selector").option("--labels", "Include viewport label overlay screenshot", false).option("--urls", "Append discovered link URLs to AI snapshots", false).option("--out <path>", "Write snapshot to a file").action(async (opts, cmd) => {
		...
		const configMode = !(typeof cmd.getOptionValueSource === "function" && cmd.getOptionValueSource("format") === "cli") && format === "ai" && loadConfig().browser?.snapshotDefaults?.mode === "efficient" ? "efficient" : void 0;
...
async function runBrowserDoctor(parent, profile) {
	const checks = [];
	let status = null;
	try {
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	browser.command("doctor").description("Check browser plugin readiness").action(async (_opts, cmd) => {
		const parent = parentOpts(cmd);
		const profile = parent?.browserProfile;
		await runBrowserCommand$1(async () => {
	...
	return {
		...
		start: async () => {
			...
			handle = await startLazyPluginServiceModule({
				...
				loadDefaultModule: async () => await import("./server-Fu2vcrgV.js"),
...
//#endregion
export { createBrowserTool as a, registerBrowserCli as i, browserHandlers as n, handleBrowserGatewayRequest as r, createBrowserPluginService as t };
