...
import { s as withTelegramApiErrorLogging } from "./format-DqAjfaGj.js";
import { C as isRecoverableTelegramNetworkError } from "./sticker-cache-Cf0p9p0n.js";
import { n as readTelegramUpdateOffset, r as writeTelegramUpdateOffset } from "./update-offset-store-DXOVZbFr.js";
import { n as TelegramExecApprovalHandler, t as createTelegramBot } from "./bot-C3BP06QT.js";
...
var TelegramPollingTransportState = class {
	...
	acquireForNextCycle() {
		const nextTransport = this.#transportDirty || !this.#telegramTransport ? this.opts.createTelegramTransport?.() ?? this.#telegramTransport : this.#telegramTransport;
...
var TelegramPollingSession = class {
	...
	async #runPollingCycle(bot) {
		...
		let latestInFlightApiStartedAt = null;
		const inFlightApiStartedAt = /* @__PURE__ */ new Map();
		let lastGetUpdatesStartedAt = null;
		let lastGetUpdatesFinishedAt = null;
		let lastGetUpdatesDurationMs = null;
		let lastGetUpdatesOutcome = "not-started";
		let lastGetUpdatesError = null;
		let lastGetUpdatesOffset = null;
		...
		bot.api.config.use(async (prev, method, payload, signal) => {
			if (method !== "getUpdates") {
				...
				latestInFlightApiStartedAt = latestInFlightApiStartedAt == null ? startedAt : Math.max(latestInFlightApiStartedAt, startedAt);
				try {
					...
					if (latestInFlightApiStartedAt === startedAt) {
						...
						for (const activeStartedAt of inFlightApiStartedAt.values()) newestStartedAt = newestStartedAt == null ? activeStartedAt : Math.max(newestStartedAt, activeStartedAt);
			...
			lastGetUpdatesOffset = payload && typeof payload === "object" && "offset" in payload ? payload.offset ?? null : null;
		...
		const watchdog = setInterval(() => {
			...
			const now = Date.now();
			const activeElapsed = inFlightGetUpdates > 0 && lastGetUpdatesStartedAt != null ? now - lastGetUpdatesStartedAt : 0;
			...
			const apiElapsed = now - (latestInFlightApiStartedAt == null ? lastApiActivityAt : Math.max(lastApiActivityAt, latestInFlightApiStartedAt));
			if (elapsed > POLL_STALL_THRESHOLD_MS && apiElapsed > POLL_STALL_THRESHOLD_MS && runner.isRunning()) {
				if (stallDiagLoggedAt && now - stallDiagLoggedAt < POLL_STALL_THRESHOLD_MS / 2) return;
				stallDiagLoggedAt = now;
				this.#transportState.markDirty();
				stalledRestart = true;
				const elapsedLabel = inFlightGetUpdates > 0 ? `active getUpdates stuck for ${formatDurationPrecise(elapsed)}` : `no completed getUpdates for ${formatDurationPrecise(elapsed)}`;
				this.opts.log(`[telegram] Polling stall detected (${elapsedLabel}); forcing restart. [diag inFlight=${inFlightGetUpdates} outcome=${lastGetUpdatesOutcome} startedAt=${lastGetUpdatesStartedAt ?? "n/a"} finishedAt=${lastGetUpdatesFinishedAt ?? "n/a"} durationMs=${lastGetUpdatesDurationMs ?? "n/a"} offset=${lastGetUpdatesOffset ?? "n/a"}${lastGetUpdatesError ? ` error=${lastGetUpdatesError}` : ""}]`);
				stopRunner();
				stopBot();
		...
		try {
			...
			if (isRecoverable) this.#transportState.markDirty();
...
//#endregion
export { TelegramExecApprovalHandler, TelegramPollingSession, readTelegramUpdateOffset, writeTelegramUpdateOffset };
