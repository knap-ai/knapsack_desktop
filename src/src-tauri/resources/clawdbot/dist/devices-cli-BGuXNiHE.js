...
import { a as listDevicePairing, f as summarizeDeviceTokens, t as approveDevicePairing } from "./device-pairing-DFjN-551.js";
...
async function approvePairingWithFallback(opts, requestId) {
	try {
		...
		if (approved.status === "forbidden") throw new Error(`missing scope: ${approved.missingScope}`, { cause: error });
		return {
			requestId,
