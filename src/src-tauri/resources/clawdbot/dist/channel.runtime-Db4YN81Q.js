...
import { i as createAttachedChannelResultAdapter } from "./channel-send-result-COXGPSxL.js";
import { t as chunkTextForOutbound } from "./text-chunking-QO5AWjBS.js";
import "./runtime-api-BtEK-kAj.js";
import { a as listChannelsForTeam, c as postGraphBetaJson, i as fetchGraphJson, l as postGraphJson, n as deleteGraphRequest, o as listTeamsByName, r as escapeOData, s as normalizeQuery, t as searchGraphUsers, u as resolveGraphToken } from "./graph-users-F-Pl04ex.js";
import { _ as createMSTeamsPollStoreFs, a as sendMessageMSTeams, i as sendAdaptiveCardMSTeams, n as deleteMessageMSTeams, o as sendPollMSTeams, r as editMessageMSTeams, t as probeMSTeams, y as createMSTeamsConversationStoreFs } from "./probe-CLVRoKDq.js";
...
async function unpinMessageMSTeams(params) {
	await deleteGraphRequest({
		token: await resolveGraphToken(params.cfg),
		path: `${resolveConversationPath(await resolveGraphConversationId(params.to)).basePath}/pinnedMessages/${encodeURIComponent(params.pinnedMessageId)}`
	});
	return { ok: true };
}
/**
...
async function listPinsMSTeams(params) {
	return { pins: ((await fetchGraphJson({
		token: await resolveGraphToken(params.cfg),
		path: `${resolveConversationPath(await resolveGraphConversationId(params.to)).basePath}/pinnedMessages?$expand=message`
	})).value ?? []).map((pin) => ({
		id: pin.id ?? "",
...
function validateReactionType(raw) {
	const normalized = raw.toLowerCase().trim();
	if (!TEAMS_REACTION_TYPES.includes(normalized)) throw new Error(`Invalid reaction type "${raw}". Valid types: ${TEAMS_REACTION_TYPES.join(", ")}`);
	return normalized;
}
/**
...
async function listReactionsMSTeams(params) {
	...
	for (const reaction of msg.reactions ?? []) {
		...
		if (reaction.user?.id) grouped.get(type).push({
			id: reaction.user.id,
			displayName: reaction.user.displayName
		});
	}
	return { reactions: Array.from(grouped.entries()).map(([type, users]) => ({
		reactionType: type,
		count: users.length,
		users
	})) };
