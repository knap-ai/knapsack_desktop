...
async function generateImage(params) {
	...
	for (const candidate of candidates) {
		...
		if (!provider) {
			...
			log.warn(`image-generation candidate failed: ${candidate.provider}/${candidate.model}: ${error}`);
			continue;
		}
		try {
			...
			log.warn(`image-generation candidate failed: ${candidate.provider}/${candidate.model}: ${described?.message ?? formatErrorMessage(err)}`);
		}
	}
