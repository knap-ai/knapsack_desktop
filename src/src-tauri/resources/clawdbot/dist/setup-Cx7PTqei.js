...
import { n as buildPluginCompatibilityNotices, o as formatPluginCompatibilityNotice } from "./status-DI5hmNdJ.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CBWv3YEd.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-C-cqj8tu.js");
		const { logConfigUpdated } = await import("./logging-Ctt_ejwe.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-5hKqhfFZ.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BYxvK4Rv.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-BoVLKXmt.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-VExPfG77.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CUgNhwBj.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DhzLW2Wf.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-flQ2ikPU.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-B9-ighlr.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-Cnt3XMM5.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-D2FNfT7d.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BmNqOn6M.js");
