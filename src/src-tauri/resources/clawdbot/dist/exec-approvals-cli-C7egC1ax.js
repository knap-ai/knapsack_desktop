...
import { A as saveExecApprovals, y as readExecApprovalsSnapshot } from "./exec-approvals-D6YTmZxM.js";
import { n as formatTimeAgo } from "./format-relative-hwB9KFeB.js";
import { n as callGatewayFromCli } from "./gateway-rpc-CeO48anM.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-BjlagKDo.js";
import { t as collectExecPolicyScopeSnapshots } from "./exec-approvals-effective-C5ss5bJn.js";
import { t as describeUnknownError } from "./shared-56whjdYX.js";
import { a as resolveNodeId, r as nodesCallOpts } from "./rpc-DGeAaoOQ.js";
...
async function loadConfigForApprovalsTarget(params) {
	try {
		if (params.source === "local") return await readBestEffortConfig();
	...
	nodesCallOpts(approvals.command("get").description("Fetch exec approvals snapshot").option("--node <node>", "Target node id/name/IP").option("--gateway", "Force gateway approvals", false).action(async (opts) => {
		try {
			...
			const effectivePolicy = buildEffectivePolicyReport({
				cfg: await loadConfigForApprovalsTarget({
					opts,
					source
				}),
				source,
