import { a as __require, n as __esmMin, o as __toCommonJS, r as __exportAll, t as __commonJSMin } from "./chunk-A-jGZS85.js";
import { a as require_axios, i as require_logging, n as require_lib$2, r as require_http$2, t as require_dist$3 } from "./dist-BboFMvQd.js";
import { t as require_jsonwebtoken } from "./jsonwebtoken-kKFs1rUv.js";
import { t as require_src } from "./src-Bfafj5FO.js";
...
var require_dist$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	var l = require_http$2(), $ = require_lib$2();
	...
	function C(o, t, e) {
		...
		for (let s of t.path ?? []) o = o.replace(`{${s}}`, e[s]);
		return `${o}${$__default.default.stringify(r, {
			addQueryPrefix: true,
			arrayFormat: "comma"
		})}`;
	}
	function y(o, t, e) {
		let r = (o.header ?? []).reduce((s, a) => (t[a] && (s[a] = t[a]), s), {});
	...
	var d = class extends Error {
		...
		constructor(t, e, r, s, a) {
			let i = e && typeof e == "object" && "error" in e ? e.error : void 0, h = i?.message ? `Graph ${r.toUpperCase()} ${s} failed (${t}): ${i.message}` : `Graph ${r.toUpperCase()} ${s} failed with status ${t}`;
			super(h, { cause: a }), this.name = "GraphError", this.statusCode = t, this.code = i?.code, this.body = e;
		}
	}, E = "https://graph.microsoft.com", _ = class {
		baseUrlRoot;
		_http;
		betaHttp;
		get http() {
		...
		constructor(t) {
			this.baseUrlRoot = t?.baseUrlRoot ?? E, t ? "request" in t ? this._http = t.clone({
		...
		async call(t, ...e) {
			let r = e[e.length - 1], s = e.length > t.length && r && typeof r == "object" && "requestConfig" in r, a = s ? e.slice(0, -1) : e, i = s ? r : void 0, { ver: h = "v1.0", method: p, path: b, paramDefs: u = {}, params: f = {}, body: U } = t(...a), c = C(b, u, f), m = y(u, f, i?.requestConfig), g = this.getHttpClient(h);
			try {
				switch (p) {
					case "delete":
					case "get": return (await g[p](c, m)).data;
					case "patch":
					case "post":
					case "put": return (await g[p](c, U, m)).data;
					default: throw new Error(`Unsupported HTTP method: ${p}`);
				}
			} catch (n) {
				if (n && typeof n == "object" && "isAxiosError" in n && "response" in n && n.response) {
					let { response: R } = n;
					throw new d(R.status, R.data, p, c, n);
				}
				throw n;
			}
		}
		...
		*/
		...
		validateIssuer(iss) {
			...
			if ("allowedTenantIds" in validateIssuer) {
				...
				if (!iss) throw new Error("Token missing issuer claim");
				if (!this.options.tenantId) return;
				const isMultiTenant = [
					"common",
					"organizations",
					"consumers"
		...
		validateScope(scp, overrideValidateScope) {
			...
			if (validateScope) {
				if (!(scp ?? "").includes(validateScope.requiredScope)) throw new Error(`Token missing required scope: ${validateScope.requiredScope}`);
			}
		}
