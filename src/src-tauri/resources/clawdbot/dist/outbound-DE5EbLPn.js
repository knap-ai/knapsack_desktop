...
function resolveQQBotLocalMediaPath(p) {
	...
	if (!isLocalPath(normalized) || fs.existsSync(normalized)) return normalized;
...
function resolveQQBotPayloadLocalFilePath(p) {
	...
	for (const root of allowedRoots) {
		...
		if (isPathWithinRoot(canonicalCandidate, fs.existsSync(resolvedRoot) ? fs.realpathSync(resolvedRoot) : resolvedRoot)) return canonicalCandidate;
	}
	return null;
}
/**
...
function detectFfmpeg() {
	...
	_ffmpegCheckPromise = (async () => {
		...
		for (const p of commonPaths) if (p && fs.existsSync(p)) {
...
async function checkSilkWasmAvailable() {
	...
	try {
		...
		debugWarn(`[platform] silk-wasm: NOT available (${err instanceof Error ? err.message : String(err)})`);
	}
...
function loadSilkWasm() {
	...
	_silkWasmPromise = import("silk-wasm").catch((err) => {
		debugWarn(`[audio-convert] silk-wasm not available; SILK encode/decode disabled (${err instanceof Error ? err.message : String(err)})`);
		return null;
	});
...
async function convertSilkToWav(inputPath, outputDir) {
	...
	const strippedBuf = stripAmrHeader(fs.readFileSync(inputPath));
...
function isVoiceAttachment(att) {
	...
	const ext = att.filename ? path$3.extname(att.filename).toLowerCase() : "";
	return [
		".amr",
		".silk",
		".slk",
...
function shouldTranscodeVoice(filePath, mimeType) {
	if (mimeType && QQ_NATIVE_VOICE_MIMES.has(mimeType.toLowerCase())) return false;
	const ext = path$3.extname(filePath).toLowerCase();
...
async function textToSpeechPCM(text, ttsCfg) {
	...
	for (const { format, needsDecode } of formats) {
		...
		try {
			...
			const tmpDir = path$3.join(fs.mkdtempSync(path$3.join(__require("node:os").tmpdir(), "tts-")));
...
async function audioFileToSilkBase64(filePath, directUploadFormats) {
	...
	if (ffmpegCmd) try {
		...
		debugError(`[audio-convert] ffmpeg conversion failed: ${err instanceof Error ? err.message : String(err)}`);
	}
...
async function wasmDecodeMp3ToPCM(buf, targetRate) {
	try {
		...
		debugError(`[audio-convert] WASM MP3 decode failed: ${err instanceof Error ? err.message : String(err)}`);
...
async function downloadFile(url, destDir, originalFilename) {
	try {
		...
		if (!filename) try {
			...
			filename = path$3.basename(urlPath) || "download";
		} catch {
			filename = "download";
		}
		const ts = Date.now();
...
const SELF_CLOSING_TAG_REGEX = new RegExp("`?[<＜<]\\s*(" + TAG_NAME_PATTERN + ")(?:\\s+(?!file|src|path|url)[a-z_-]+\\s*=\\s*[\"']?[^\"'/>＞>]*?[\"']?)*\\s+(?:file|src|path|url)\\s*=\\s*[\"']?([^\"'/>＞>]+?)[\"']?(?:\\s+[a-z_-]+\\s*=\\s*[\"']?[^\"'/>＞>]*?[\"']?)*\\s*/?\\s*[>＞>]`?", "gi");
...
async function sendPhoto(ctx, imagePath) {
	...
	if (isLocal) {
		...
		const ext = path$2.extname(mediaPath).toLowerCase();
		const mimeType = {
			".jpg": "image/jpeg",
			".jpeg": "image/jpeg",
			".png": "image/png",
...
async function sendVoice(ctx, voicePath, directUploadFormats, transcodeEnabled = true) {
	...
	if (mediaPath.startsWith("http://") || mediaPath.startsWith("https://")) {
		if (shouldDirectUploadUrl(ctx.account)) try {
			...
			debugWarn(`${prefix} sendVoice: URL direct upload failed (${err instanceof Error ? err.message : String(err)}), downloading locally and retrying...`);
		}
...
function getCleanExt(filePath) {
	...
	return path$2.extname(cleanPath).toLowerCase();
}
...
//#endregion
export { getQQBotMediaDir as $, getAccessToken as A, sendChannelMessage as B, formatDuration as C, textToSilk as D, resolveTTSConfig as E, sendC2CImageMessage as F, sendGroupVideoMessage as G, sendGroupFileMessage as H, sendC2CInputNotify as I, sendProactiveGroupMessage as J, sendGroupVoiceMessage as K, sendC2CMessage as L, initApiConfig as M, onMessageSent as N, PLUGIN_USER_AGENT as O, sendC2CFileMessage as P, getQQBotDataDir as Q, sendC2CVideoMessage as R, convertSilkToWav as S, isVoiceAttachment as T, sendGroupImageMessage as U, sendDmMessage as V, sendGroupMessage as W, stopBackgroundTokenRefresh as X, startBackgroundTokenRefresh as Y, getHomeDir as Z, normalizeMediaTags as _, sendCronMessage as a, sanitizeFileName as at, formatFileSize as b, sendPhoto as c, sendVideoMsg as d, isLocalPath as et, sendVoice as f, parseQQBotPayload as g, isMediaPayload as h, recordMessageReply as i, runDiagnostics as it, getGatewayUrl as j, clearTokenCache as k, sendProactiveMessage as l, isCronReminderPayload as m, getMessageReplyConfig as n, normalizePath as nt, sendDocument as o, debugError as ot, encodePayloadForCron as p, sendProactiveC2CMessage as q, getMessageReplyStats as r, resolveQQBotPayloadLocalFilePath as rt, sendMedia as s, debugLog as st, checkMessageReplyLimit as t, isWindows as tt, sendText as u, MAX_UPLOAD_SIZE as v, isGlobalTTSAvailable as w, audioFileToSilkBase64 as x, downloadFile as y, sendC2CVoiceMessage as z };
