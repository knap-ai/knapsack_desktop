import { b as shortenHomeInString, g as resolveHomeDir, m as pathExists, v as resolveUserPath, x as shortenHomePath } from "./utils-CE3P21nG.js";
...
import { t as formatSessionArchiveTimestamp } from "./artifacts-DtJnd71X.js";
...
async function resolveOutputPath(params) {
	...
	try {
		if ((await fs.stat(resolved)).isDirectory()) return path.join(resolved, basename);
	} catch {}
	return resolved;
}
...
async function publishTempArchive(params) {
	try {
		...
		try {
			await fs.copyFile(params.tempArchivePath, params.outputPath, constants.COPYFILE_EXCL);
		} catch (copyErr) {
			const copyCode = copyErr?.code;
			if (copyCode !== "EEXIST") await fs.rm(params.outputPath, { force: true }).catch(() => void 0);
...
async function createBackupArchive(opts = {}) {
	...
	try {
		...
		await fs.rm(tempArchivePath, { force: true }).catch(() => void 0);
		await fs.rm(tempDir, {
			recursive: true,
			force: true
		}).catch(() => void 0);
