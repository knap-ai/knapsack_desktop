...
import { a as isAutoLinkedFileRef } from "./text-runtime-B0ZpmkER.js";
...
import { r as normalizeMatrixResolvableTarget, t as isMatrixQualifiedUserId } from "./target-ids-CDZUajR8.js";
import { a as EventType, l as buildMatrixReactionContent, n as persistMatrixDirectRoomMapping, o as MsgType, s as RelationType, t as inspectMatrixDirectRooms } from "./direct-management-BdPeLV7M.js";
import { r as parseBuffer } from "./lib-BOE2t2B6.js";
import { i as isStrictDirectRoom } from "./direct-room-CQa3X8cA.js";
...
async function loadMatrixSendClientRuntime() {
	matrixSendClientRuntimePromise ??= import("./client-bootstrap-BSfcJukK.js");
...
async function sendMessageMatrix(to, message, opts = {}) {
	...
	return await withResolvedMatrixSendClient({
		...
		if (opts.mediaUrl) {
			...
			lastMessageId = await sendContent(content) ?? lastMessageId;
			...
			for (const chunk of textChunks) {
				...
				lastMessageId = await sendContent(followup) ?? lastMessageId;
			}
...
async function editMessageMatrix(roomId, originalEventId, newText, opts = {}) {
	return await withResolvedMatrixSendClient({
		...
		const replaceMentions = diffMatrixMentions(extractMatrixMentions(newContent), previousMentions);
