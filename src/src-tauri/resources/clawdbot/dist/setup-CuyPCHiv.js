...
import { f as readConfigFileSnapshot, x as writeConfigFile } from "./io-DhtVmzAJ.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-CbjtrFX4.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-AooBPI9-.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-mXCGXxzF.js");
		const { logConfigUpdated } = await import("./logging-CRy-SqDQ.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-gFBGg02L.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BBYXoz-J.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-BMR4wjAo.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-vqxRF9zQ.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-D0Xol5Fn.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C0IUyhUQ.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-C_I9d7IN.js");
		const { setupChannels } = await import("./onboard-channels-BWAtMpCw.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-Os_mmTke.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-C5WQseL2.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-D3z_0JdW.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DQXEyAtu.js");
