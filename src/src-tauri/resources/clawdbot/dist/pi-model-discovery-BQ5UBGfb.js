...
import { r as getPluginRegistryState } from "./runtime-state-qlSEpF_I.js";
import { c as loadAuthProfileStoreForSecretsRuntime, n as ensureAuthProfileStore } from "./store-CfHec0eX.js";
import { P as resolveProviderSyntheticAuthWithPlugin, _ as normalizeProviderResolvedModelWithPlugin, a as applyProviderResolvedTransportWithPlugin, i as applyProviderResolvedModelCompatWithPlugins } from "./provider-runtime-C1HXSRlX.js";
import { v as resolveProviderEnvApiKeyCandidates } from "./model-auth-markers-CZrGSAU9.js";
...
function createOpenClawModelRegistry(authStorage, modelsJsonPath, agentDir, options) {
	...
	registry.getAll = () => getAll().filter((entry) => matchesProviderFilter(entry)).map((entry) => normalizeDiscoveredPiModel(entry, agentDir));
	registry.getAvailable = () => getAvailable().filter((entry) => matchesProviderFilter(entry)).map((entry) => normalizeDiscoveredPiModel(entry, agentDir));
...
//#endregion
export { normalizeDiscoveredPiModel as a, scrubLegacyStaticAuthJsonEntriesForDiscovery as c, discoverModels as i, resolveRuntimeSyntheticAuthProviderRefs as l, PiModelRegistryClass as n, resolvePiCredentialsForDiscovery as o, discoverAuthStorage as r, addEnvBackedPiCredentials as s, PiAuthStorageClass as t };
