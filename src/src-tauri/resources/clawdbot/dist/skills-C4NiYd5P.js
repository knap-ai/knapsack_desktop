...
import { x as normalizeSkillFilter } from "./agent-scope-CYXg_wTS.js";
import { c as isPathInside } from "./file-identity-B8OnIgGC.js";
import { a as openVerifiedFileSync, i as openBoundaryFileSync } from "./boundary-file-read-LPHTYrMX.js";
import { n as resolveOpenClawPackageRootSync } from "./openclaw-root-Q4UoS1xk.js";
import { r as isPathInsideWithRealpath } from "./scan-paths-CFWv0hpJ.js";
...
import { d as mergeBundlePathLists, f as normalizeBundlePathList, o as CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH } from "./min-host-version-B_bhUtXS.js";
import { n as loadPluginManifestRegistry } from "./manifest-registry-BSmqODu2.js";
import { i as resolveSandboxPath } from "./sandbox-paths-COznanRn.js";
import { u as parseFrontmatterBlock } from "./frontmatter-BIEujD1s.js";
import { d as resolveOpenClawMetadata, f as resolveSkillInvocationPolicy, l as shouldIncludeSkill, u as parseFrontmatter } from "./env-overrides-BeEQZ3es.js";
import { fileURLToPath } from "node:url";
import fsSync from "node:fs";
import path from "node:path";
import os from "node:os";
import { createSyntheticSourceInfo, formatSkillsForPrompt } from "@mariozechner/pi-coding-agent";
...
function resolveClaudeCommandRootDirs(rootDir) {
	...
	return mergeBundlePathLists(fsSync.existsSync(path.join(rootDir, "commands")) ? ["commands"] : [], declared);
}
...
function loadEnabledClaudeBundleCommands(params) {
	...
	for (const record of registry.plugins) {
		...
		for (const relativeRoot of resolveClaudeCommandRootDirs(record.rootDir)) {
			...
			if (!fsSync.existsSync(commandRoot)) continue;
...
function listCandidateSkillDirs(dir) {
	try {
		return fsSync.readdirSync(dir, { withFileTypes: true }).filter((entry) => entry.isDirectory() && !entry.name.startsWith(".") && entry.name !== "node_modules").map((entry) => path.join(dir, entry.name)).sort((left, right) => left.localeCompare(right));
	} catch {
		return [];
	}
}
function loadSkillsFromDirSafe(params) {
	...
	let rootRealPath;
	try {
		rootRealPath = fsSync.realpathSync(rootDir);
	} catch {
		return { skills: [] };
	}
...
function readSkillFrontmatterSafe(params) {
	let rootRealPath;
	try {
		rootRealPath = fsSync.realpathSync(path.resolve(params.rootDir));
	} catch {
		return null;
	}
	const raw = readSkillFileSync({
...
//#endregion
export { filterWorkspaceSkillEntries as a, syncSkillsToWorkspace as c, resolveBundledSkillsDir as d, buildWorkspaceSkillsPrompt as i, resolvePluginSkillDirs as l, buildWorkspaceSkillCommandSpecs as n, loadWorkspaceSkillEntries as o, buildWorkspaceSkillSnapshot as r, resolveSkillsPromptForRun as s, resolveSkillsInstallPreferences as t, loadSkillsFromDirSafe as u };
