...
import { t as CONFIG_PATH } from "./paths-CD8i0MSg.js";
import { t as isBlockedObjectKey } from "./prototype-keys-C9Ot198c.js";
import { n as info, o as success, t as danger } from "./globals-BJZkpw-q.js";
import { F as validateConfigObjectRaw, N as collectUnsupportedSecretRefPolicyIssues, f as readConfigFileSnapshot } from "./io-DhtVmzAJ.js";
import { d as resolveSecretInputRef, i as coerceSecretRef, s as isValidEnvSecretRefId } from "./types.secrets-DTXJhyWe.js";
import { a as formatExecSecretRefIdValidationMessage, c as isValidSecretProviderAlias, d as validateExecSecretRefId, o as isValidExecSecretRefId, s as isValidFileSecretRefId, u as secretRefKey } from "./ref-contract-BW7up38N.js";
...
import { a as normalizeConfigIssues, n as formatConfigIssueLines } from "./issue-format-DnqtJUII.js";
import { c as resolveConfigSecretTargetByPath, n as discoverConfigSecretTargets } from "./target-registry-CO65L87H.js";
import { i as redactConfigObject, n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-DkKVOTRU.js";
import { n as setCommandJsonMode } from "./json-mode-DaChiPcs.js";
...
function parseBatchSource(opts) {
	...
	return parseBatchEntries(fsSync.readFileSync(pathname, "utf8"), "--batch-file");
}
//#endregion
...
async function runConfigSet(opts) {
	...
	try {
		...
		for (const operation of operations) setAtPath(next, operation.setPath, operation.value);
		...
		if (opts.cliOptions.dryRun) {
			...
			const errors = [];
			if (!hasJsonMode && policyIssueLines.length > 0) errors.push(...policyIssueLines.map((message) => ({
				kind: "schema",
				message
			})));
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-DsdURZhx.js");
