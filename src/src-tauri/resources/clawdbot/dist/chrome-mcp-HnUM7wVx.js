...
import { a as BrowserProfileUnavailableError, c as BrowserTabNotFoundError } from "./errors-zZtFFUsY.js";
...
async function createRealSession(profileName, userDataDir) {
	...
	const ready = (async () => {
		try {
			...
			throw new BrowserProfileUnavailableError(`Chrome MCP existing-session attach failed for profile "${profileName}". Make sure ${userDataDir ? `the configured Chromium user data dir (${userDataDir})` : "Google Chrome's default profile"} is running locally with remote debugging enabled. Details: ${err instanceof Error ? err.message : String(err)}`);
		}
	})();
	ready.catch(() => {});
	return {
...
async function getSession(profileName, userDataDir, timeoutMs) {
...
async function createEphemeralSession(profileName, userDataDir, timeoutMs) {
...
async function leaseSession(profileName, userDataDir, options = {}) {
	const cacheKey = buildChromeMcpSessionCacheKey(profileName, userDataDir);
...
async function callTool(profileName, userDataDir, name, args = {}, options = {}) {
...
async function ensureChromeMcpAvailable(profileName, userDataDir, options = {}) {
	const lease = await leaseSession(profileName, userDataDir, options);
	if (lease.temporary) await lease.session.client.close().catch(() => {});
}
...
async function listChromeMcpPages(profileName, userDataDir, options = {}) {
	return extractStructuredPages(await callTool(profileName, userDataDir, "list_pages", {}, options));
}
async function listChromeMcpTabs(profileName, userDataDir, options = {}) {
