...
import { a as formatExecSecretRefIdValidationMessage, l as resolveDefaultSecretProviderAlias, o as isValidExecSecretRefId, s as isValidFileSecretRefId } from "./ref-contract-BW7up38N.js";
import { t as encodeJsonPointerToken } from "./json-pointer-Dvh5pX8b.js";
...
function loadSecretResolve() {
	secretResolvePromise ??= import("./resolve-D9gd2lVp.js");
...
function resolveDefaultProviderEnvVar(provider) {
	return getProviderEnvVars(provider)?.find((candidate) => candidate.trim().length > 0);
}
...
function resolveRefFallbackInput(params) {
	...
	const value = (params.env ?? process.env)[fallbackEnvVar]?.trim();
...
async function promptEnvSecretRefForSetup(params) {
	...
	const envVarRaw = await params.prompter.text({
		...
		validate: (value) => {
			...
			if (!env[candidate]?.trim()) return params.copy?.envVarMissingError?.(candidate) ?? `Environment variable "${candidate}" is missing or empty in this session.`;
		}
	});
