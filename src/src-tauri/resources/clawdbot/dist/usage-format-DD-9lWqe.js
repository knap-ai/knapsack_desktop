...
import { r as DEFAULT_PROVIDER } from "./defaults-BwiMD7ye.js";
import { r as normalizeProviderId } from "./provider-id-CTiEzT1T.js";
import { f as parseModelRef, i as buildModelAliasIndex, l as modelKey, u as normalizeModelRef, v as resolveModelRefFromString } from "./model-selection-8a6zD_aX.js";
import { t as resolveOpenClawAgentDir } from "./agent-paths-BiYMSfDv.js";
import { f as normalizeProviderModelIdWithPlugin } from "./provider-runtime-Dhor2yYF.js";
import { n as resolvePluginWebSearchConfig } from "./legacy-web-search-BnVhJVMd.js";
...
function listLikePrimary(value) {
	if (typeof value === "string") return value.trim() || void 0;
	return value?.primary?.trim() || void 0;
}
function listLikeFallbacks(value) {
	if (!value || typeof value !== "object") return [];
	return Array.isArray(value.fallbacks) ? value.fallbacks.filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean) : [];
}
...
function collectConfiguredModelPricingRefs(config) {
	...
	addResolvedModelRef({
		raw: resolvePluginWebSearchConfig(config, "google")?.model,
		aliasIndex,
		refs
	});
	addResolvedModelRef({
		raw: resolvePluginWebSearchConfig(config, "xai")?.model,
		aliasIndex,
		refs
	});
	addResolvedModelRef({
	...
	addResolvedModelRef({
		raw: resolvePluginWebSearchConfig(config, "perplexity")?.model,
		aliasIndex,
		refs
	});
...
//#endregion
export { startGatewayModelPricingRefresh as a, resolveModelCostConfig as i, formatTokenCount as n, formatUsd as r, estimateUsageCost as t };
