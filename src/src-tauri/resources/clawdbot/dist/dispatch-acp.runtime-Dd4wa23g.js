...
import { B as resolveAcpThreadSessionDetailLines, H as resolveAcpAgentPolicyError, Mn as toAcpRuntimeError, On as isSessionIdentityPending, Rn as emitAgentEvent, U as resolveAcpDispatchPolicyError, fn as getAcpSessionManager, kn as resolveSessionIdentityFromMeta, pn as readAcpSessionEntry, s as EmbeddedBlockChunker } from "./pi-embedded-BYdcxQ5A.js";
import { a as generateSecureUuid } from "./secure-random-CzuMrSfY.js";
import { d as maybeApplyTtsToPayload, p as resolveTtsConfig } from "./speech-runtime-DncpdrLW.js";
import "./tts-DmlPQvyr.js";
import { s as hasOutboundReplyContent } from "./reply-payload-SzVz53hc.js";
import { n as formatToolSummary, r as resolveToolDisplay } from "./tool-display-CSvtnoK0.js";
import { _ as isMediaUnderstandingSkipError, g as MediaAttachmentCache, o as resolveMediaAttachmentLocalRoots, y as normalizeAttachments } from "./runner-D2riOkUz.js";
import { n as runMessageAction } from "./message-action-runner-D1gVvkVo.js";
import { n as routeReply } from "./route-reply-BBjY9kE8.js";
import { a as isCommandEnabled, f as maybeResolveTextAlias, y as shouldHandleTextCommands } from "./commands-registry-dA2_ExHR.js";
import { r as prefixSystemMessage } from "./system-message-BmvmDwY7.js";
import { r as resolveEffectiveBlockStreamingConfig, t as clampPositiveInteger } from "./block-streaming-BhVEIUhH.js";
import { t as formatAcpRuntimeErrorText } from "./error-text-CtgJQqdg.js";
import { r as createBlockReplyPipeline } from "./block-reply-pipeline-PsCTP6oT.js";
import { t as applyMediaUnderstanding } from "./apply-41Abj0q9.js";
...
function createAcpDispatchDeliveryCoordinator(params) {
	...
	const deliver = async (kind, payload, meta) => {
		...
		const ttsPayload = meta?.skipTts ? payload : await maybeApplyTtsToPayload({
			payload,
			cfg: params.cfg,
...
async function resolveAcpAttachments(ctx, cfg) {
	const mediaAttachments = normalizeAttachments(ctx).map((attachment) => attachment.path?.trim() ? {
		...attachment,
		url: void 0
	} : attachment);
	const cache = new MediaAttachmentCache(mediaAttachments, { localPathRoots: resolveMediaAttachmentLocalRoots({
		cfg,
		ctx
	}) });
	const results = [];
	for (const attachment of mediaAttachments) {
		...
		try {
			...
			if (isMediaUnderstandingSkipError(error)) logVerbose(`dispatch-acp: skipping attachment #${attachment.index + 1} (${error.reason})`);
			else {
...
async function finalizeAcpTurnOutput(params) {
	...
	if (ttsMode === "final" && hasAccumulatedBlockText) try {
...
async function tryDispatchAcpReply(params) {
	...
	const shouldEmitResolvedIdentityNotice = !params.suppressUserDelivery && identityPendingBeforeTurn && (Boolean(params.ctx.MessageThreadId != null && String(params.ctx.MessageThreadId).trim()) || hasBoundConversationForSession({
