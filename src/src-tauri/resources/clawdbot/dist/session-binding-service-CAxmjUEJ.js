...
import { t as getActivePluginChannelRegistry } from "./runtime-DZz1f0_h.js";
import { a as normalizeAnyChannelId } from "./registry-DTO_OK4F.js";
import { t as loadJsonFile } from "./json-file-Cl6_4Szy.js";
...
function normalizeConversationRef$1(ref) {
	return {
...
function buildConversationKey(ref) {
	const normalized = normalizeConversationRef$1(ref);
	return [
		normalized.channel,
...
function loadBindingsIntoMemory() {
	...
	for (const record of bindings ?? []) {
		...
		bindingsByConversationKey.set(buildConversationKey(record.conversation), {
			...record,
			conversation: normalizeConversationRef$1(record.conversation)
		});
	}
}
...
function resolveChannelSupportsCurrentConversationBinding(channel) {
	...
	const matchesPluginId = (plugin) => plugin.id === normalized || (plugin.meta?.aliases ?? []).some((alias) => alias.trim().toLowerCase() === normalized);
	if ((getActivePluginChannelRegistry()?.channels.find((entry) => matchesPluginId(entry.plugin))?.plugin)?.conversationBindings?.supportsCurrentConversationBinding === true) return true;
...
async function bindGenericCurrentConversation(input) {
	const conversation = normalizeConversationRef$1(input.conversation);
...
async function unbindGenericCurrentConversationBindings(input) {
	...
	if (removed.length > 0) await enqueuePersist();
	return removed;
}
const __testing$1 = {
