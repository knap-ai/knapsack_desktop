import { y as postTrustedWebToolsJson } from "./provider-web-search-BvBfxXst.js";
...
function resolveLegacyXSearchConfig(config) {
	...
	return isRecord$1(xSearch) ? cloneRecord(xSearch) : void 0;
}
function resolvePluginXSearchConfig(config) {
	const pluginConfig = config?.plugins?.entries?.xai?.config;
	if (!isRecord$1(pluginConfig?.xSearch)) return;
