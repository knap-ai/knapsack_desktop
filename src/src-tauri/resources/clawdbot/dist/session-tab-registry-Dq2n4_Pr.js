...
import { n as getBridgeAuthForPort } from "./bridge-auth-registry-Zwe97Ix0.js";
...
async function fetchBrowserJson(url, init) {
	...
	try {
		...
		const { dispatchBrowserControlRequest } = await import("./local-dispatch.runtime-D7zG4cmo.js");
...
async function browserDoctor(baseUrl, opts) {
	return await fetchBrowserJson(withBaseUrl(baseUrl, `/doctor${buildProfileQuery(opts?.profile)}`), { timeoutMs: 3e3 });
}
