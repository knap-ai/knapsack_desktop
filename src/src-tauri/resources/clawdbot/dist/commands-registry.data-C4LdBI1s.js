...
function buildChatCommands() {
	const commands = [...buildBuiltinChatCommands(), ...listChannelPlugins().filter(supportsNativeCommands).map((plugin) => defineDockCommand(plugin))];
	assertCommandRegistry(commands);
	return commands;
}
...
function getNativeCommandSurfaces() {
	...
	cachedNativeCommandSurfaces = new Set(listChannelPlugins().filter(supportsNativeCommands).map((plugin) => plugin.id));
