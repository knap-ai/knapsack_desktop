...
import { a as createProviderHttpError } from "./provider-http-errors-CqyrJgS5.js";
...
import { a as resolveOpenAIProviderConfigRecord, i as readRealtimeErrorDetail, o as trimToUndefined, r as captureOpenAIRealtimeWsClose, t as asFiniteNumber } from "./realtime-provider-shared-Xya1odbK.js";
...
var OpenAIRealtimeVoiceBridge = class OpenAIRealtimeVoiceBridge {
	...
	resolveConnectionParams() {
		...
		if (cfg.azureEndpoint && cfg.azureDeployment) return {
			url: `${cfg.azureEndpoint.replace(/\/$/, "").replace(/^http(s?):/, (_, secure) => `ws${secure}:`)}/openai/realtime?api-version=${cfg.azureApiVersion ?? "2024-10-01-preview"}&deployment=${encodeURIComponent(cfg.azureDeployment)}`,
			headers: { "api-key": cfg.apiKey }
		};
		if (cfg.azureEndpoint) return {
			url: `${cfg.azureEndpoint.replace(/\/$/, "").replace(/^http(s?):/, (_, secure) => `ws${secure}:`)}/v1/realtime?model=${encodeURIComponent(cfg.model ?? OpenAIRealtimeVoiceBridge.DEFAULT_MODEL)}`,
			headers: { Authorization: `Bearer ${cfg.apiKey}` }
		};
		return {
			url: `wss://api.openai.com/v1/realtime?model=${encodeURIComponent(cfg.model ?? OpenAIRealtimeVoiceBridge.DEFAULT_MODEL)}`,
			headers: {
