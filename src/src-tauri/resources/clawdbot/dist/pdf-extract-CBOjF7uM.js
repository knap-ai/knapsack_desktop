...
function resolveBundledDocumentExtractorCompatPluginIds(params) {
	...
	return loadPluginManifestRegistry({
		...
		env: params.env
	}).plugins.filter((plugin) => plugin.origin === "bundled" && (!onlyPluginIdSet || onlyPluginIdSet.has(plugin.id)) && (plugin.contracts?.documentExtractors?.length ?? 0) > 0).map((plugin) => plugin.id).toSorted((left, right) => left.localeCompare(right));
}
