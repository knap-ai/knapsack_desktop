    ...
    unsafe {
      ...
      extern "C" fn run_file_upload_panel(
        ...
        unsafe {
          ...
          // Guard against nil NSOpenPanel (can happen on some macOS versions).
          // Without this check, msg_send! dereferences a null pointer and the
          // extern "C" fn cannot unwind, causing the process to abort.
          if open_panel.is_null() {
            (*handler).call((nil,));
            return;
          }
