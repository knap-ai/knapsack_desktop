...
pub async fn update_curation_settings(payload: Json<UpdateCurationSettingsRequest>) -> impl Responder {
    let mut cfg = CuratorSettings::load();
    if let Some(v) = payload.enabled { cfg.enabled = v; }
    if let Some(v) = payload.sources_email { cfg.sources_email = v; }
    if let Some(v) = payload.sources_calendar { cfg.sources_calendar = v; }
    if let Some(v) = payload.sources_drive { cfg.sources_drive = v; }
    if let Some(v) = payload.sources_local_files { cfg.sources_local_files = v; }
    let _ = cfg.save();
    HttpResponse::Ok().json(CurationStatusResponse {
        success: true,
        enabled: cfg.enabled,
