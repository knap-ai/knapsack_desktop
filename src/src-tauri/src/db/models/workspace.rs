...
impl Workspace {
  ...
  pub fn delete_stale_auto_projects(
    keep_slugs: &std::collections::HashSet<String>,
  ) -> Result<usize, Error> {
    let connection = get_db_conn();
  ...
  pub fn wipe_auto_curated() -> Result<usize, Error> {
    ...
    let result = (|| -> Result<usize, rusqlite::Error> {
      ...
      let n = connection.execute("DELETE FROM workspaces WHERE auto_curated = 1", [])?;
      Ok(n)
    })();
    match result {
      Ok(n) => {
  ...
  pub fn find_all_with_documents() -> Result<Vec<Workspace>, Error> {
    ...
    let mut uuid_to_index: std::collections::HashMap<String, usize> =
  ...
  pub fn delete(uuid: String) -> Result<(), Error> {
    ...
    let result = (|| -> Result<(), rusqlite::Error> {
      ...
      connection.execute("DELETE FROM workspaces WHERE uuid = ?1", params![uuid])?;
      Ok(())
    })();
    match result {
      Ok(()) => {
...
impl WorkspaceDocument {
  ...
  pub fn delete(id: u64) -> Result<(), Error> {
    let connection = get_db_conn();
    connection.execute("DELETE FROM workspace_documents WHERE id = ?1", params![id])?;
    Ok(())
  }
}
