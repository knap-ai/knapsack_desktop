  ...
  if should_notify && !notification_message.is_empty() {
    ...
    let is_duplicate = {
      ...
      recent.iter().any(|(prev, _)| {
        ...
        prev_lower == msg_lower
          || msg_lower.contains(&prev_lower)
          || prev_lower.contains(&msg_lower)
      })
    };

    if is_duplicate {
      info!(
        "[heartbeat] Suppressed duplicate notification: {}",
      ...
      if let Err(e) = app_handle.emit_all("heartbeat_notification", &payload) {
        ...
        info!(
          "[heartbeat] Notification emitted: {}",
          &notification_message
        );
...
async fn call_llm_for_decision(prompt: &str) -> Result<LlmDecision, String> {
  ...
  let response_text = if provider.is_anthropic {
    ...
    let json: serde_json::Value =
      serde_json::from_str(&text).map_err(|e| format!("Anthropic JSON parse failed: {}", e))?;
...
fn resolve_heartbeat_provider() -> Result<HeartbeatProvider, String> {
  ...
  log::info!(
    "[heartbeat] Active provider '{}' unavailable, trying cheapest fallback",
    active
  );
  ...
  if !disable_paid || !active_is_free {
    ...
    log::info!(
      "[heartbeat] Skipping paid providers (KNAPSACK_DISABLE_PAID_FALLBACK, active={})",
      active
    );
  }
...
mod tests {
  ...
  fn test_parse_llm_decision_direct_json() {
    ...
    assert_eq!(result.message.unwrap(), "You have a meeting in 25 minutes");
  }
