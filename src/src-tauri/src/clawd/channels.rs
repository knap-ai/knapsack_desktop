...
fn humanize_config_error(channel: &str, raw_error: &str) -> String {
  ...
  if channel == "slack"
    && (lower.contains("apptoken") || lower.contains("app_token") || lower.contains("app-level"))
  {
  ...
  if channel == "discord" && lower.contains("token") {
    return "Discord requires a bot token. Create one at discord.com/developers/applications."
      .to_string();
  }
...
fn build_enable_patch(channel_patch: &str, snapshot: &serde_json::Value) -> String {
  ...
  if needs_channel_reply_tools {
    ...
    tools_val
      .as_object_mut()
      .unwrap()
      .insert("sandbox".to_string(), tools_patch["sandbox"].clone());
...
pub async fn whatsapp_login(_cfg: web::Data<SharedClawdbotConfig>) -> impl Responder {
  ...
  // Gateway/channel initialization can take longer on slower machines
  // (node bootstrap + Baileys startup). Use a longer retry window so the
  // UI reliably gets a QR instead of failing early.
  let delays = [
  ...
  for (attempt, delay) in delays.iter().enumerate() {
    ...
    match gateway_client::call_channel_method("web.login.start", Some(params.clone()), None).await {
      Ok(result) => {
...
pub async fn whatsapp_relink(_cfg: web::Data<SharedClawdbotConfig>) -> impl Responder {
  ...
  if let Ok(snapshot) = gateway_client::config_get(None).await {
    ...
    if !has_whatsapp {
      ...
      if let Err(e) = gateway_client::config_patch(&patch, &base_hash, None).await {
        eprintln!(
          "[channels] relink: failed to re-enable whatsapp config: {}",
          e
        );
      }
      gateway_client::invalidate();
  ...
  let mut last_err = String::new();
  // Re-link often follows a logout/restart cycle; allow enough time for
  // channel internals to become ready before declaring failure.
  let delays = [
    Duration::from_secs(1),
    Duration::from_secs(2),
    Duration::from_secs(3),
...
pub async fn whatsapp_login_phone(
  ...
  if let Ok(snapshot) = gateway_client::config_get(None).await {
    ...
    if !has_whatsapp {
      ...
      if let Err(e) = gateway_client::config_patch(&patch, &base_hash, None).await {
        eprintln!(
          "[channels] login-phone: failed to re-enable whatsapp config: {}",
          e
        );
      }
      gateway_client::invalidate();
  ...
  for (attempt, delay) in delays.iter().enumerate() {
    ...
    match gateway_client::call_channel_method("web.login.phone", Some(params.clone()), None).await {
      Ok(result) => {
    ...
    Ok(config_snapshot) => {
      ...
      match gateway_client::config_patch(patch, &base_hash, None).await {
        Ok(_) => HttpResponse::Ok().json(GenericResponse {
          success: true,
          message: Some(if body.enabled {
            "Voice calls enabled. Configure your Twilio/Telnyx credentials to start making calls."
              .to_string()
          } else {
...
pub async fn telegram_validate(body: web::Json<TelegramValidateRequest>) -> impl Responder {
...
pub async fn whatsapp_disconnect(
  ...
  let account_id = body.account_id.as_deref().unwrap_or("default").to_string();
...
pub async fn imessage_disconnect(_cfg: web::Data<SharedClawdbotConfig>) -> impl Responder {
      ...
      match config_patch_with_reconnect(&config_snapshot, build_patch).await {
        Ok(_) => {
          ...
          HttpResponse::Ok().json(GenericResponse {
            success: true,
            message: Some(format!(
              "{} configured. The channel should connect shortly.",
              channel
            )),
            configured: Some(true),
            linked: None,
        ...
        Err(e) => {
          log::error!(
            "[channels] {} configure config.patch failed: {}",
            channel,
            e
          );
...
#[post("/api/clawd/channels/slack/accounts/{account_id}/disconnect")]
pub async fn slack_account_disconnect(path: web::Path<String>) -> impl Responder {
  let account_id = path.into_inner();
  if !valid_channel_account_id(&account_id) {
    return HttpResponse::BadRequest().json(GenericResponse {
      success: false,
      message: Some("Invalid Slack workspace identifier.".to_string()),
      configured: None,
      linked: None,
    });
  }
...
pub async fn signal_install_cli() -> impl Responder {
  ...
  if os == "windows" {
    return HttpResponse::Ok().json(SignalCliResponse {
      ...
      version: None,
      message: Some(
        "Automatic signal-cli installation is not supported on Windows yet.".to_string(),
      ),
    });
  }
...
async fn signal_install_from_release() -> HttpResponse {
  ...
  if cfg!(target_os = "windows") {
    ...
    return HttpResponse::Ok().json(SignalCliResponse {
      ...
      version: None,
      message: Some(
        "signal-cli auto-install is not supported on Windows. Please install it manually."
          .to_string(),
      ),
    });
  }
...
fn find_signal_cli_binary(dir: &str) -> Option<String> {
  ...
  if let Ok(entries) = std::fs::read_dir(path) {
    for entry in entries.flatten() {
      let sub = entry.path();
      if sub.is_dir() {
        let sub_candidates = vec![sub.join("bin").join("signal-cli"), sub.join("signal-cli")];
...
pub async fn signal_link(body: web::Json<SignalLinkRequest>) -> impl Responder {
  ...
  let device_name = body
    .device_name
    .clone()
    .unwrap_or_else(|| "Knapsack".to_string());
  ...
  match result {
    Ok(Ok(uri)) => HttpResponse::Ok().json(SignalRegResponse {
      success: true,
      message: Some(
        "Scan this QR code with Signal (Settings > Linked Devices > Link New Device)".to_string(),
      ),
      link_uri: Some(uri),
...
pub async fn signal_register(body: web::Json<SignalRegisterRequest>) -> impl Responder {
  ...
  let result = tokio::task::spawn_blocking(move || {
    ...
    if output.status.success() {
      Ok((
        false,
        "Verification SMS sent. Enter the code you receive.".to_string(),
      ))
    } else if stderr.contains("captcha")
      || stderr.contains("CAPTCHA")
      || stderr.contains("rate limit")
    {
      Ok((
        true,
        "Captcha required. Please complete the captcha and try again.".to_string(),
      ))
    } else {
      Err(format!(
        "Registration failed: {}",
...
pub async fn telegram_user_sign_up(body: web::Json<TelegramUserSignUpRequest>) -> impl Responder {
  let phone = body.phone_number.trim().to_string();
  let hash = body.phone_code_hash.trim().to_string();
...
pub async fn telegram_provision_chief_of_staff_bot() -> impl Responder {
  match std::env::var("KNAPSACK_TG_CHIEF_OF_STAFF_BOT_TOKEN") {
    ...
    _ => HttpResponse::Ok().json(TelegramProvisionBotResponse {
      success: false,
      message: Some(
        "No pre-configured bot token found. Please enter a bot token manually.".to_string(),
      ),
      bot_token: None,
      bot_username: None,
    }),
...
fn upsert_telegram_channel_entry(
  ...
  let raw = std::fs::read_to_string(config_path).map_err(|e| format!("read openclaw.json: {e}"))?;
  let mut cfg: serde_json::Value =
    serde_json::from_str(&raw).map_err(|e| format!("parse openclaw.json: {e}"))?;
  ...
  let arr = telegram
    .as_array_mut()
    .ok_or("channels.telegram not an array")?;
...
#[allow(dead_code)]
fn remove_telegram_channel_entry(
  config_path: &std::path::Path,
  agent_id: &str,
) -> Result<(), String> {
  let raw = std::fs::read_to_string(config_path).map_err(|e| format!("read openclaw.json: {e}"))?;
...
pub async fn telegram_get_agent_bot_deep_link(
  ...
  let deeplink = format!(
    "tg://newbot?manager={}&username={}&name={}",
    m, suggested, name_enc
  );
    ...
    Ok(v) => {
      ...
      if let (Some(tok), Some(uname)) = (token.as_deref(), username.as_deref()) {
        ...
        HttpResponse::Ok().json(TelegramAgentBotProvisionResponse {
          ...
          bot_id: None,
          message: Some(
            "Child bot not created yet — complete the deep link flow in Telegram.".to_string(),
          ),
        })
      }
    }
          ...
          tokio::spawn(async move {
            if let Ok(cur) = crate::clawd::gateway_client::config_get(None).await {
              if let Some(hash) = cur.get("hash").and_then(|h| h.as_str()) {
                if !hash.is_empty() {
                  let _ =
                    crate::clawd::gateway_client::config_patch(&patch.to_string(), hash, None)
                      .await;
                }
              }
            }
        ...
        arr
          ...
          .filter_map(|e| {
            ...
            let configured = e
              ...
              .map(|t| !t.is_empty())
              .unwrap_or(false);
            Some(TelegramAgentBotStatusEntry {
              agent_id,
              username,
              configured,
            })
...
mod reconnect_retry_tests {
  ...
  fn send_failure_and_timeout_are_retryable() {
    assert!(is_connection_level_error(
      "Failed to send request: broken pipe"
    ));
