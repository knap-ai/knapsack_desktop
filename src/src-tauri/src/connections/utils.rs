...
pub async fn fetch_user_uuid(
  email: &str,
  refresh_internal: Option<String>,
) -> Result<String, FetchUuidError> {
  let mut user = None;
  ...
  let access_token = get_api_access_token(email, refresh_internal)
    .await
  ...
  if response_data.success {
    if let Some(uuid) = response_data.uuid {
      ...
      Err(FetchUuidError::ParseError(
        "UUID not found in response".into(),
      ))
    }
  } else {
...
pub async fn get_api_access_token(
  email: &str,
  refresh_internal: Option<String>,
  ...
  let result = Retry::spawn(retry_strategy, || {
    ...
    async move {
      ...
      if response.status().is_success() {
        ...
        log::warn!(
          "Server error refreshing API token ({}), retrying...",
          response.status()
        );
...
pub fn create_knapsack_api_connection(
  user_email: String,
  refresh_internal: &str,
...
pub fn get_knapsack_api_connection(
  user_email: String,
) -> Result<UserConnection, Box<dyn std::error::Error + Send + Sync>> {
  let user_connection =
    match UserConnection::find_by_user_email_and_scope(user_email, KNAPSACK_ACCESS_KEY.to_string())
    {
      Ok(uc) => uc,
      Err(error) => {
