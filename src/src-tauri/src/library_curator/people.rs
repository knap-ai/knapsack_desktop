...
impl ContactStats {
    fn score(&self) -> f32 {
        (self.email_count as f32) * EMAIL_WEIGHT
            + (self.meeting_count as f32) * MEETING_WEIGHT
    }
}
...
fn load_own_addresses() -> Vec<String> {
    ...
    if let Ok(mut stmt) = connection.prepare("SELECT DISTINCT email FROM users WHERE email IS NOT NULL AND email != ''") {
...
fn ingest_calendar(
    ...
    log::info!("[library_curator/people] scanned {} calendar events", event_count);
    Ok(())
}
...
fn attach_calendar_event(workspace_uuid: &str, event_id: u64) -> Result<(), Error> {
    let connection = get_db_conn();
    let mut stmt = connection.prepare(
        "SELECT title, description, start FROM calendar_events WHERE id = ?1",
    )?;
    let row = stmt
