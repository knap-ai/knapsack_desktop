...
pub async fn execute_workflow(workflow: &BrowserWorkflow) -> Result<WorkflowResult, Error> {
    ...
    let succeeded = results.iter().filter(|r| r.status == "success" || r.status == "fallback_success").count();
...
async fn get_snapshot(client: &Client, profile: &str) -> Result<String, Error> {
    let url = format!("{}/snapshot?profile={}&format=ai", OPENCLAW_BASE_URL, profile);
    let resp = client
        .get(&url)
        .send()
        .await
...
fn resolve_ref_from_snapshot(snapshot: &str, selector: &ElementSelector) -> Option<String> {
    ...
    if let Some(ref text) = selector.text_content {
        if !text.is_empty() && text.len() > 1 {
            let search_text = if text.len() > 50 { &text[..50] } else { text.as_str() };
            for line in &lines {
            ...
            match client.post(&act_url).json(&body).send().await {
                Ok(resp) if resp.status().is_success() => {
                    ...
                    return StepResult {
                        step_index: index,
                        action_type: action.into(),
                        status: if attempt == 0 { "success" } else { "fallback_success" }.into(),
...
async fn try_llm_fallback(
    ...
    if let Some(ref text) = selector.text_content {
        let truncated = if text.len() > 100 { &text[..100] } else { text.as_str() };
    ...
    {
        Ok(resp) if resp.status().is_success() => {
            ("fallback_success".to_string(), format!("LLM agent resolved: {}", element_desc))
        }
        Ok(resp) => {
            let err = resp.text().await.unwrap_or_default();
            ("failed".to_string(), format!("LLM fallback failed: {}", err))
        }
