...
pub async fn list_scenarios(scenarios: web::Data<SharedScenarios>) -> impl Responder {
  ...
  if let Some(ref snapshot) = dev_state.a11y_snapshot {
    ...
    if issues.is_empty() {
      results.push(QATestResult {
        ...
        screenshot_base64: None,
        details: Some(format!(
          "Scanned {} characters of a11y tree",
          snapshot.len()
        )),
      });
    } else {
...
async fn run_console_error_check(dev_url: &str) -> Vec<QATestResult> {
  ...
  if dev_state.terminal_errors.is_empty() {
    ...
    results.push(QATestResult {
      ...
      passed: false,
      message: format!(
        "{} terminal error(s) detected",
        dev_state.terminal_errors.len()
      ),
      screenshot_base64: None,
...
fn check_a11y_issues(snapshot: &str) -> Vec<A11yIssue> {
  ...
  let labeled_buttons =
    lower.matches("aria-label").count() + lower.matches("aria-labelledby").count();
