...
Section "Install" SecMain
  ...
  FileWrite $R0 '  # Show live progress by polling the destination file size.$\r$\n'
  FileWrite $R0 '  $wc   = New-Object System.Net.WebClient$\r$\n'
  FileWrite $R0 '  $done = $false; $err = $null$\r$\n'
  FileWrite $R0 '  $wc.add_DownloadProgressChanged({$\r$\n'
  FileWrite $R0 '    $pct = $args[1].ProgressPercentage$\r$\n'
  FileWrite $R0 '    $mb  = [math]::Round($args[1].BytesReceived / 1MB, 1)$\r$\n'
  FileWrite $R0 '    $tot = [math]::Round($args[1].TotalBytesToReceive / 1MB, 1)$\r$\n'
  FileWrite $R0 '    if ($pct % 5 -eq 0) { Write-Host "  $pct% — $mb MB / $tot MB" }$\r$\n'
  FileWrite $R0 '  })$\r$\n'
  FileWrite $R0 '  $wc.add_DownloadFileCompleted({$\r$\n'
  FileWrite $R0 '    if ($args[1].Error) { $script:err = $args[1].Error.Message }$\r$\n'
  FileWrite $R0 '    $script:done = $true$\r$\n'
  FileWrite $R0 '  })$\r$\n'
  FileWrite $R0 '  $wc.DownloadFileAsync([uri]$exeUrl, $dest)$\r$\n'
  FileWrite $R0 '  while (-not $script:done) { Start-Sleep -Milliseconds 200 }$\r$\n'
  FileWrite $R0 '  if ($script:err) { throw $script:err }$\r$\n'
